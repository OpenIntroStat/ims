...
```{exercise echo=TRUE, label = "test", name = "Proportion of stroke victims"}
...
Table \@ref(tab:email50DF) displays the first six rows of a data set concerning 50 emails received in 2012. These observations will be referred to as the \data{email50} data set, and they are a random sample from a larger data set that we will see in Section \@ref(categoricalData).
...
```{r email50DF}
library(openintro)
email50 %>% select(spam, num_char, line_breaks, format, number) %>%
  head() %>%
  kable(caption = "Six rows from the `email50` data matrix.")
```
...
```{exercise echo=TRUE}
We consider a publicly available data set that summarizes information about the 3,143 counties in the United States, and we call this the \data{county} data set. 
...
```{r countyDF}
library(openintro)
county <- countyComplete[,c("name", "state", "pop2000", "pop2010", "fed_spending", "poverty", "home_ownership", "housing_multi_unit", "per_capita_income", "median_household_income")]
colnames(county) <- c("name", "state", "pop2000", "pop2010", "fed_spend", "poverty", "homeownership", "multiunit", "income", "med_income")
county$fed_spend <- county$fed_spend / county$pop2010

county %>% head(7) %>% 
  kable(caption = "Seven rows from the `county` data set.") 
```
...
```{r variables, out.width="57%", fig.cap='Breakdown of variables into their respective types.'}
...
```{exercise echo=TRUE} 
Consider the variables `group` and `outcome` (at 30 days) from the stent study in Section \@ref(basicExampleOfStentsAndStrokes). 
Are these numerical or categorical variables?^[There are only two possible values for each variable, and in both cases they describe categories. Thus, each is a categorical variable.]
```
...
Dataset can be viewed through the RStudio envionment, but the data can also be investigated through the notebook features of an RMarkdown file.
...
For example `data[i,j]` will produce the $(i,j)^{th}$ intry of data, and `data[i,]` will produce the $i^{th}$ row.
...
Hadley Wickham provides a thorough discussion and advice for cleaning up the data in @Wickham14.
