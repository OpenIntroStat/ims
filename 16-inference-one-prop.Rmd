...
```{r nullDistForPHatIfLiverTransplantConsultantIsNotHelpful, fig.cap="The null distribution for $\\hat{p},$ created from 10,000 simulated studies. The left tail, representing the p-value for the hypothesis test, contains 12.22% of the simulations.", fig.width=10}
...
[^inference-one-prop-2]: There isn't sufficiently strong evidence to support an association between the consultant's work and fewer surgery complications.
...
```{r include=FALSE}
terms_chp_16 <- c(terms_chp_16, "success-failure condition", "standard error of single proportion")
```
...
```{r choosingZForCI, fig.cap="The area between -$z^{\\star}$ and $z^{\\star}$ increases as $|z^{\\star}|$ becomes larger. If the confidence level is 99%, we choose $z^{\\star}$ such that 99% of the normal curve is between -$z^{\\star}$ and $z^{\\star},$ which corresponds to 0.5% in the lower tail and 0.5% in the upper tail: $z^{\\star}=2.58.$", fig.width=10}
...
Is it reasonable use a normal distribution to model $\hat{p}$ for a hypothesis test here?[^inference-one-prop-9]
:::
