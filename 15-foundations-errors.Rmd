...
```{r include=FALSE}
terms_chp_15 <- c(terms_chp_15, "confirmation bias", "one-sided hypothesis test", "two-sided hypothesis test")
```
...
::: {.data data-latex=""}
The [`cpr`](http://openintrostat.github.io/openintro/reference/cpr.html) data from this study can be found in the [**openintro**](http://openintrostat.github.io/openintro) R package.
:::
...
```{r type1ErrorDoublingExampleFigure, fig.cap="The shaded regions represent areas where we would reject $H_0$ under the bad practices considered in when $\\alpha = 0.05.$", fig.asp = 0.5, out.width = "50%"}
