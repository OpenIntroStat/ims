...
After selecting 120 marbles, we counted 2 red for $\hat{p}_{BS1} = 0.0167.$ As we did with the randomization technique, seeing what would happen with one simulation isn't enough.
...
(ref:twoSampleNormalsStacked-cap) The two normal models shown in the Figure \@ref(fig:twoSampleNormals) but plotted together on the same scale.
...
::: {.guidedpractice}
Let $X$ represent a random variable from $N(\mu=3, \sigma=2),$ and suppose we observe $x=5.19.$ (a) Find the Z score of $x.$ (b) Use the Z score to determine how many standard deviations above or below the mean $x$ falls.[^intro-stat-inference-19]
:::
...
[^intro-stat-inference-20]: For $x_1=95.4$ mm: $Z_1 = \frac{x_1 - \mu}{\sigma} = \frac{95.4 - 92.6}{3.6} = 0.78.$ For $x_2=85.8$ mm: $Z_2 = \frac{85.8 - 92.6}{3.6} = -1.89.$
...
(a) $Z_{Mike} = \frac{67 - 70}{3.3} = -0.91\ \to\ 0.1814.$ (b) $Z_{Jim} = \frac{76 - 70}{3.3} = 1.82\ \to\ 0.9656.$
...
We find that 0.4000 falls in row $-0.2$ and between columns $0.05$ and $0.06.$ Since it falls closer to $0.05,$ we take this one: $Z=-0.25.$
...
The point estimate from this study can reasonably be modeled with a normal distribution, and a proper standard error for this point estimate is $SE = 0.078.$ Construct a 95% confidence interval.[^intro-stat-inference-33]
...
Figure \@ref(fig:95PercentConfidenceInterval) shows 25 confidence intervals for a proportion that were constructed from simulations where the true proportion was $p = 0.3.$ However, 1 of these 25 confidence intervals happened not to include the true value.
...
::: {.guidedpractice}
In Figure \@ref(fig:95PercentConfidenceInterval), one interval does not contain the true proportion, $p = 0.3.$ Does this imply that there was a problem with the simulations run?[^intro-stat-inference-34]
:::
