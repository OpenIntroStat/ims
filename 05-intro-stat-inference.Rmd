...
```{r opportunity-cost-obs-bar, fig.cap = "Segmented bar plot of results of the opportunity cost study."}
...
Another useful way to review the results from Table \@ref(tab:OpportunityCostTable) is using row proportions, specifically considering the proportion of participants in each group who said they would buy or not buy the DVD.
...
$$\hat{p}_{trmt} - \hat{p}_{ctrl} = \frac{34}{75} - \frac{19}{75} = 0.453 - 0.253 = 0.200$$
...
$$\hat{p}_{trmt, sim} - \hat{p}_{ctrl, sim} = \frac{24}{75} - \frac{29}{75} = 0.32 - 0.387 = - 0.067$$
...
```{r boot2, fig.cap = "Bootstrap sampling provides a measure of the variability of sample to sample.  Note that we are taking samples from the estimated population that was created from the sample.", out.width="75%"}
...
::: {.todo}
Add fig ref in caption, it should read `"The two normal models shown in the Figures \@ref(fig:twoSampleNormals) but plotted together on the same scale."`
:::
...
Using the normal probability table, identify the row of $-0.3$ and column of $0.03$, which corresponds to the probability $0.3707$.
...
Finally, the height $x$ is found using the Z score formula with the known mean $\mu$, standard deviation $\sigma$, and Z score $Z=0.92$:
...
::: {.todo}
Add a note / term box about advanced methods here.
:::
...
We are 95% confident that the DVD purchase rate resulting from the treatment is between 4.7% and 35.3% lower than in the control group.
...
> We are XX% confident that the population parameter is between... *Incorrect* language might try to describe the confidence interval as capturing the population parameter with a certain probability.
