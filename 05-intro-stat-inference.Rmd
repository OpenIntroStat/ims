...
The distribution of $\hat{p}_{BS}$ for the example scenario is shown in Figure \@ref(fig:boot3), and the full bootstrap distribution for the medical consultant data is shown in Figure \@ref(fig:MedConsBSSim).

```{r boot3, fig.cap = "The bootstrapped proportion is estimated for each bootstrap sample.  The resulting bootstrap distribution (dotplot) provides a measure for how the proportions vary from sample to sample", warning = FALSE,  out.width="75%"}
...
If we choose 62 marbles out of the bag (one at a time with replacement) and compute the proportion of simulated patients with complications, $\hat{p}_{BS},$ then this "bootstrap" proportion represents a single simulated proportion from the "resample from the sample" approach.
...
A bootstrap sample behaves similarly to how an actual sample from a populationwould behave, and we compute the point estimate of interest (here, compute $\hat{p}_{BS}$).

Due to theory that is beyond this text, we know that the bootstrap proportions $\hat{p}_{BS}$ vary around $\hat{p}$ in a similar way to how different sample proportions (i.e., values of $\hat{p}$) vary around the true parameter $p.$
...
histPlot(diffs, breaks = breaks, axes = FALSE,
         col = COL[2,2], xlab = xlab=expression('H'[0]': p'[T]' - p'[C]' = 0'), ylab = "")
axis(1)
...
(b) What is the $97.5^{th}$ percentile of the male heights? As always with normal probability problems, first draw a picture.[^intro-stat-inference-27]
:::

[^intro-stat-inference-27]: Remember: draw a picture first, then find the Z score.
...
(b) What is the probability that a male adult is shorter than 5'9'' (69 inches)?[^intro-stat-inference-28]
:::

[^intro-stat-inference-28]: Numerical answers: (a) 0.1131.
    (b) 0.3821.
...
::: {.guidedpractice}
What percent of SAT takers get between 1500 and 2000?[^intro-stat-inference-29]
:::

[^intro-stat-inference-29]: This is an abbreviated solution.
...
::: {.guidedpractice}
What percent of adult males are between 5'5'' and 5'7''?[^intro-stat-inference-30]
:::

[^intro-stat-inference-30]: 5'5'' is 65 inches.
    5'7'' is 67 inches.
...
Similarly there should be an area of about 0.95 between $Z=-2$ and $Z=2.$[^intro-stat-inference-31]
:::

[^intro-stat-inference-31]: First draw the pictures.
...
?[^intro-stat-inference-32]
:::

[^intro-stat-inference-32]: (a) 900 and 2100 represent two standard deviations above and below the mean, which means about 95% of test takers will score between 900 and 2100.
...
::: {.guidedpractice}
Compute the area between -1.96 and 1.96 for a normal distribution with mean 0 and standard deviation 1.[^intro-stat-inference-33]
:::

[^intro-stat-inference-33]: We will leave it to you to draw a picture.
...
The point estimate from this study can reasonably be modeled with a normal distribution, and a proper standard error for this point estimate is $SE = 0.078.$ Construct a 95% confidence interval.[^intro-stat-inference-34]
...
[^intro-stat-inference-34]: We've used $SE = 0.078$ from the last section.
...
::: {.guidedpractice}
In Figure \@ref(fig:95PercentConfidenceInterval), one interval does not contain the true proportion, $p = 0.3.$ Does this imply that there was a problem with the simulations run?[^intro-stat-inference-35]
:::

[^intro-stat-inference-35]: No.
