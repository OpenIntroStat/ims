...
As you learned in Chapter \@ref(getting-started-with-data), a **randomized experiment** is done to assess whether or not one variable (the **explanatory** variable) causes changes in a second variable (the **response** variable).
...
The data are visualized in Figure \@ref(fig:genderrand1).
Note that the promoted decision is colored in red (promoted) and white (not promoted).
Additionally, the observations are broken up into the male and female groups.

```{r genderrand1, out.width="50%", fig.cap="The gender descriminiation study can be thought of as 48 red and white cards."}
...
From this table, we can compute a difference that occurred from chance alone: \begin{align*}
\\hat{p}\_{trmt, sim} - \\hat{p}\_{ctrl, sim}
...
we estimate $p$ using the data, and label this estimation $\hat{p}$.
...
-->
```
```{r boot2, fig.cap = "Bootstrap sampling provides a measure of the variability of sample to sample.  Note that we are taking samples from the estimated population that was created from the sample.", warning = FALSE,  out.width="75%"}
...
```{r boot3, fig.cap = "The bootstrapped proportion is estimated for each bootstrap sample.  The resulting bootstrap distribution (histogram) provides a measure for how the proportions vary from sample to sample", warning = FALSE,  out.width="75%"}
include_graphics("05/figures/boot1prop3.png")
```

```{r pollBootDotPlot, fig.cap="The creation of the histogram that goes in the one proportion bootstrapping ppt figures", echo = FALSE, eval = FALSE, warning=FALSE, fig.width=10}
...
::: {.guidedpractice}
In a simulation of 62 patients, about how many would we expect to have had a complication?[^intro-stat-inference-9]
:::
...
A Stanford University graduate student named Elizabeth Newton conducted an experiment using the tapper-listener game.[^intro-stat-inference-10]
...
Here we've repeated the entire simulation ten times: \begin{align*}
0.0417 \\quad 0.0250 \\quad 0.0250 \\quad 0.0083 \\quad
0.0500 \\quad 0.0333 \\quad 0.0250 \\quad 0.000 \\quad
0.0083 \\quad 0.000
\\end{align\*} <!-- % round(rbinom(10, 120, 0.5) / 120, 3) --> As before, we'll run a total of 10,000 simulations using a computer.
...
```
::: {.guidedpractice}
Do the data provide "statistically significant" evidence against the claim that 50% of listeners can guess the tapper's tune?[^intro-stat-inference-11]
:::

[^intro-stat-inference-11]: Because 50% is not in the interval estimate for the true parameter, there is statistically significant evidence.
...
::: {.guidedpractice}
If we want to be very certain we capture the population parameter, should we use a wider interval or a smaller interval?[^intro-stat-inference-12]
:::

[^intro-stat-inference-12]: If we want to be more certain we will capture the fish, we might use a wider net.
...
3.  Randomly sample a 62nd observation from the 62 patients.
...
-->
```
```{r FourCaseStudies, fig.cap="The null distribution for each of the four case studies presented in previous.", warning=FALSE, fig.width=10}
...
::: {.guidedpractice}
Describe the shape of the distributions and note anything that you find interesting.[^intro-stat-inference-13]
:::

[^intro-stat-inference-13]: In general, the distributions are reasonably symmetric.
...
It is often difficult to construct a reliable confidence interval in many situations using simulations.[^intro-stat-inference-14]
However, doing so is reasonably straightforward using the normal distribution.
:::

[^intro-stat-inference-14]: The percentile bootstrap method has been introduced in Section \@ref(ConfidenceIntervals).
...
It is so common that people know it as a variety of names including the **normal curve**\index{normal curve}, **normal model**\index{normal model}, or **normal distribution**\index{normal distribution}.[^intro-stat-inference-15]
...
[^intro-stat-inference-15]: It is also introduced as the Gaussian distribution after Frederic Gauss, the first person to formalize its mathematical expression.
...
-->
```
```{r twoSampleNormalsStacked, fig.cap="The two normal models shown in the previous figures but plotted together on the same scale.", warning=FALSE, fig.width=10}
...
The two distributions in Figure \@ref(fig:twoSampleNormalsStacked) can be written as \begin{align*}
...
Who performed better?[^intro-stat-inference-17]
:::

[^intro-stat-inference-17]: We use the standard deviation as a guide.
...
We compute the Z score for an observation $x$ that follows a distribution with mean $\mu$ and standard deviation $\sigma$ using \begin{eqnarray*}
Z = \frac{x-\mu}{\sigma}
\end{eqnarray*}
:::

::: {.guidedpractice}
Use Tom's ACT score, 24, along with the ACT mean and standard deviation to compute his Z score.[^intro-stat-inference-18]
:::

[^intro-stat-inference-18]: $Z_{Tom} = \frac{x_{Tom} - \mu_{ACT}}{\sigma_{ACT}} = \frac{24 - 21}{5} = 0.6$
...
Compute the Z scores for possums with head lengths of 95.4 mm and 85.8 mm.[^intro-stat-inference-20]
:::

[^intro-stat-inference-20]: For $x_1=95.4$ mm: $Z_1 = \frac{x_1 - \mu}{\sigma} = \frac{95.4 - 92.6}{3.6} = 0.78$.
...
::: {.guidedpractice}
Which of the two brushtail possum observations in the previous guided practice is more *unusual*?[^intro-stat-inference-21]
:::

[^intro-stat-inference-21]: Because the *absolute value* of Z score for the second observation is larger than that of the first, the second observation has a more unusual head length.
...
::: {.guidedpractice}
Determine the proportion of SAT test takers who scored better than Ann on the SAT.[^intro-stat-inference-22]
:::

[^intro-stat-inference-22]: If 84% had lower scores than Ann, the number of people who had better scores must be 16%.
...
With $\mu=1500$, $\sigma=300$, and the cutoff value $x=1630$, the Z score is computed as \begin{eqnarray*}
Z = \frac{x - \mu}{\sigma} = \frac{1630 - 1500}{300} = \frac{130}{300} = 0.43
\end{eqnarray*} We use software to find the percentile of $Z=0.43$, which yields 0.6664.
...
Draw the normal curve representing this exercise, shading the lower region instead of the upper one.[^intro-stat-inference-23]
:::

[^intro-stat-inference-23]: We found the probability to be 0.6664.
...
\end{eqnarray*} Using the normal probability table, identify the row of $-0.3$ and column of $0.03$, which corresponds to the probability $0.3707$.
...
(b) What percent of SAT takers did better than Stuart?[^intro-stat-inference-24]
:::

[^intro-stat-inference-24]: Numerical answers: (a) 0.9772.
    (b) 0.0228.

Based on a sample of 100 men,[^intro-stat-inference-25] the heights of male adults between the ages 20 and 62 in the US is nearly normal with mean 70.0'' and standard deviation 3.3''.

[^intro-stat-inference-25]: This sample was taken from the USDA Food Commodity Intake Database.
...
(a) $Z_{Mike} = \frac{67 - 70}{3.3} = -0.91\ \to\ 0.1814$.
(b) $Z_{Jim} = \frac{76 - 70}{3.3} = 1.82\ \to\ 0.9656$.
...
\end{eqnarray*} Solving for $x_{Erik}$ yields the height 69.18 inches.
...
Finally, the height $x$ is found using the Z score formula with the known mean $\mu$, standard deviation $\sigma$, and Z score $Z=0.92$: \begin{eqnarray*}
...
[^intro-stat-inference-26]: Remember: draw a picture first, then find the Z score.
    ...
    (a) We look for 0.95 in the probability portion (middle part) of the normal probability table, which leads us to row 1.6 and (about) column 0.05, i.e., $Z_{95}=1.65$.
    Knowing $Z_{95}=1.65$, $\mu = 1500$, and $\sigma = 300$, we setup the Z score formula: $1.65 = \frac{x_{95} - 1500}{300}$.
    We solve for $x_{95}$: $x_{95} = 1995$.
    (b) Similarly, we find $Z_{97.5} = 1.96$, again setup the Z score formula for the heights, and calculate $x_{97.5} = 76.5$.
...
::: {.guidedpractice}
What percent of SAT takers get between 1500 and 2000?[^intro-stat-inference-28]
:::
...
::: {.guidedpractice}
What percent of adult males are between 5'5'' and 5'7''?[^intro-stat-inference-29]
:::

[^intro-stat-inference-29]: 5'5'' is 65 inches.
    5'7'' is 67 inches.
...
Similarly there should be an area of about 0.95 between $Z=-2$ and $Z=2$.[^intro-stat-inference-30]
:::

[^intro-stat-inference-30]: First draw the pictures.
...
\end{align*} We can either use statistical software or look up $Z = 2.56$ in the normal probability table to determine the right tail area: 0.0052, which is about the same as what we got for the right tail using the randomization approach (0.006).
...
In the context of a hypothesis test, the Z score for a point estimate is \begin{align*}
Z = \frac{\text{point estimate} - \text{null value}}{SE}
\end{align*} The standard error in this case is the equivalent of the standard deviation of the point estimate, and the null value comes from the null hypothesis.
:::
...
\begin{align*}
Z = \\frac{\\hat{p} - p_0}{SE\_{\\hat{p}}} = \\frac{0.048 - 0.10}{0.038} = -1.37
\\end{align\*} Identifying $Z = -1.37$ using statistical software or in the normal probability table, we can determine that the left tail area is 0.0853 which is the estimated p-value for the hypothesis test.
...
\end{align} We can be **95% confident** this interval captures the true value.
:::
...
::: {.guidedpractice}
Compute the area between -1.96 and 1.96 for a normal distribution with mean 0 and standard deviation 1.[^intro-stat-inference-32]
:::

[^intro-stat-inference-32]: We will leave it to you to draw a picture.
    The Z scores are $Z_{left} = -1.96$ and $Z_{right} = 1.96$.
...
Since the conditions for the normal approximation have already been verified, we can move forward with the construction of the 95% confidence interval: \begin{align*}
\text{point estimate}\ \pm\ 1.96 \times SE \quad \rightarrow \quad
0.20\ \pm\ 1.96 \times 0.078 \quad \rightarrow \quad
(0.047, 0.353)
\end{align*} We are 95% confident that the DVD purchase rate resulting from the treatment is between 4.7% and 35.3% lower than in the control group.
...
[^intro-stat-inference-33]: We've used $SE = 0.078$ from the last section.
...
The conditions necessary to ensure the point estimate $p_{trmt} - p_{ctrl} = 0.090$ is nearly normal have been verified for you, and the estimate's standard error is $SE = 0.028$.
Construct a 95% confidence interval for the change in 30-day stroke rates from usage of the stent.

------------------------------------------------------------------------

The conditions for applying the normal model have already been verified, so we can proceed to the construction of the confidence interval: \begin{align*}
\text{point estimate}\ \pm\ 1.96 \times SE \quad \rightarrow \quad
0.090\ \pm\ 1.96 \times 0.028 \quad \rightarrow \quad
(0.035, 0.145)
\end{align*} We are 95% confident that implanting a stent in a stroke patient's brain increased the risk of stroke within 30 days by a rate of 0.035 to 0.145.
...
Does this imply that there was a problem with the simulations run?[^intro-stat-inference-34]
:::

[^intro-stat-inference-34]: No.
