...
```{r include=FALSE}
terms_chp_26 <- c("inference on multiple linear regression")
```
...
lm(interest_rate ~ debt_to_income + term + credit_checks, data = loans) %>%
  ...
  kbl(booktabs = TRUE, caption = "Summary of a linear model for predicting interest rate based on `debt_to_income`, `term`, and `credit_checks`. Each of the variables has its own coefficient estimate as well as p-value significance.") %>%
...
```{r include=FALSE}
terms_chp_26 <- c(terms_chp_26, "predictor")
```
...
Figure \@ref(fig:money) illustrates a sample of U.S. coins, their total worth (`amount`), the total `number of coins`, and the `number of low coins`.
...
```{r coinfig, fig.cap = "Plot describing the amount of money (USD) as a fucntion of the number of coins and the number of low coins.  As you might expect, the amount of money is more highly postively correlated with the total number of coins than with the number of low coins."}
# Data are simulated
...
num_coins <- c(9,10,3,5,10,37,28,9,11,4,6,17,15,7,9,1,5,9,36,30,47,13,5,7,18,16)
num_low_coins <- c(4,8,0,4,9,34,9,3,2,2,5,12,11,4,8,0,4,9,34,9,3,2,2,5,12,11)

money <- data.frame(`number of coins` = num_coins, 
                    `number of low coins` = num_low_coins,
                    `total amount` = amount)

p1 <- ggplot(money, aes(y = total.amount, x = number.of.coins)) + 
  geom_point() +
  labs(x = "Number of coins", y = "Total amount ($)")

p2 <- ggplot(money, aes(y = total.amount, x = number.of.low.coins)) + 
  geom_point() +
  labs(x = "Number of low coins", y = "Total amount ($)")
...
lm(amount ~ number.of.coins, data = money) %>%
  ...
  kbl(booktabs = TRUE, caption = "Linear model output predicting the total amount of money based on the total number of coins.", digits = 2) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE)
```
...
lm(amount ~ number.of.low.coins, data = money) %>%
  tidy() %>%
  mutate(p.value = ifelse(p.value < .0001, "<0.0001", round(p.value, 4))) %>%
  kbl(booktabs = TRUE, caption = "Linear model output predicting the total amount of money based on the number of low coins.", digits = 2) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE)
```
...
Note also, however, that the significance on the `number of low coins` coefficient changed from Table \@ref(tab:coinlow) to Table \@ref(tab:coinhighlow).
...
lm(amount ~ number.of.coins + number.of.low.coins, data = money) %>%
  tidy() %>%
  mutate(p.value = ifelse(p.value < .0001, "<0.0001", round(p.value, 4))) %>%
  kbl(booktabs = TRUE, caption = "Linear model output predicting the total amount of money based on both the total number of coins and the number of low coins.", digits = 2) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE)
```
...
```{r include=FALSE}
terms_chp_26 <- c(terms_chp_26, "multicollinearity")
```
...
```{r include=FALSE}
terms_chp_26 <- c(terms_chp_26, "cross validation", "prediction error")
```
...
lm(body_mass_g ~ bill_length_mm, data = penguins) %>% 
  tidy() %>%
   ...
   kbl(booktabs = TRUE, caption = "The smalller model: least squares estimates of the regression model predicting `body_mass_g` from `bill_length_mm`.", digits = 2) %>%
 kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE) 
```
...
lm(body_mass_g ~ bill_length_mm + bill_depth_mm  + flipper_length_mm + sex + species, data = penguins) %>% 
  tidy()%>%
  mutate(p.value = ifelse(p.value < .0001, "<0.0001", round(p.value, 3))) %>%
   kbl(booktabs = TRUE, caption = "The larger model: least squares estimates of the regression model predicting `body_mass_g` from `bill_length_mm`, `bill_depth_mm`, `flipper_length_mm`, `sex`, and `species`.", digits = 2) %>%
 kable_styling(bootstrap_options = c("striped", "condensed"),
               latex_options = "striped", full_width = FALSE)
```
