...
The dataset includes results from 10,000 loans, and we will be looking at a subset of the available variables, some of which will be new from those we saw in earlier chapters.
...
Two of the other variables are categorical (`verified_income` and `issue_month`), each of which can take one of a few different non-numerical values; we will discuss how these are handled in the model in Section \@ref(ind-and-cat-predictors).
...
Let us start by fitting a linear regression model for interest rate with a single predictor indicating whether a person has a bankruptcy in their record:
...
Let us figure out how to use the equation for each level of the `verified_income` variable.
...
However, note that the situation may be more complex, and there may be confounding variables that we did not account for.
...
::: {.guidedpractice data-latex=""}
Suppose you added another predictor to the model, but the variance of the errors $Var(e_i)$ did not go down.
...
However, when there is only $k = 1$ predictors, adjusted $R^2$ is very close to regular $R^2$, so this nuance is not typically important when the model has only one predictor.
...
It is common (and hip, at least in the statistical world) to refer to models that have undergone such variable pruning as **parsimonious**.
...
The full model may not be the best model, and if it is not, we want to identify a smaller model that is preferable.
...
Let us consider two models, which are shown in Table \@ref(tab:loans-full-for-model-selection) and Table \@ref(tab:loans-full-except-issue-month).
...
This is because the decision for whether to include a given variable depends on the other variables that are already in the model.
...
Stepwise selection can also be carried out with decision criteria other than adjusted $R^2$, such as p-values, which you will learn about in Chapters \@ref(inf-model-slr) onwards, or AIC (Akaike information criterion) or BIC (Bayesian information criterion), which you might learn about in more advanced courses.
