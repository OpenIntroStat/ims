...
Generally, as you have seen in the logistic regression modeling examples, it is imperative that the response variable in binary.
...
The first logistic regression model condition --- independence of the outcomes --- is reasonable if we can assume that the emails that arrive in an inbox within a few months are independent of each other with respect to whether they are spam.
...
Let us first visualize these data by plotting the true classification of the emails against the model's fitted probabilities, as shown in Figure \@ref(fig:spam-predict).
...
We would like to assess the quality of the model.
...
Using the example above and focusing on each of the variable p-values (here we will not discuss the p-value associated with the intercept), we can write out the four different hypotheses (associated with the p-value corresponding to each of the coefficients / row in Table \@ref(tab:emaillogmodel)):
...
It turns out that in units of standard errors (0.02 here), 0.07 is actually quite far from zero, it is all about context!
