...
Because, as it turns out, the conditions of working with the normal distribution are not met (see Section \@ref(one-prop-norm)), the uncertainty associated with the sample proportion should not be modeled using the normal distribution. However, we would still like to assess the hypotheses from the previous Guided Practice in absence of the normal framework. To do so, we need to evaluate the possibility of a sample value ($\hat{p}$) as far below the null value, $p_0=0.10$ as what was observed. The devaition of the sample value from the hypothesized parameter is usually quantified with a p-value.
...
need a new physical object.size
```
...
In the previous Guided Practice, the p-value is *estimated*. It is not exact because the simulated null distribution itself is not exact, only a close approximation. However, we can generate an exact null distribution and p-value using the binomial model which is covered in the Appendix, Section \@ref(probability).
...
### One proprotion: mathematical model {#one-prop-norm}
...
We have seen $\hat{p}$ to be the sample proportion.  The value $z^{\star}$ determines the confidence level (previously set to be 1.96) and will be discussed in detail in the examples following.  The value of the standard error, $SE$, changes depending on the statistic of interest.

```{block2, type = "onebox", echo = TRUE}
**Standard Error of one proportion, $\hat{p}$**
  
The variability of a single proportion, $\hat{p}$ is well described by:
...
We already did the randomization structure.  one quick example here then an outline which represents the structure of the test (?).
