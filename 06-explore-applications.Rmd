...
When creating a graphic, keep in mind what it is that you would like your reader to see.
...
Of course, you should still think about how visually pleasing your visualization is, and if you are adding color for making it visually pleasing without drawing attention to a particular feature, that might be fine.
...
Also note that not everyone sees color the same way, often it is useful to add color and one more feature (e.g., pattern) so that you can refer to the features you are drawing attention to in multiple ways.
...
As seen in Figure \@ref(fig:brexit-bars), the alphabetical ordering is not particularly meaningful for describing the data.
...
brexit <- tibble(
  opinion = c(
    ...
    rep("Very badly", 818), rep("Do not know", 151)
    ),
  region = c(
...
p_ordered <- ggplot(data = brexit, 
                    aes(y = fct_rev(fct_relevel(opinion, c("Very well", "Fairly well", "Fairly badly", "Very badly", "Do not know"))))) +
  geom_bar() + 
    labs(y = "Opinion", x = "Count")
...
brexit <- brexit %>%
  ...
  mutate(Opinion = fct_relevel(opinion, c("Very well", "Fairly well", "Fairly badly", "Very badly", "Do not know")))
...
Do not forget titles and, if possible, include the source of the data.
