...
3.  **Meat consumption and Life Expectancy.** In data collected for @You:2022, total meat intake is associated with life expectancy (at birth) in 175 countries.
    ...
    The scatterplot on the top left displays the relationship between life expectancy at birth plotted against per capita meat consumption.
    The scatterplot on the top right displays the same relationship colored by income status of the country.
    The set of scatterplots across the below displays the same relationship broken apart by income status of the country.
    ...
    c.  Is the relationship between meat consumption and life expectancy stronger, similar, or weaker when broken down by income bracket in each of the separate plots along the bottom (as compared with the relationship when combined in the top left figure)?
...
4.  **Arrival delays.** Consider all of the flights out of New York City in 2013 that flew into Puerto Rico (BQN) or San Francisco (SFO) on the following two airlines: JetBlue (B6) or United Airlines (UA).
    ...
    p_delay + p_delay_city + plot_layout(guides='collect') & 
      theme(legend.position='bottom')
    ```
...
5.  **Training for the 5K.** Nico signs up for a 5K (a 5,000 metre running race) 30 days prior to the race.
