...
In data collected for @You:2022, total meat intake is associated with life expectancy (at birth) in 175 countries.  Meat intake is measured in kg per capita per year (averaged over 2011 to 2013). The scatterplot on the top left displays the relationship between life expectancy at birth plotted against per capita meat consumption.  The scatterplot on the top right displays the same relationship colored by income status of the country.  The scatterplot below displays the same relationship, now broken apart by income status of the country.
    ...
    (p_life + p_life_confound) / p_life_confound2
    ```
    ...
    c.  Is the relationship between meat consumption and life expectancy stronger, similar, or weaker when broken down by income bracket in the bottom figure (as compared with the relationship when combined in the top left figure)?

1.  **Arrival delays.** 
Consider all of the flights out of New York City in 2013 that flew into San Francisco (SFO) or Puerto Rico (BQN) on the following two airlines: JetBlue (B6) or United Airlines (UA).  We consider the relationship between day of the year and arrival delay. Note that a negative arrival delay means that the flight arrived early.  The figures display a least squares regression line for arrival delay versus time (day of the year).^[The `flights` data used in this exercise can be found in the [**nycflights13**](https://github.com/tidyverse/nycflights13) R package.]
    ...
    p_delay <- flights |>
      ...
      ggplot(aes(x = time_hour, y = arr_delay, color = carrier)) +
      #geom_point(size = 0.001, alpha = 0.5) +
      geom_smooth(method = "lm", se = FALSE) + 
      labs(x = "", y = "", subtitle =  "Arrival delay (min)") +
...
```
    ...
    a.  Does it seem like there are differences in arrival delays across time when looking at BQN and SFO airports combined (left figure)? Explain.

    b.  Does it seem like there are difference in arrival delays across time when looking at BQN and SFO airports separately (right figure)? Explain.
