...
The data frame below contains information on Marvel Cinematic Universe films through the Infinity saga (a 23 movie storyline spanning from Ironman in 2008 to Endgame in 2019). 
For each of the movies the data frame contains information on the title (`movies`), the number of hours (`length_hrs`) and number of minutes (`length_min`) of the movie length, the release date (`release_date`), the box office totals for opening weekend in the US (`opening_weekend_us`), all box office totals in the US (`gross_us`), and all box office totals worldwide (`gross_world`). These values are given in millions of US Dollars. How many observations and how many variables does this data frame have?^[The data used in this exercise can be found in the **openintro** package: [`mcu_films`](http://openintrostat.github.io/openintro/reference/mcu_films.html).]
...
How many observations and how many variables does this data frame have?^[The data used in this exercise can be found in the **openintro** package: [`run17`](http://openintrostat.github.io/openintro/reference/run17.html).]
...
1.  **UK baby names.** 
The visualization below shows the number of baby girls born in the United Kingdom (comprised of England & Wales, Northern Ireland, and Scotland) over the years.^[The data used in this exercise can be found in the **ukbabynames** package: [`ukbabynames`](https://mine-cetinkaya-rundel.github.io/ukbabynames/reference/ukbabynames.html).]
...
The Stanford Open Policing project gathers, analyzes, and releases records from traffic stops by law enforcement agencies across the United States. Their goal is to help researchers, journalists, and policy makers investigate and improve interactions between police and the public. The following is an excerpt from a summary table created based off of the data collected as part of this project. [@pierson2017large]
    ...
    sop <- read_csv("data/stanford-open-policing-combined-data.csv") %>%
      ...
      rename(
        drvier_race_eth    = driver_race,
        car_search_rate    = search_rate,
        driver_arrest_rate = arrest_rate
        ) %>%
      mutate(across(everything(), as.character))
    
    sop_top    <- sop %>% filter(state == "AZ")
    sop_filler <- sop %>% slice(n = 1) %>% 
      mutate(across(everything(), str_replace, ".*", "..."))
    sop_bottom <- sop %>% filter(state == "WI")
