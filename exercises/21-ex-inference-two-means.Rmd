...
Make sure to state your hypotheses clearly, check relevant conditions, and interpret your results in context of the data.[@ggplot2]
    ...
    diamonds_100_99 <- diamonds %>%
      mutate(ppc = price / (carat*100)) %>%
      filter(carat %in% c(0.99, 1))
    
    nn_df <- diamonds_100_99 %>%
      filter(carat == 0.99)
    
    # weird sampling to match old code/data
    set.seed(123)
    rows_to_sample <- sample(1:length(diamonds_100_99$ppc[diamonds_100_99$carat == 1]), size = 23, replace = FALSE)
    one_df <- diamonds_100_99 %>% 
      filter(carat == 1) %>% 
      slice(rows_to_sample)
    
    diamonds_100_99_sample <- bind_rows(nn_df, one_df) %>%
      mutate(carat = paste(carat, "carats"))
    ...
    ggplot(diamonds_100_99_sample, aes(y = carat, x = ppc)) +
      geom_boxplot() +
