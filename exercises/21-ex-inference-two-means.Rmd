...
The randomization distribution (with 1,000 repetitions) below describes the null distribution of the difference in sample means (of price per carat) if there really was no difference in the population from which these diamonds came.^[The [`diamonds`](https://ggplot2.tidyverse.org/reference/diamonds.html) data used in this exercise can be found in the **ggplot2** R package.] [@ggplot2]
    ...
    diamonds_sample %>%
      ...
      labs(
        ...
        x = "Differences in average price per carat\n(0.99 carats - 1 carat)", 
        y = "Count"
        )
    ```
...
The original observed difference in lizard speeds is $\bar{x}_{Western fence} - \bar{x}_{Sagebrush} = 0.7 \mbox{m/sec}.$ The histogram below shows the distribution of average differences when speed has been randomly allocated across lizard species 1,000 times.^[The [`lizard_run`](http://openintrostat.github.io/openintro/reference/lizard_run.html) data used in this exercise can be found in the **openintro** R package.] [@Adolph:1987]
    ...
    lizard_run %>%
      ...
      labs(
        title = "1,000 randomized differences in means",
        x = "Differences in average randomized running speed across lizard species\n(Western fence - Sagebush)", 
        y = "Count"
        )
    ```
...
Using the bootstrap distribution, create a (rough) 95% confidence interval for the true population difference in prices per carat of diamonds that weigh 0.99 carats and 1 carat. [@ggplot2]
    ...
    diamonds_100_99_sample %>%
      ...
      geom_histogram(fill = IMSCOL["green", "full"]) +
      labs(
        title = "1,000 bootstrap differences in means",
        x = "Differences in average price per carat\n(0.99 carats - 1 carat)",
        y = "Count"
      )
    ```
...
Using the bootstrap distribution, create a (rough) 90% confidence interval for the true population difference in price per carat for Western fence lizard as compared with Sagebrush lizard. [@Adolph:1987]
    ...
    lizard_run %>%
      ...
      labs(
        ...
        x = "Differences in average bootstrapped running speed across lizard species\n(Western fence - Sagebush)", 
        y = "Count"
        )
    ```
...
Make sure to state your hypotheses clearly, check relevant conditions, and interpret your results in context of the data. [@ggplot2]
    ...
    diamonds_100_99_summary %>%
      kbl(linesep = "\\addlinespace", booktabs = TRUE, align = "lccc", col.names = c("", "Mean", "SD", "n"), digits = 2) %>%
      kable_styling(bootstrap_options = c("striped", "condensed"), 
                    latex_options = c("striped", "HOLD_position"),
                    full_width = FALSE) %>%
...
Two user experience designers at the company, Noëlly and Müge, are tasked with conducting the analysis of the data collected.
They agree on how the null hypothesis should be set: on average, users spend the same amount of time on Page A and Page B.
Noëlly believes that Page B will provide a better experience for users and hence wants to use a one-tailed test, Müge believes that a two-tailed test would be a better choice.
...
Below are some summary statistics from this dataset along with box plots showing the distribution of weights by feed type.^[The [`chickwts`](https://stat.ethz.ch/R-manual/R-devel/library/datasets/html/chickwts.html) data used in this exercise can be found in the **datasets** R package.] [@data:chickwts]
...
Do these data provide strong evidence of a difference between the average fuel efficiency of cars with manual and automatic transmissions in terms of their average city mileage?^[The [`epa2021`](http://openintrostat.github.io/openintro/reference/epa2021.html) data used in this exercise can be found in the **openintro** R package.] [@data:epaMPG2021]
    ...
    epa2021_sample %>%
      ...
      kbl(linesep = "\\addlinespace", booktabs = TRUE, align = "lccc", col.names = c("CITY", "Mean", "SD", "n"), digits = 2) %>%
      kable_styling(bootstrap_options = c("striped", "condensed"), 
                    latex_options = c("striped", "HOLD_position"),
                    full_width = FALSE)
...
If your hypothesis test yields a statistically significant result, discuss whether or not the higher average weight of chickens can be attributed to the casein diet. [@data:chickwts]
...
Each year the US Environmental Protection Agency (EPA) releases fuel economy data on cars manufactured in that year. 
Below are summary statistics on fuel efficiency (in miles/gallon) from random samples of cars with manual and automatic transmissions manufactured in 2021. 
Do these data provide strong evidence of a difference between the average fuel efficiency of cars with manual and automatic transmissions in terms of their average highway mileage? [@data:epaMPG2021]
    ...
    epa2021_sample %>%
      ...
      kbl(linesep = "\\addlinespace", booktabs = TRUE, align = "lccc", col.names = c("HIGHWAY", "Mean", "SD", "n"), digits = 2) %>%
      kable_styling(bootstrap_options = c("striped", "condensed"), 
                    latex_options = c("striped", "HOLD_position"),
                    full_width = FALSE)
