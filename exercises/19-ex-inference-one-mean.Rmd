...
As you have seen, the tails of a $t-$distribution are longer than the standard normal which results in $t^{\star}_{df}$ being larger than $z^{\star}$ for any given confidence level. When finding a CI for a population mean, explain how mistakenly using $z^{\star}$ (instead of the correct $t^{*}_{df}$) would affect the confidence level.
