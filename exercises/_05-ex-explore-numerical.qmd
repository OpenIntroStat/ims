1.  **Mammal life spans.** Data were collected on life spans (in years) and gestation lengths (in days) for 62 mammals.
    A scatterplot of life span versus length of gestation is shown below.[^mammal_life_spans_q-1]
    ...
    [^mammal_life_spans_q-1]: The [`mammals`](http://openintrostat.github.io/openintro/reference/mammals.html) data used in this exercise can be found in the [**openintro**](http://openintrostat.github.io/openintro) R package.

1.  **Associations.** Indicate which of the plots show (a) a positive association, (b) a negative association, or (c) no association.
...
1.  **Infant mortality.** The infant mortality rate is defined as the number of infant deaths per 1,000 live births.
    This rate is often used as an indicator of the level of health in a country.
    The relative frequency histogram below shows the distribution of estimated infant death rates for 224 countries for which such data were available in 2014.[^infant_mortality_rel_freq_q-1]
    ...
    [^infant_mortality_rel_freq_q-1]: The [`cia_factbook`](http://openintrostat.github.io/openintro/reference/cia_factbook.html) data used in this exercise can be found in the [**openintro**](http://openintrostat.github.io/openintro) R package.
...
1.  **Medians and IQRs.** For each part, compare distributions A and B based on their medians and IQRs.
...
1.  **Means and SDs.** For each part, compare distributions A and B based on their means and standard deviations.
...
1.  **Air quality.** Daily air quality is measured by the air quality index (AQI) reported by the Environmental Protection Agency.
    ...
    AQI was reported for a 356 days in 2022 in Durham, NC. The histogram below shows the distribution of the AQI values on these days.[^air_quality_durham_q-1]
    ...
    [^air_quality_durham_q-1]: The [`pm25_2022_durham`](http://openintrostat.github.io/openintro/reference/pm25_2022_durham.html) data used in this exercise can be found in the [**openintro**](http://openintrostat.github.io/openintro) R package.
...
1.  **Histograms vs. box plots.** Compare the two plots below.
...
1.  **Distributions and appropriate statistics.** For each of the following, state whether you expect the distribution to be symmetric, right skewed, or left skewed.
...
1.  **Oscar winners.** The first Oscar awards for best actor and best actress were given out in 1929.
    ...
    Compare the distributions of ages of best actor and actress winners.[^oscar_winners_q-1]
    ...
    [^oscar_winners_q-1]: The [`oscars`](http://openintrostat.github.io/openintro/reference/oscars.html) data used in this exercise can be found in the [**openintro**](http://openintrostat.github.io/openintro) R package.
...
1.  **Income at the coffee shop.** The first histogram below shows the distribution of the yearly incomes of 40 patrons at a college coffee shop.
...
1.  **A new statistic.** The statistic $\frac{\bar{x}}{median}$ can be used as a measure of skewness.
...
1.  **Commute times.** The US census collects data on the time it takes Americans to commute to work, among many other variables..
    The histogram below shows the distribution of average commute times in 3,142 US counties in 2017.
    Also shown below is a spatial intensity map of the same data.[^county_commute_times_q-1]
    ...
    [^county_commute_times_q-1]: The [`county_complete`](http://openintrostat.github.io/openintro/reference/county_complete.html) data used in this exercise can be found in the [**usdata**](http://openintrostat.github.io/usdata) R package.

1.  **Hispanic population.** The US census collects data on race and ethnicity of Americans, among many other variables.
    ...
    Also shown is a histogram of logs of these values.[^county_hispanic_pop_q-1]
    ...
    [^county_hispanic_pop_q-1]: The [`county_complete`](http://openintrostat.github.io/openintro/reference/county_complete.html) data used in this exercise can be found in the [**usdata**](http://openintrostat.github.io/usdata) R package.

1.  **NYC marathon winners.** The histogram and box plots below show the distribution of finishing times for male and female (combined) winners of the New York City Marathon between 1970 and 2023.[^marathon_winners_q-1]
    ...
    [^marathon_winners_q-1]: The [`nyc_marathon`](http://openintrostat.github.io/openintro/reference/nyc_marathon.html) data used in this exercise can be found in the [**openintro**](http://openintrostat.github.io/openintro) R package.
