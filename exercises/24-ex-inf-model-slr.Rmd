...
A linear model is built to predict height based on shoulder girth (circumference of shoulders measured over deltoid muscles), both measured in centimeters.^[The [`bdims`](http://openintrostat.github.io/openintro/reference/bdims.html) data used in this exercise can be found in the **openintro** R package.] [@Heinz:2003]
...
The scatterplot and least squares summary below show the relationship between weight measured in kilograms and height measured in centimeters of 507 physically active individuals. [@Heinz:2003]
    ...
    c.  Do the data provide convincing evidence that an increase in height is associated with an increase in weight? State the null and alternative hypotheses, report the p-value (using a mathematical model), and state your conclusion.

    d.  The correlation coefficient for height and weight is `r r_wgt_hgt`. Calculate $R^2$ and interpret it in context.

1.  **Body measurements, percentile bootstrap interval.** 
...
A histogram of these slopes is shown below. [@Heinz:2003]
    ...
    a. Using the percentile bootstrap method and the histogram above, find a 98% confidence interval for the slope parameter.
...
A linear model is built to predict height based on shoulder girth (circumference of shoulders measured over deltoid muscles), both measured in centimeters. [@Heinz:2003]
    ...
    b. Find a 98% confidence interval for the slope parameter using the SE bootstrap method.
...
The table below shows the output of a linear model annual murders per million (`annual_murders_per_mil`) from percentage living in poverty (`perc_pov`) in a random sample of 20 metropolitan areas.
    ...
    lm(annual_murders_per_mil ~ perc_pov, data = murders) %>%
      tidy() %>%
      mutate(p.value = ifelse(p.value < 0.0001, "<0.0001", round(p.value, 4))) %>%
      kbl(linesep = "\\addlinespace", booktabs = TRUE, align = "lrrrr", digits = 4) %>%
      kable_styling(bootstrap_options = c("striped", "condensed"), 
                    latex_options = c("striped", "HOLD_position"),
                    full_width = FALSE) %>%
      column_spec(1, width = "10em", monospace = TRUE) %>%
...
1.  **Murders and poverty, percentile bootstrap interval.**
...
A linear model is built to predict annual murders per million (`annual_murders_per_mil`) from percentage living in poverty (`perc_pov`) in a random sample of 20 metropolitan areas.
    ...
    b. Find a 90% confidence interval for the slope parameter using the SE bootstrap method.
...
Here, we study the relationship between the father's age and the weight of the baby.^[The [`births14`](http://openintrostat.github.io/openintro/reference/births14.html) data used in this exercise can be found in the **openintro** R package.] [@data:births14]
...
The scatterplot and least squares summary below show the relationship between baby's weight (measured in pounds) and father's age or a random sample of babies. [@data:births14]
...
1.  **Baby's weight and father's age, percentile bootstrap interval.**
US Department of Health and Human Services, Centers for Disease Control and Prevention collect information on births recorded in the country.
The data used here are a random sample of 1000 births from 2014.
...
Below is the bootstrap distribution of the slope statistic from 1,000 different bootstrap samples of the data. [@data:births14]
    ...
    a. Using the percentile bootstrap method and the histogram above, find a 95% confidence interval for the slope parameter.
...
Here, we study the relationship between the father's age and the weight of the baby. [@data:births14]
    ...
    b. Find a 95% confidence interval for the slope parameter using the SE bootstrap method.
    
    c. Interpret the confidence interval in the context of the problem.

1. **I heart cats.**
Researchers collected data on heart and body weights of 144 domestic adult cats. 
The table below shows the output of a linear model predicting heat weight (measured in grams) from body weight (measured in kilograms) of these cats.^[The [`cats`](https://stat.ethz.ch/R-manual/R-patched/library/MASS/html/cats.html) data used in this exercise can be found in the **MASS** R package.]
...
Many people believe that weight, drinking habits, and many other factors are much more important in predicting blood alcohol content (BAC) than simply considering the number of drinks a person consumed. Here we examine data from sixteen student volunteers at Ohio State University who each drank a randomly assigned number of cans of beer. These students were evenly divided between men and women, and they differed in weight and drinking habits. Thirty minutes later, a police officer measured their blood alcohol content (BAC) in grams of alcohol per deciliter of blood. The scatterplot and regression table summarize the findings. ^[The [`bac`](http://openintrostat.github.io/openintro/reference/bac.html) data used in this exercise can be found in the **openintro** R package.] [@Malkevitc+Lesser:2008] 
