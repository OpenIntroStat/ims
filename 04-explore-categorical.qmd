...
@fig-loan-homeownership-app-type-bar-plot shows three such plots that visualize @fig-loan-homeownership-app-type-bar-plot-1 is a **stacked bar plot**.
...
@fig-loan-homeownership-app-type-bar-plot-2 is a **standardized bar plot** (also known as **filled bar plot**).
...
Finally, @fig-loan-homeownership-app-type-bar-plot-3 is a **dodged bar plot**.
...
@fig-loan-waffle-1 and @fig-loan-waffle-2 display two examples of waffle charts: one of homeownership and one of loan status, respectively.

```{r}
#| label: fig-loan-waffle
#| fig-cap: Waffle charts of homeownership and loan status.
#| fig-subcap:
#|   - Homeownership, with levels rent, mortgage, and own.
#|   - Loan status, with levels current, fully paid, in grace period, and late.
#| fig-alt: |
...
#| tbl-cap: |
#|   The median household income from a random sample of 5 counties with
...
Plot A in @fig-countyIncomeSplitByPopGainFacetHist displays the same information as @fig-countyIncomeSplitByPopGain-1, however here the distributions of median household income for counties with and without population gain are faceted across two plotting windows.
...
::: {#fig-countyIncomeSplitByPopGainFacetHist layout="[[ 25, 75 ]]"}


```{r}
#| label: fig-countyIncomeSplitByPopGainFacetHist-1
#| fig-cap: Facet by population gain.
#| fig-alt: |
...
#| fig-cap: Facet by both population gain and metropolitan area.
#| fig-alt: |
