...
```{r include = FALSE}
terms_chp_13 <- c(terms_chp_13, "parameter", "point estimate", "statistic")
```
...
[^foundations-bootstrapping-1]: About 5% of the patients (6.2 on average) in the simulation will have a complication, though we will see a little variation from one simulation to the next.
...
ggplot(bsprop_med, aes(x = bsprop)) +
  ...
  annotate("segment", 
           ...
           linetype = "dashed") +
  annotate("text", x = bsprop_med_summary$bsprop_025, y = 1200, label = "2.5\npercentile") +
  annotate("text", x = bsprop_med_summary$bsprop_975, y = 1200, label = "97.5\npercentile") +
  labs(
...
[^foundations-bootstrapping-2]: This case study is described in [Made to Stick](http://www.openintro.org/redirect.php?go=made-to-stick&redirect=simulation_textbook_pdf_preliminary) by Chip and Dan Heath.
...
ggplot(bsprop_tap, aes(x = bsprop)) +
  ...
  annotate("segment", 
           ...
           linetype = "dashed") +
  annotate("text", x = bsprop_tap_summary$bsprop_025, y = 1200, label = "2.5\npercentile") +
  annotate("text", x = bsprop_tap_summary$bsprop_975, y = 1200, label = "97.5\npercentile") +
  labs(
...
::: {.guidedpractice data-latex=""}
Do the data provide "statistically significant" evidence against the claim that 50% of listeners can guess the tapper's tune?[^foundations-bootstrapping-3]
:::

[^foundations-bootstrapping-3]: Because 50% is not in the interval estimate for the true parameter, there is statistically significant evidence.
...
::: {.guidedpractice data-latex=""}
If we want to be very certain we capture the population parameter, should we use a wider interval or a smaller interval?[^foundations-bootstrapping-4]
:::
...
```{r include = FALSE}
terms_chp_13 <- c(terms_chp_13, "bootstrap sample", "bootstrap percentile confidence interval", "sampling with replacement") 
```
...
inference_method_summary_table %>%
  ...
  kbl(booktabs = TRUE, caption = "Summary of bootstrapping as an inferential statistical method.",
