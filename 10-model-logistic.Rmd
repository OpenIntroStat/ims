...
```{r include=FALSE}
terms_chp_10 <- c("logistic regression", "generalized linear model")
```
...
::: {.data data-latex=""}
The data from this study can be found in the [**openintro**](http://openintrostat.github.io/openintro) R package: [`resume`](http://openintrostat.github.io/openintro/reference/resume.html).
:::
...
resume_names_1 %>%
  ...
  kbl(booktabs = TRUE, align = "lllllllll", 
...
resume_variables %>%
  kbl(booktabs = TRUE, caption = caption_helper("Descriptions of nine variables from the `resume` dataset. Many of the variables are indicator variables, meaning they take the value 1 if the specified characteristic is present and 0 otherwise.")) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
...
```{r include=FALSE}
terms_chp_10 <- c(terms_chp_10, "probability of an event")
```
...
```{r include=FALSE}
terms_chp_10 <- c(terms_chp_10, "transformation")
```
...
```{r include=FALSE}
terms_chp_10 <- c(terms_chp_10, "logit transformation")
```
...
(a) If a randomly chosen resume from those sent out is considered, and it does not list honors, then `honors` takes the value 0 and the right side of the model equation equals -2.4998.
...
To convert from values on the logistic regression scale (e.g. -2.4998 and -1.6330), use the following formula, which is the result of solving for $p_i$ in the regression model:
...
resume_full_fit %>%
  ...
  kbl(booktabs = TRUE, 
      caption = "Summary table for the full logistic regression model for the resume callback example.", digits = 2, align = "lrrrrr") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = c("striped", "hold_position"), full_width = FALSE) %>%
...
Here we'll use a statistic called **Akaike information criterion (AIC)**\index{AIC}, which is an analogue to how we used adjusted R-squared in multiple regression, and we look for models with a lower AIC through a backward elimination strategy.
...
```{r include=FALSE}
terms_chp_10 <- c(terms_chp_10, "Akaike information criterion")
```
...
&\quad= - 2.7162 - 0.4364 \times \texttt{job\_city}_{\texttt{Chicago}}        + 0.0206 \times \texttt{years\_experience} \\
...
Luckily, we have 4870 resume submissions in the dataset!
...
```{r resume-predict, fig.cap = "The predicted probability that each of the 4870 resumes results in a callback. Points have been jittered so that those with nearly identical values aren’t plotted exactly on top of one another.", fig.asp = 0.4}
...
ggplot(resume_pred, aes(x = .pred_1, y = received_callback)) +
  ...
  labs(y = "Recevied callback",
       x = "Predicticted probability that resume results in a callback")
```
...
::: {.workedexample data-latex=""}
Let's consider a sex-imbalanced company that consists of 20% women and 80%[^model-logistic-2] men, and we'll suppose that the company is very large, consisting of perhaps 20,000 employees.
...
[^model-logistic-2]: A more thoughtful example would include non-binary individuals.
...
Additionally, if we would complete a handful of examples like the one above with different numbers, we'd learn that the greater the imbalance in the population groups, the more the smaller group is disproportionately impacted.[^model-logistic-3]

[^model-logistic-3]: If a proportion $p$ of a company are women and the rest of the company consists of men, then under the hypothetical situation the ratio of rates of discrimination against women versus men would be given by $(1 - p) / p$; this ratio is always greater than 1 when $p < 0.5$.
...
## Exercises {#chp10-exercises}

::: {.exercises}
```{r exercises-10, child = "exercises/10-ex-model-logistic.Rmd", eval=FALSE}
```
:::
