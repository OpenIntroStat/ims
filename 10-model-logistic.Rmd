...
resume_variables %>%
  ...
  kbl(caption = caption_helper("Descriptions of nine variables from the `resume` dataset. Many of the variables are indicator variables, meaning they take the value 1 if the specified characteristic is present and 0 otherwise.")) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
...
resume_full_fit %>%
  ...
  kbl(caption = "Summary table for the full logistic regression model for the resume callback example.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE)
```
...
resume_fit %>%
  tidy() %>%
  mutate(p.value = "<0.0001") %>%
  kbl(caption = "Summary table for the logistic regression model for the resume callback example, where variable selection has been performed using AIC.", digits = 4) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE)
```
