...
The following logistic regression model was found using statistical software:
...
(a) If a randomly chosen resume from those sent out is considered, and it does not list honors, then `honors` takes the value and the right side of the model equation equals -2.4998.
...
Here we'll use a statistic called **Akaike information criterion (AIC)**\index{AIC}, which is an analogue to how we used adjusted R-squared in multiple regression.
When using AIC for model selection, models with a lower AIC value are considered to be "better."
We will look for models with a lower AIC through a backward elimination strategy.
...
[^model-logistic-2]: A more thoughtful example would include more inclusive gender identities. 
