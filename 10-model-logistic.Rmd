...
resume_full_fit %>%
  ...
  kbl(booktabs = TRUE, caption = "Summary table for the full logistic regression model for the resume callback example.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = c("striped", "hold_position"), full_width = FALSE) %>%
  column_spec(1, monospace = TRUE)
```
...
resume_fit %>%
  tidy() %>%
  mutate(p.value = "<0.0001") %>%
  kbl(booktabs = TRUE, caption = "Summary table for the logistic regression model for the resume callback example, where variable selection has been performed using AIC.", digits = 4) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = c("striped", "hold_position"), full_width = FALSE) %>%
  column_spec(1, monospace = TRUE)
```
