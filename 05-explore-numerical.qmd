...
```{r}
#| label: loan50-amount-income
#| fig.cap: (ref:loan50-amount-income-cap)
...
```{r}
#| label: median-hh-income-poverty
#| fig.cap: (ref:median-hh-income-poverty-cap)
...
```{r}
#| label: loan-int-rate-dotplot
#| fig.cap: (ref:loan-int-rate-dotplot-cap)
...
#| fig.cap: A histogram of interest rate. This distribution is strongly skewed to the
#|   right.
ggplot(loan50, aes(x = interest_rate)) +
...
```{r}
#| label: loan50IntRateDensity
#| fig.cap: A density plot of interest rate. Again, the distribution is strongly skewed
#|   to the right.
ggplot(loan50, aes(x = interest_rate)) +
...
Figure \@ref(fig:loan50IntRateDensity) shows a **density plot** which is a smoothed out histogram.
...
::: {.guidedpractice data-latex=""}
Besides the mean (since it was labeled), what can you see in the dot plot in Figure \@ref(fig:loan-int-rate-dotplot) that you cannot see in the histogram in Figure \@ref(fig:loan50IntRateHist)?[^05-explore-numerical-6]
:::
...
#| fig.cap: Counting only prominent peaks, the distributions are (left to right) unimodal,
#|   bimodal, and multimodal. Note that the left plot is unimodal because we are counting
...
#| fig.cap: (ref:sdRuleForIntRate-cap))
box_sd1 <- tibble(
...
#| fig.cap: Three very different population distributions with the same mean (0) and
...
Using Figure \@ref(fig:severalDiffDistWithSdOf1) as an example, explain why such a description is important.[^05-explore-numerical-9]
:::

[^05-explore-numerical-9]: Figure \@ref(fig:severalDiffDistWithSdOf1) shows three distributions that look quite different, but all have the same mean, variance, and standard deviation.
...
## Box plots, quartiles, and the median {#boxplots}
...
```{r}
#| label: loan-int-rate-boxplot-dotplot
#| fig.cap: (ref:loan-int-rate-boxplot-dotplot-cap)
#| fig.asp: 0.5
...
::: {.guidedpractice data-latex=""}
Using the box plot in Figure \@ref(fig:loan-int-rate-boxplot-dotplot), estimate the values of the $Q_1,$ $Q_3,$ and IQR for `interest_rate` in the `loan50` dataset.[^05-explore-numerical-11]
:::
...
```{r}
#| label: county-unemployed-pop-transform
#| fig.cap: 'Plot A: A histogram of the percentage of unemployed in all US counties.
#|   Plot B: A histogram of log$_{10}$-transformed unemployed percentages. Plot C: A
#|   histogram of population in all US counties. Plot D: A histogram of log$_{10}$-transformed
#|   populations. For Plots B and D, the x-value corresponds to the power of 10, e.g.,
#|   1 on the x-axis corresponds to $10^1 =$ 10 and 5 on the x-axis corresponds to $10^5
#|   =$ 100,000. Data are from 2017.'
#| out.width: 100%
...
```{r}
#| label: county-pop-change-transform
...
::: {.guidedpractice data-latex=""}
What interesting features are evident in the median household income intensity map in Figure \@ref(fig:county-intensity-map-howownership-median-income)?[^05-explore-numerical-14]
:::
...
```{r}
#| label: county-intensity-map-poverty-unemp
...
```{r}
#| label: county-intensity-map-howownership-median-income
