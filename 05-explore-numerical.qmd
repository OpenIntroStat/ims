...
#|   dataset.
#| fig-alt: |
#|   A scatter plot with total income on the x-axis and loan amount on the 
...
@fig-county-intensity-map-poverty-unemp and @fig-county-intensity-map-howownership-median-income show intensity maps for poverty rate in percent (`poverty`), unemployment rate in percent (`unemployment_rate`), homeownership rate in percent (`homeownership`), and median household income in \$1000s (`median_hh_income`).
...
::: {.workedexample data-latex=""}
What interesting features are evident in the poverty and unemployment rate intensity maps in @fig-county-intensity-map-poverty-unemp?
...
#|   - Unemployment rate
#| fig-alt: |
#|   Two intensity map (also called heatmaps) of the United States.
...
::: {.guidedpractice data-latex=""}
What interesting features are evident in the median household income intensity map in @fig-county-intensity-map-howownership-median-income-2?[^05-explore-numerical-14]
:::
