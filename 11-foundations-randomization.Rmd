...
Using a few different case studies, let us look more carefully at this idea of a **randomization test**\index{randomization test}.
...
Since we would not expect the sample proportions to be *exactly* equal, even if the truth was that the promotion decisions were independent of sex, we cannot rule out random chance as a possible explanation when simply comparing the sample proportions.
:::
...
There are many nuances to hypothesis testing, so do not worry if you are not a master of hypothesis testing at the end of this section.
We will discuss these ideas and details many times in this chapter as well as in the chapters that follow.
:::
...
Before getting into the nuances of hypothesis testing, let us work through another case study.
...
In this section, we will explore whether college student consumers always consider the following: money not spent now can be spent later.
...
In this section, we will explore an experiment conducted by researchers that investigates this very question for students at a university in the southwestern United States.
[@Frederick:2009]
...
[^foundations-randomization-6]: This context might feel strange if physical video stores predate you.
    If you are curious about what those were like, look up "Blockbuster".
...
Another useful way to review the results from Table \@ref(tab:opportunity-cost-obs) is using row proportions, specifically considering the proportion of participants in each group who said they would buy the video.
...
Because this is an experiment, we will use the same procedure we applied in Section \@ref(caseStudySexDiscrimination): randomization.

Let us think about the data in the context of the hypotheses.
...
Using the same randomization technique from the last section, let us see what happens when we simulate the experiment under the scenario where there is no effect from the treatment.
...
We will simulate another set of simulated groups and compute the new difference: `r opportunity_cost_rand_dist$stat[1]`.

And again: `r opportunity_cost_rand_dist$stat[2]`.

And again: `r opportunity_cost_rand_dist$stat[3]`.

We will do this 1,000 times.
...
If you are a little puzzled, that probably means you are reading with a critical eye -- good job!
We have made a video to help clarify *why 0.05*:

<https://www.openintro.org/book/stat/why05/>

Sometimes it is also a good idea to deviate from the standard.
We will discuss when to choose a threshold different than 0.05 in Section \@ref(decerr).
:::
...
-   **Analyze the data.** Choose an analysis technique appropriate for the data and identify the p-value. So far, we have only seen one analysis technique: randomization. Throughout the rest of this textbook, we will encounter several new methods suitable for many other contexts.
