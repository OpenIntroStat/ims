...
#| out.width: 75%
#| fig.alt: Photographs of 5 different automobiles.  The cars are different color and
#|   different makes and models.  On top of the image of each car is its price; the five
#|   prices range from 9600 dollars to 27000 dollars.
...
```{r}
#| label: bootpop1mean
#| fig.cap: As seen previously, the idea behind bootstrapping is to consider the sample
...
#| fig.alt: The sample of 5 cars is drawn from a large population with other values (i.e.,
...
```{r}
#| label: bootsamps1mean
#| fig.cap: To estimate the natural variability in the sample mean, different bootstrap
...
#| fig.alt: The sample is shown being taken from the large unknown population.  The bootstrap
#|   resamples, however, are taken directly from the original sample (sampling with replacement)
#|   as if the resamples had been taken from an infinitely large proxy population.  Three
#|   bootstrap resamples of 5 cars each are shown, each resample is slightly different
#|   due to the process of resampling with replacement.
...
```{r}
#| label: bootmeans1mean
#| fig.cap: Because each of the bootstrap resamples respresents a different set of cars,
...
#| out.width: 100%
#| fig.alt: The sample is shown being taken from the large unknown population.  Three
...
#| fig.cap: The larger the degrees of freedom, the more closely the $t$-distribution
...
#| fig.cap: The $t$-distribution with 18 degrees of freedom. The area below -2.10 has
#|   been shaded.
#| fig.asp: 0.5
#| out.width: 60%
normTail(L = -2.10,
...
#| fig.cap: 'Top: The $t$-distribution with 20 degrees of freedom, with the area above
#|   1.65 shaded.'
#| out.width: 50%
normTail(U = 1.65,
  df = 12,
...
#| fig.cap: The $t$-distribution with 2 degrees of freedom, with the area further than
...
#| out.width: 75%
#| fig.alt: Photograph of a Risso's dolphin.
