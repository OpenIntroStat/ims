...
Focusing now on Statistical Inference for **numerical data**, again, we will revisit and expand upon the foundational aspects of hypothesis testing from [Chapter -@sec-foundations-randomization].
...
@fig-bootpop1mean shows how the unknown original population can be estimated by using the sample to approximate the distribution of car prices from the population of cars at Awesome Auto.

```{r}
#| label: fig-bootpop1mean
#| fig-cap: |
#|   As seen previously, the idea behind bootstrapping is to consider the sample
#|   at hand as an estimate of the population.  Sampling from the sample (of 5 cars)
#|   is identical to sampling from an infinite population which is made up of only the
#|   cars in the original sample.
#| fig-alt: |
#|   The sample of 5 cars is drawn from a large population with other values (i.e.,
#|   other car prices) unknown. The sample of five cars is replicated infinitely many
#|   times to create a proxy population where the car prices are given by the original
#|   dataset in the same relative distribution as measured in the sample.
...
#| fig-cap: The original Awesome Auto data is bootstrapped 1,000 times. The histogram
#|   provides a sense for the variability of the average car price from sample to sample.
#| fig-alt: A histogram of the means of 1000 bootstrapped samples 
...
[^19-inference-one-mean-3]: By looking at the percentile values in @fig-carsbssd, the middle 90% of the bootstrap standard deviations are given by the 5 percentile (\$3,602.5) and 95 percentile (\$8,737.2).
...
#|   provides a sense for the variability of the standard deviation of the car prices
#|   from sample to sample.
#| fig-alt: A histogram of the standard deviations of 1000 
...
#| fig-alt: Two histogram and box plot pairs.  The first pair of 
...
#| fig-alt: Two symmetric bell-shaped curves on top of one another.  
...
#| fig-alt: A normal distribution and four t distributions, all 
...
Just like a normal probability problem, we first draw the picture in @fig-tDistDF18LeftTail2Point10 and shade the area below -2.10.
...
#| fig-cap: |
#|   The $t$-distribution with 18 degrees of freedom. The area below -2.10 has
#|   been shaded.
#| fig-alt: A t distribution with 18 degrees of freedom.  The area below -2.10 has been shaded.
#| fig-asp: 0.5
#| out-width: 60%
normTail(
  L = -2.10,
...
::: {.workedexample data-latex=""}
A $t$-distribution with 20 degrees of freedom is shown in @fig-tDistDF20RightTail1Point65.
...
```{r}
#| label: fig-tDistDF20RightTail1Point65
#| fig-cap: |
#|   The $t$-distribution with 20 degrees of freedom, with the area above
#|   1.65 shaded.
#| fig-alt: A t distribution with 20 degrees of freedom.  The area above 1.65 has been shaded.
#| out.width: 50%
normTail(
  U = 1.65,
  df = 12,
...
::: {.workedexample data-latex=""}
A $t$-distribution with 2 degrees of freedom is shown in @fig-tDistDF23UnitsFromMean.
...
#| fig-cap: |
#|   The $t$-distribution with 2 degrees of freedom, with the area further than
#|   3 units from 0 shaded.
#| fig-alt: A t distribution with 2 degrees of freedom.  The area 
#|   below negative 3 and above positive 3 has been shaded.
#| out-width: 50%
normTail(
  L = -3,
  U = 3,
...
#| fig-cap: A Risso's dolphin. Photo by Mike Baird, www.bairdphotos.com.
