...
If you would like to review the variables from either dataset, see Tables \@ref(tab:loan-50-variables) and \@ref(tab:county-variables).
...
It is useful to think of the mean as the balancing point of the distribution, and it is shown as a triangle in Figure \@ref(fig:loan-int-rate-dotplot).
...
Looking for modes is not about finding a clear and correct answer about the number of modes in a distribution, which is why *prominent*\index{prominent} is not rigorously defined in this book.
...
There is some mathematical nuance here, but the end result is that doing this makes this statistic slightly more reliable and useful.
...
In this first scatterplot, it is hard to decipher any interesting patterns because the population variable is so strongly skewed (left plot).
