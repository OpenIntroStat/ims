...
Here, we discuss whether each of the variables individually is an important predictor or whether the model might be just as strong without that variable.
...
When working with multiple regression models, understanding the size and sign of the coefficient is not always as straightforward as it was with the coin example.
...
If the predictor variables have some degree of correlation, it can be quite difficult to interpret the value of the coefficient or its impact.
...
Given what we know about high correlations between body measurements, it is somewhat unsurprising that all of the variables are large (large slope estimates and small p-values) predictors of `body_mass_g`.
