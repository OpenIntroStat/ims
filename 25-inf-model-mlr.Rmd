...
In Section \@ref(inf-model-slr), you learned that the hypothesis test for a linear model with **one predictor** can be written as:
...
We ask 26 students to tell us about their individual coin dishes, collecting data on the total dollar amount, the total number of coins, and the total number of low coins.[^inf-model-mlr-1]
...
[^inf-model-mlr-1]: In all honesty, this particular dataset is fabricated, and the original idea for the problem comes from Jeff Witmer at Oberlin College.
