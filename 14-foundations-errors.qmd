...
```{r}
#| label: CPR-study-right-tail
#| fig.cap: Null distribution of the point estimate for the difference in proportions,
#|   $\hat{p}_T - \hat{p}_C.$ The shaded right tail shows observations that are at least
#|   as large as the observed difference, 0.13.
set.seed(47)
cpr_rand_dist <- cpr %>%
...
```{r}
#| label: CPR-study-p-value
#| fig.cap: Null distribution of the point estimate for the difference in proportions,
#|   $\hat{p}_T - \hat{p}_C.$ All values that are at least as extreme as +0.13 but in
#|   either direction away from 0 are shaded.
cpr_rand_dist %>%
  ggplot(aes(x = stat)) +
  geom_histogram(binwidth = 0.05) +
  labs(
...
#| fig.cap: The shaded regions represent areas where we would reject $H_0$ under the
