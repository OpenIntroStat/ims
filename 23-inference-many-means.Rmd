...
```{r include=FALSE}
terms_chp_23 <- c("ANOVA", "analysis of variance")
```
...
::: {.workedexample data-latex=""}} College departments commonly run multiple sections of the same introductory course each semester because of high demand.
...
::: {.workedexample data-latex=""}} Examine Figure \@ref(fig:toyANOVA).
...
```{r toyANOVA, fig.cap="Side-by-side dot plot for the outcomes for six groups."}
toy_anova %>%
...
mlb_players_18 %>%
  ...
  head(6) %>%
  kable(caption = "Six cases and some of the variables from the `mlb_players_18` data frame.") %>%
  kable_styling()
```
...
::: {.workedexample data-latex=""}} The player positions have been divided into three groups: outfield (OF), infield (IF), and catcher (C).
...
mlb_players_18 %>% 
  ...
  kable(
    ...
    col.names = c("Position", "$n$", "$\\bar{x}$", "$s$")
  ) %>%
  kable_styling()
```

```{r mlbANOVABoxPlot, fig.cap="Side-by-side box plot of the on-base percentage for 429 players across three groups. There is one prominent outlier visible in the infield group, but with 154 observations in the infield group, this outlier is not extreme enough to have an impact on the calculations, so it is not a concern for moving forward with the analysis."}
...
::: {.workedexample data-latex=""}} The largest difference between the sample means is between the catcher and the outfielder positions.
...
[^inference-many-means-2]: See, for example, [textbook-prosecutors_fallacy](https://statmodeling.stat.columbia.edu/2007/05/18/the_prosecutors/).

```{r include=FALSE}
terms_chp_23 <- c(terms_chp_23, "data snooping", "data fishing", "prosecutor's fallacy")
```
...
```{r include=FALSE}
terms_chp_23 <- c(terms_chp_23, "mean square between groups (MSG)", "degrees of freedom", "sum of squares between groups (SSG)", "mean square error (MSE)", "sum of squares total (SST)", "sum of squared error (SSE)")
```
...
classdata %>% 
  ...
  kable(
    ...
    col.names = c("Exam", "$n$", "$\\bar{x}$", "$s$", "minimum", "maximum")
  ) %>%
  kable_styling()
```
...
## Mathematical model for testing many means {#mathANOVA}
...
```{r include=FALSE}
terms_chp_23 <- c(terms_chp_23, "F-test")
```
...
::: {.workedexample data-latex=""}} The p-value corresponding to the shaded area in Figure \@ref(fig:fDist2And423Shaded) is equal to about 0.0066.
...
```{r include=FALSE}
terms_chp_23 <- c(terms_chp_23, "multiple comparisons")
```
