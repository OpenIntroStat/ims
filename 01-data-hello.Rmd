...
::: {.data data-latex=""}
The data from this study can be found in the [**openintro**](http://openintrostat.github.io/openintro) R package: [`stent30`](http://openintrostat.github.io/openintro/reference/stent30.html) and [`stent365`](http://openintrostat.github.io/openintro/reference/stent365.html).
:::
...
stent %>%
  ...
  mutate(patient = 1:n()) %>%
  relocate(patient) %>%
  kbl(booktabs = TRUE, caption = "Results for five patients from the stent study.",
      align = "llll") %>%
...
stent %>%
  ...
  kbl(booktabs = TRUE, caption = "Descriptive statistics for the stent study.",
...
::: {.data data-latex=""}
The data can be found in the [**openintro**](http://openintrostat.github.io/openintro) R package: [`loan50`](http://openintrostat.github.io/openintro/reference/loans_full_schema.html).
:::
...
loan50 %>% 
  ...
  slice_head(n = 6) %>%
  kbl(booktabs = TRUE, caption = caption_helper("Six observations from the `loan50` dataset"),
      row.names = TRUE) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = c("striped", "hold_position"))
```
...
loan50_var_def %>%
  kbl(booktabs = TRUE, caption = caption_helper("Variables and their descriptions for the `loan50` dataset."), 
...
A data frame where each row is a unique case (observational unit), each column is a variable, and each cell is a single value is commonly referred to as **tidy data** @wickham2014.
...
county %>%
  ...
  slice_head(n = 6) %>% 
  kbl(booktabs = TRUE, caption = caption_helper("Six observations and six variables from the `county` dataset.")) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = c("striped", "hold_position"))
```
...
county_var_def %>%
  kbl(booktabs = TRUE, caption = caption_helper("Variables and their descriptions for the `county` dataset."), 
      col.names = c("Variable", "Description")) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = c("striped", "hold_position"), full_width = TRUE) %>%
  column_spec(1, monospace = TRUE) %>%
...
::: {.data data-latex=""}
These data can be found in the [**usdata**](http://openintrostat.github.io/usdata) R package: [`county`](http://openintrostat.github.io/usdata/reference/county.html).
:::
...
```{r variables, fig.cap="Breakdown of variables into their respective types.", fig.asp = 0.5}
...
```{r county-pop-change-med-hh-income, fig.cap = "A scatterplot showing population chance against median household income. Owsley County of Kentucky, is highlighted, which lost 3.63\\% of its population from 2010 to 2017 and had median household income of \\$22,736."}
