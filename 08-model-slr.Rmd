...
temptbl %>%
 kbl(caption = "Correlation of percentage of total yield across different crops.",
...
The value of the correlation is *unitless*.
...
The only visual change to the plot is the axis *labelling* of the points.

```{r bdims-units, fig.cap = "Two scatterplots, both displaying the relationship between weights and heights of 507 physically healthy adults. In Plot A, the units are kilograms and centimeters. In Plot B, the units are pounds and inches. Also noted on both plots is the correlation coefficient, $r = 0.72.$"}
...
elmhurst %>%
  slice_head(n = 5) %>%
  kbl(caption = "First five rows of the `elmhurst` dataset.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE)
```
...
m_ga_fi %>%
  kbl(caption = "Summary of least squares fit for the Elmhurst data.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE) 
```
...
m_total_pr_cond_tidy %>%
  kbl(caption = "Least squares ression summary for the final auction price against the condition of the game.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE) 
```
...
```{r outlier-plots-1, fig.cap = "Three plots, each with a least squares line and corresponding residual plot. Each data set has at least one outlier.", out.width="100%", fig.width = 8}
...
```{r outlier-plots-2, fig.cap="Three plots, each with a least squares line and residual plot. All data sets have at least one outlier.", out.width="100%"}
