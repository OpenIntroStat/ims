...
For the Elmhurst data, we could write the equation of the least squares regression line as $$\widehat{\texttt{aid}} = \beta_0 + \beta_{1}\times \texttt{family\_income}$$
...
elmhurst %>%
  slice_head(n = 5) %>%
  kbl(booktabs = TRUE, caption = caption_helper("First five rows of the `elmhurst` dataset.")) %>%
...
m_ga_fi %>%
  kbl(booktabs = TRUE, caption = "Summary of least squares fit for the Elmhurst data.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE) 
```
...
m_total_pr_cond_tidy %>%
  kbl(booktabs = TRUE, caption = "Least squares ression summary for the final auction price against the condition of the game.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE) 
```
...
```{r outlier-plots-1, fig.cap = "Three plots, each with a least squares line and corresponding residual plot. Each data set has at least one outlier.", out.width = "100%", fig.width = 8, fig.asp = 0.6}
...
```{r include = FALSE}
terms_chp_8 <- c(terms_chp_8, "high leverage", "influential point", "outlier", "leverage point")
```
