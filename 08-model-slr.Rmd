...
Linear models can be used for prediction or to evaluate whether there is a linear relationship between a numerical variable on the horizontal axis and the average of the numberical variable on the vertical axis.
:::
...
```{r imperfLinearModel, fig.cap = "Three data sets where a linear model may be useful even though the data do not all fall exactly on the line.", out.width="100%", fig.asp = 0.4}
...
ggplot(possum, aes(x = total_l, y = head_l)) +
  ...
  geom_segment(aes(x = 85, y = preds[2], xend = 85, yend = obs[2]-0.5), 
               color = IMSCOL["green", "full"], inherit.aes = FALSE) +
...
::: {.guidedpractice}
Compute the residuals for the observation marked by a green diamond, $(85.0, 98.6),$ and the observation marked by a pink triangle, $(95.5, 94.0),$ in the figure using the linear relationship $\hat{y} = 41 + 0.59x.$[^model-slr-2]
:::
...
```{r outlierPlots1, fig.cap="Three plots, each with a least squares line and corresponding residual plot. Each data set has at least one outlier.", out.width="100%", fig.width = 8}
...
Correlation is a measure of the strength of the linear relationship of two variables, without specifying which variable is the explanatory and which is the respons
e. Future chapters will focus on generalizing the linear model from the sample of data to claims about the population of interes
t.\
