...
```{r include=FALSE}
terms_chp_8 <- c("predictor", "outcome")
```
...
```{r include=FALSE}
terms_chp_8 <- c(terms_chp_8, "residuals")
```
...
::: {.guidedpractice}
Compute the residuals for the observation marked by a green diamond, $(85.0, 98.6),$ and the observation marked by a yellow triangle, $(95.5, 94.0),$ in the figure using the linear relationship $\hat{y} = 41 + 0.59x.$[^model-slr-2]
:::

[^model-slr-2]: Green diamond: $\hat{y} = 41+0.59x = 41+0.59\times 85.0 = 91.15 \rightarrow e = y - \hat{y} = 98.6-91.15=7.45.$ This is close to the earlier estimate of 7.
...
```{r include=FALSE}
terms_chp_8 <- c(terms_chp_8, "correlation")
```

We can compute the correlation using a formula, just as we did with the sample mean and standard deviation.
This formula is rather complex[^model-slr-3], and like with other statistics, we generally perform the calculations on a computer or calculator.
...
```{r include=FALSE}
terms_chp_8 <- c(terms_chp_8, "least squares line")
```
...
> So clearly folks the climate debate rages on.[^model-slr-7]
>
> Stephen Colbert\
> April 6th, 2010
...
```{r include=FALSE}
terms_chp_8 <- c(terms_chp_8, "extrapolation")
```
...
```{r include=FALSE}
terms_chp_8 <- c(terms_chp_8, "R-squared")
```
...
```{r include=FALSE}
terms_chp_8 <- c(terms_chp_8, "coefficient of determination")
```
...
```{r include=FALSE}
terms_chp_8 <- c(terms_chp_8, "total sum of squares", "sum of squared error")
```
...
```{r include=FALSE}
terms_chp_8 <- c(terms_chp_8, "indicator variable")
```
...
```{r include=FALSE}
terms_chp_8 <- c(terms_chp_8, "high leverage", "influential point", "outlier")
```
