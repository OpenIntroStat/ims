...
%{
%\eoce{Cyril Burt, a researcher was interested in the possible effect of nurture on nature is studying the intelligence levels of identical twins raised in different conditions. He randomly sampled 27 pairs of identical twins (one raised by birth parents and the other raised in a foster home) and gave them IQ tests. He hypothesized that how the twins are raised will have an effect on their IQ scores. The histograms below show the distributions of IQ scores in the these groups as well as some summary statistics. \\
%\begin{minipage}[c]{0.7\textwidth}
%\begin{center}
%\includegraphics[width=120mm]{06/figures/eoce/twins}
%\end{center}
%\end{minipage}
...
\item Since p-value $< \alpha$ (use $\alpha = 0.05$ since not given) we reject $H_0$. The data provide convincing evidence that New Yorkers sleep less than 8 hours per night on average.
...
\begin{enumerate}[(a)]
\setlength{\itemsep}{0mm}
\item Calculate a 90\% confidence interval for the true average amount of time students who use social networking sites at this college spend on social networking sites per day.
...
\begin{align*}
T_{df} &= \frac{\bar{x} - \mu}{\frac{s}{\sqrt{n}}} = \frac{1900 - 1806}{\frac{310}{\sqrt{30}}} = -1.66 \\
df &= n - 1 = 30 - 1 = 29 \\
0.10 &< p-value < 0.20
\end{align*}
...
\begin{center}
\includegraphics[width=0.75\textwidth]{06/figures/eoce/prius09}
\end{center}
\begin{enumerate}[(a)]
\setlength{\itemsep}{0mm}
...
&= (-0.92, 4.92)
\end{align*}
We are 95\% confident that those in the group that got the weight loss pill lost 0.92 lbs less to 4.92 lbs more than those in the placebo group.
...
\item Based on your confidence interval, is there a significant evidence that there is a difference in performance in engines made in the two factories? If so, can you tell which factory produces motors with lower performance? Explain.
...
\eoce{Chicken farming is a multi-billion dollar industry, and any methods that increase the growth rate of young chicks can reduce consumer costs while increasing company profits, possibly by millions of dollars. An experiment was conducted to measure and compare the effectiveness of various feed supplements on the growth rate of chickens. Newly hatched chicks were randomly allocated into six groups, and each group was given a different feed supplement. Their weights in grams after six weeks are given along with feed types in the data set called \texttt{chickwts}. Below are some summary statistics from this data set along with box plots showing the distribution of weights by feed type. \citep{data:chickwts} \vspace{2mm}\\
...
$H_0: \mu_M = \mu_A$ \\
$H_0: \mu_M \ne \mu_A$ \\
We are told to assume that assumptions and conditions for inference are satisfied, and since sample sizes are small, we use a T test.

\[ T = \frac{(\bar{x}_M - \bar{x}_A) - (\mu_M - \mu_A)}{\sqrt{ \frac{s_M^2}{n_M} + \frac{s_A^2}{n_A} }} = \frac{(21.08 - 15.62) - 0}{ \sqrt{\frac{4.29^2}{26} + \frac{2.76^2}{26}} } = 5.46 \]
\[ df = min(n_M - 1, n_A - 1) = min(26 - 1, 26 - 1) = 25 \]
\[ p-value = P(|t| > 5.6) < 0.01 \]
Since p-value $< \alpha$ (use $\alpha = 0.05$ since not given), reject $H_0$. The data provide strong evidence that there is a difference in the average city mileage between cars with automatic and manual transmissions.}\label{epaMPG2010}
...
$H_A: \mu_{T} \ne \mu_{C}$. \\
Since population standard deviations are unknown and samples are small we calculate a T score.
\[ T = \frac{(\bar{x}_T - \bar{x}_C) - (\mu_T - \mu_C)}{\sqrt{ \frac{s_T^2}{n_T} + \frac{s_C^2}{n_C} }} = \frac{(4.9 - 6.1) - 0}{ \sqrt{\frac{1.8 ^2}{22} + \frac{1.8 ^2}{22}} } = -2.21 \]
\[ df = min(n_1 - 1, n_2 - 1) = min(22 - 1, 22 - 1) = 21 \]
\[ 0.02 < p-value < 0.05 \]
Since p-value $<$ 0.05, we reject $H_0$. The data provide convincing evidence that the average number of food items recalled by the patients in the treatment and control groups are different.
}
}
...
\begin{center}
\includegraphics[width=0.85\textwidth]{06/figures/eoce/egypt}
\end{center}
\end{enumerate}
}
{
...
\begin{center}
\includegraphics[width=0.85\textwidth]{06/figures/eoce/art}
\end{center}
\end{enumerate}
}
{
...
\begin{minipage}[c]{0.5\textwidth}
\begin{center}
\includegraphics[width= 0.9\textwidth]{figures/artSoln}
\end{center}
\end{minipage}
\end{enumerate}
}
...
\begin{center}
\includegraphics[width=0.85\textwidth]{06/figures/eoce/socExp}
\end{center}
\begin{enumerate}[(a)]
\setlength{\itemsep}{0mm}
...
\begin{center}
\includegraphics[width=0.85\textwidth]{06/figures/eoce/yawn}
\end{center}
\begin{enumerate}[(a)]
\setlength{\itemsep}{0mm}
\item What are the hypotheses?
