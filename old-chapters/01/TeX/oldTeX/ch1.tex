...
Each row relates to a single possum or \term{case} in the table, and contains that individual possum's measurements for seven variables. For example, Possum 104 is a 3 year old female.
\begin{figure}[tb]
   \centering
...
Each column of Table~\ref{possumDF} represents an attribute known about each case, and these attributes are called \term{variables}. For instance, the \var{age} variable holds the age of every possum in the data set. Descriptions of all of these variables are given in Table~\ref{possumVariables}.
\begin{table}[ht]
\begin{center}\small
\begin{tabular}{l l}
\hline
...
\begin{enumerate}
\item[(1)] If a possum has a shorter-than-average head, will its skull width usually be smaller or larger than the average skull width? \label{possumHeadSizeQuestion}
\item[(2)] Will males or females, on average, be longer? \label{possibleCausationQuestionForPossums}
\item[(3)] Which population of possum will be larger on average: \resp{Vic} or \resp{other}?
...
Give an answer (guess) to each of the questions above without examining the data\footnote{Exercises scattered throughout this chapter are generally informal and are intended to be very brief. Questions at the end of the chapter are more formal and intensive.}.
\end{exercise}
...
First consider \var{price}, which is said to be a \term{numerical} variable since the values it takes are numbers \emph{and} those numbers have a meaningful ordering. That is, all 50 cars could be ordered from most expensive to least expensive or vice-versa.
...
The \emph{population} mean is also computed in the same way, however, it has a special label: $\mu_x$. The symbol, $\mu$\marginpar[\hfill $\mu$]{$\mu$}, is the Greek letter \emph{mu} and represents a population average. \\
...
Histograms are especially convenient for describing the data's \term{shape}\label{shapeFirstDiscussed}. Figure~\ref{carsPriceHist} shows that most cars have a lower price, while fewer cars have higher prices. When data trails off to the right in this way and has a longer right tail, the shape is said to be \term{skewed to the right}\footnote{Other ways to describe right-skewed data: \term{right skewed}, \term{skewed to the high end}, or \term{skewed to the positive end}.}.
...
\end{eqnarray*}
If we square these deviations and then take an average, the result is about equal to the sample \term{variance}\label{varianceIsDefined}, denoted by $s_x^2$\marginpar[\hfill $s_{x}^2$]{$s^2_{x}$}:
\begin{eqnarray*}
s_x^2 = \frac{(-4.1)^2 + (13.9)^2 + (6.7)^2}{54-1} = \frac{16.8 + 193.2 + 44.9}{53} = 132.4
\end{eqnarray*}
...
The \term{standard deviation} is defined as the square root of the variance: $s_x^{}=\sqrt{132.4} = 11.5$\marginpar[\hfill $s_x^{}$]{$s_x^{}$}. The subscript of $x$ in $s_x^2$ and $s_x^{}$ is used as a reminder that these are the variance and standard deviation of the observations represented by $x_1^{}$, $x_2^{}$, ..., $x_n^{}$. \\
...
\begin{exercise}
On page~\pageref{shapeFirstDiscussed}, the shape of a distribution was introduced. A description of the shape of a distribution should include modality and whether the distribution is symmetric or skewed to one side. Using Figure~\ref{severalDiffDistWithSdOf1} as evidence, explain why such a description is important.
\end{exercise}

\begin{exercise}
Describe the distribution of the \var{price} variable, shown in a histogram on page~\pageref{carsPriceHist}. Your description should incorporate the center, variability, and shape of the distribution, and it should also be placed in context of the problem: the price of cars.
\end{exercise}
...
In practice, the variance and standard deviation are sometimes used as a means to an end, where the ``end'' is being able to accurately estimate the uncertainty associated with a sample's estimate. For example, the variance and standard deviation are useful in determining how close the sample mean is to the population mean. \\
...
\begin{figure}[bth]
   ...
   \caption{A vertical dot plot next to a labeled box plot of the \var{price} data. The median (\$17,250), splits the data into the top 50\% and the bottom 50\%, marked in the dot plot by light-colored diamonds and dark circles, respectively.}
   \label{boxPlotLayout}
\end{figure}
...
Tables~\ref{rowPropTypeDriveTrain} and~\ref{colPropTypeDriveTrain} are useful for examining how two categorical variables are related. Mosaic plots provide a way to put these tables in a graphical form. We will only consider vehicles with front and rear wheel drive in this section to reduce complexity, as shown in Table~\ref{typeDriveTrainTableTotalsMinus4wd}.
\begin{table}[ht]
\begin{center}
...
\end{tabular}
\end{center}
\caption{A contingency table for \var{type} and \var{driveTrain} with the two vehicles with \var{driveTrain} = \resp{4WD} removed along with the row proportions.}
...
A \term{mosaic plot} provides one method to graphically display contingency table information. Here we discuss the mosaic plot representing the row proportions of \var{type} and \var{driveTrain} in the right of Table~\ref{typeDriveTrainTableTotalsMinus4wd}. The left panel of Figure~\ref{typeDriveTrainMosaicPlot} shows a mosaic plot of \var{type} where the width of the first column represents the proportion of \resp{small} cars in the data set. In the right of Table~\ref{typeDriveTrainTableTotalsMinus4wd}, the proportion of each \var{driveTrain} level was computed within \resp{small}, \resp{midsize}, and \resp{large} cars separately. Similarly, we break apart each of the columns in the left panel of Figure~\ref{typeDriveTrainMosaicPlot} into cars with \resp{front} and \resp{rear} drive trains, which results in the right panel. For instance, the top of the middle column represents \resp{midsize} cars with front wheel drive, and the lower part of the middle column represents \resp{midsize} cars with rear wheel drive. Each column is broken apart in very different places, and we can interpret this difference as the proportion of vehicles with front wheel drive differs with vehicle \var{type}. That is, \var{driveTrain} and \var{type} are associated.
\begin{figure}[hht]
   \centering
...
In a similar way, a plot comparable to the column proportions of Table~\ref{typeDriveTrainTableTotals} can be constructed, which is shown in Figure~\ref{driveTrainTypeMosaicPlot}. \\
...
\multicolumn{2}{c}{\bf small} && \multicolumn{2}{c}{\bf midsize} && {\bf large} \\ 
  \cline{1-2} \cline{4-5}  \cline{7-7}
15900 & 11600 &\hspace{3mm}& 33900 & 28000 &\hspace{3mm}& 20800 \\ 
...
\begin{caution}{association does not imply causation}{Labeling variables as \emph{explanatory} and \emph{response} does not guarantee the relationship between the two is actually causal, even if there is some apparent relationship identified between the two variables. We use these labels only to keep track of which variable we suspect affects the other.}
\end{caution}
...
\textbf{Control.} Control the value of the explanatory variable in each subject or trial, and control the effects of other potentially influential variables.

\vspace{0.3cm}
\textbf{Randomize.} Use study \term{subjects} (e.g. possums) from a random sample of the population. In an experiment, subjects must be randomly split into groups, one group for each level of the explanatory variable.
...
The study examining the effect of sulphinpyrazone was double-blinded, and the results are given in Table~\ref{sulphinpyrazoneResults}. The variables have been called \var{trmt} (treatment) and \var{outcome}. Do these results mean the drug was effective at reducing deaths? In the observed groups, a smaller proportion of individuals died in the treatment group than the control group (0.056 versus 0.081), however, it is unclear whether that difference is \emph{convincing evidence} that the drug is effective. \\
\begin{table}[ht]
\begin{center}
...
& & \multicolumn{2}{c}{\var{outcome}} \\
  ...
  \cline{2-5}
		&	\resp{drug} 	& 692    		& 41   & 733  	 \\ 
  \raisebox{1.5ex}[0pt]{\var{trmt}}		&	\resp{placebo} 	& 682    		& 60     & 742	 \\ 
  \cline{2-5}
\end{tabular}
\end{center}
...
\begin{example}{Suppose there had been only 45 deaths in the placebo group. Would this be convincing evidence that the drug was effective?} \label{45PlaceboDeaths}
...
Example~\exam{45PlaceboDeaths} is a reminder that the sample will not perfectly reflect the population. It is possible to see a small difference \emph{by chance}. Small differences in large samples can be important and meaningful but it is unclear when we should say that a difference is so large it was probably not due to chance. In Section~\ref{chanceModel}, we evaluate whether Table~\ref{sulphinpyrazoneResults} shows convincing evidence that sulphinpyrazone is effective at reducing deaths or whether the difference of 2.5\% in the death rates was due to chance. %Chapter~\ref{foundationsForInference} provides a more general and rigorous framework to form conclusions based on data.
...
\begin{example}{Suppose your professor splits the students in the front row into two groups: students on the left and students on the right. If $\bar{x}_L$ and $\bar{x}_R$ represent the mean height of students on the left and right, respectively, would you be surprised if $\bar{x}_L$ did not {exactly} equal $\bar{x}_R$?}\label{classRightLeftSideForHeight}
...
Table~\ref{sulphinpyrazoneResults} shows there were 19 fewer deaths in the treatment group than in the control group for the sulphinpyrazone study, a difference in death rates of 2.5\% $\left( \frac{60}{742} - \frac{41}{733} = 0.025 \right)$. Is it plausible that this difference is due to chance and the drug actually had no effect on the patient outcome? Or is this convincing evidence that sulphinpyrazone works? We label these two competing claims, $H_0$ and $H_A$:
\begin{itemize}
\item[$H_0$] \textbf{Independence model.} The variables \var{trmt} and \var{outcome} are independent. They have no relationship, and the difference in death rates, 2.5\%, was due to chance.
\item[$H_A$] \textbf{Alternate model.} The \var{trmt} and \var{outcome} variables are \emph{not} independent. The difference in death rates of 2.5\% was not due to chance and the treatment did reduce the death rate.
\end{itemize}
...
Consider what it would mean to the study if the independence model, which says that the variables \var{trmt} and \var{outcome} are unrelated, is true. Each person was either going to live or die, and the drug had no effect on the outcome. The researchers were just randomly splitting up these individuals into two groups, just like we split our class in half in Example~\exam{classRightLeftSideForHeight}. The researchers observed a difference of 2.5\% by chance.
...
If the data conflicts so much with $H_0$ that the independence model cannot be deemed reasonable, we will reject it in favor the alternate model, $H_A$. In other words, we hold the position that $H_0$ is true unless the evidence from the study in favor of $H_A$ is too convincing.
...
We can simulate differences due to chance using a \term{randomization technique}. If we randomly assign each person in the study into either a fake treatment or fake control group, we observe such a difference. We do this by taking 733 \resp{drugFake} and 742 \resp{placeboFake} labels\footnote{These label counts correspond to the number of \var{drug} and \var{placebo} assignments in the actual study.}, and we randomly assign them to the patients. Because these assignments are made independent of the outcomes, any difference in these fake groups is due to chance.
...
If we do this simulation many times, we get an idea for what represents a ``regular'' difference under the independence model, and we can evaluate whether the observed difference, 2.5\%, is too big to plausibly be due to chance.
\begin{center}
Independence model \\
$\downarrow$ \\
...
%\begin{tabular}{l cc}
%\hline
%Person	&	\var{trmt}	&	\var{outcome} \\
%\hline
%1		&	\resp{drug} &	\resp{lived} \\
%2		&	\resp{drug} &	\resp{lived} \\
%$\vdots$	&	$\vdots$	  &	$\vdots$ \\
%1475	&	\resp{placebo} &	\resp{lived} \\
%\hline
%\end{tabular}
%\end{center}
%\caption{The rows 1, 2, and 1475 for the data matrix representing the data in Table~\ref{sulphinpyrazoneResults}.}
%\label{sulphinpyrazoneDF}
%% trmt <- c(rep('drug', 733), rep('placebo', 742))
%% outcome <- c(rep(c('lived', 'died'), c(692, 41)), rep(c('died', 'lived'), c(60, 682)))
%\end{table}

We use a computer program to randomly assign \resp{drugFake} and \resp{placeboFake} labels to the patients. A contingency table representing results from such a randomization is shown in Table~\ref{sulphinpyrazoneRand1}. \\
\begin{table}[ht]
\begin{center}
\begin{tabular}{l l cc rr}
& & \multicolumn{2}{c}{\var{outcome}} \\
  \cline{3-4}
		&			& 	\resp{lived} 	& \resp{died} & Total & \hspace{3mm}  \\ 
  \cline{2-5}
		&	\resp{drugFake} 					& 686    		& 47    & 733 	 \\ 
  \raisebox{1.5ex}[0pt]{\var{trmtFake}}		&	\resp{placeboFake} 	& 688    		& 54 & 742    	 \\ 
  \cline{2-5}
\end{tabular}
\end{center}
\caption{Simulation results, where any difference in death rates between \resp{drugFake} and \resp{placeboFake} is purely due to chance.}
...
We computed one possible difference under the independence model in Exercise~\exer{sampleDifferenceInDrugAndPlaceboGroupSulph}. We want to repeat the simulation many more times to get a good idea of what represents a difference due to chance. Figure~\ref{sulphRandHist} shows a histogram of the differences found from 100 simulations. \\
 \begin{figure}[htbp]
    \centering
    \includegraphics[height=2in]{ch1/sulphRandHist/sulphRandHist}
    \caption{A histogram representing 100 simulations, produced under the independence model, $H_0$, where \var{trmtFake} and \var{outcome} are independent. Four of the one-hundred simulations had an a difference of at least 2.5\%.}
    \label{sulphRandHist}
 \end{figure}
 
\begin{exercise}\label{isThe2Point5DifferenceDueToChance}
Consider the observed difference in the data, 2.5\%. Does this difference appear to be just another regular observation in the histogram shown in Figure~\ref{sulphRandHist}? Possible answer in the footnote\footnote{If the independence model were true, then 2.5\% is pretty large relative to most observations. Only 4\% of the simulated differences were at least as large, suggesting an observation this large is relatively uncommon and probably not due to chance. We reject the independence model and conclude the drug actually did reduce deaths. (If you think a 4\% chance is not very unusual, your answer should differ.)}.
\end{exercise}
