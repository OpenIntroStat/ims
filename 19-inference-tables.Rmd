...
ask %>%
  ...
  pivot_wider(names_from = question_class, values_from = n) %>%
  adorn_totals(where = c("row", "col")) %>%
  kable(
    caption = "Summary of the iPod study, where a question was posed to the study participant who acted.",
    col.names = c("Response", "General", "Positive assumption", "Negative assumption", "Total")
    ) %>%
  kable_styling()
```

::: {.data data-latex=""}
These data can be found in the [openintro](http://openintrostat.github.io/openintro) package: [`ask`](http://openintrostat.github.io/openintro/reference/ask.html).
:::
...
ask_chi_sq_tabs %>%
  mutate(question_class_type = paste0(question_class, "-", type)) %>%
  select(-question_class, -type) %>%
  pivot_wider(names_from = question_class_type, values_from = n) %>%
  relocate(
    response,
    contains("General"),
    contains("Positive"),
  ...
  mutate(
    `General-expected` = ifelse(`General-expected` == "-", NA, `General-expected`),
    `Positive assumption-expected` = ifelse(`Positive assumption-expected` == "-", NA, `Positive assumption-expected`),
    `Negative assumption-expected` = ifelse(`Negative assumption-expected` == "-", NA, `Negative assumption-expected`)
  ) %>%
 kable(
  ...
  add_header_above(c(" ", "General" =2, "Positive assumption" = 2, "Negative assumption" = 2, "Total")) %>%
  kable_styling()
```
...
```{r include=FALSE}
terms_chp_19 <- c(terms_chp_19, "expected counts")
```
...
set.seed(4747)
ask %>%
  mutate(question_class = sample(question_class)) %>% # randomize
  count(question_class, response) %>%
  pivot_wider(names_from = question_class, values_from = n) %>%
  adorn_totals(where = c("row", "col")) %>%
  kable(
    caption = "Randomized allocation of the data to the question posed in the iPod study.",
    col.names = c("Response", "General", "Positive assumption", "Negative assumption", "Total")
    ) %>%
  kable_styling()
```
...
```{r include=FALSE}
terms_chp_19 <- c(terms_chp_19, "chi-squared statistic")
```
...
```{r chisqDistDF, fig.cap = "The chi-squared distribution for differing degrees of freedom. The larger the degrees of freedom, the longer the right tail extends."}
...
```{r include=FALSE}
terms_chp_19 <- c(terms_chp_19, "chi-squared distribution")
```
...
```{r iPodChiSqTail, fig.cap = "Visualization of the p-value for $X^2 = 40.13$ when $df = 2.$"}
x <- 40.13
ChiSquareTail(
  x, 2,
  c(0, 50),
...
::: {.workedexample data-latex=""}} Find the p-value and draw a conclusion about whether the question affects the sellers likelihood of reporting the freezing problem.
...
::: {.workedexample data-latex=""}} Table \@ref(tab:diabetes2ExpMetRosiLifestyleSummary) summarizes the results of an experiment evaluating three treatments for Type 2 Diabetes in patients aged 10-17 who were being treated with metformin.
...
::: {.data data-latex=""}
These data can be found in the [openintro](http://openintrostat.github.io/openintro) package: [`diabetes2`](http://openintrostat.github.io/openintro/reference/diabetes2.html).
:::
