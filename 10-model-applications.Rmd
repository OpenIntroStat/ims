...
Take a walk around your neighborhood and you will probably see a few houses for sale.
If you find a house for sale, you can probably go online and look up its price.
You will quickly note that the prices seem a bit arbitrary -- the homeowners get to decide what the amount they want to list their house for, and many criteria factor into this decision, e.g., what do comparable houses ("comps" in real estate speak) sell for, how quickly they need to sell the house, etc.

In this case study we will formalize the process of figuring out how much to list a house for by using data on current home sales In November of 2020, information on `r nrow(duke_forest)` houses in the Duke Forest neighborhood of Durham, NC were scraped from [Zillow](https://www.zillow.com).
...
The residuals from the linear model can be to assess whether a linear model is appropriate.
...
You consider taking a variable out, but you are not sure which one to remove.
...
However, note that `area` is still in the model, and it is quite likely that the area of the home and the number of bedrooms are highly associated.
Therefore, the model already has information on "how much space is available in the house" with the inclusion of `area` alone.
...
[^model-applications-7]: A residual of \$`r format(new_house_resid, big.mark = ",")` is reasonably big.
    ...
    After we learn that the house is in California, we realize that the model should not be applied to the new home at all!
