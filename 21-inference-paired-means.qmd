...
#| fig-alt: |
#|   Line plot connecting the tread for the 4th car in the data set. The first
...
#|   original groups."
#| fig-alt: |
#|   Line plot connecting the tread for the 5th car in the data set. The first
#|   plot is the original data and the second plot is the permuted data where
...
#|   data set. The first plot is the original data and the second plot is
...
A portion of the data set from these courses is shown in @tbl-textbooksDF, where prices are in US dollars.
...
#| tbl-cap: Four cases from the `ucla_textbooks_f18` data set.
ucla_textbooks_f18 |>
...
That is, the original data set would include the list of 68 price differences, and each resample will also include 68 price differences (some repeated through the bootstrap resampling process).
