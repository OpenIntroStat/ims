...
\index{point estimate!single mean} However, we will soon learn that a new distribution, called the $t$-distribution, tends to be more useful when working with the sample mean.
We will first learn about this new distribution, then we will use it to construct confidence intervals and conduct hypothesis tests for the mean.
...
As part of your planning, you would like to know for how much an average car from Awesome Autos sells.
In order to go through the example more clearly, let us say that you are only able to randomly sample five cars from Awesome Auto.
...
```{r 5cars, fig.cap = "A sample of five cars from Awesome Auto.", out.width = "75%", fig.alt = "Photographs of 5 different automobiles.  The cars are different color and different makes and models.  On top of the image of each car is its price; the five prices range from 9600 dollars to 27000 dollars."}
...
To repeat, the idea is that we want to know how datasets differ from one another, but we are not ever going to take more than one sample of observations.
...
```{r bootsamps1mean, fig.cap = "To estimate the natural variability in the sample mean, different bootstrap samples are taken from the original sample.  Notice that each bootstrap resample is different from each other as well as from the original sample", fig.alt = "The sample is shown being taken from the large unknown population.  The bootstrap resamples, however, are taken directly from the original sample (sampling with replacement) as if the resamples had been taken from an infinitely large proxy population.  Three bootstrap resamples of 5 cars each are shown, each resample is slightly different due to the process of resampling with replacement."}
...
The bootstrap SE is a good approximation to the SE of means as if we had taken repeated samples from the original population (which we agreed is not something we would do because of wasted resources).
...
Although we will not spend a lot of energy on this concept, you may be wondering some of the differences between a standard error and a standard deviation.
...
In this first course in statistics, you are not expected to develop perfect judgment on the normality condition.
...
[^inference-one-mean-4]: More nuanced guidelines would consider further relaxing the *particularly extreme outlier* check when the sample size is very large.
    However, we will leave further discussion here to a future course.
...
Let us next check the normality condition for each using the rule of thumb.
...
It is often helpful to also visualize the data using a box plot to assess skewness and existence of outliers.
...
In practice, it is typical to also do a mental check to evaluate whether we have reason to believe the underlying population would have moderate skew (if $n < 30)$ or have particularly extreme outliers $(n \geq 30)$ beyond what we observe in the data.
...
We will employ a similar strategy for computing the standard error of $\bar{x},$ using the sample standard deviation $s$ in place of $\sigma:$
...
We will find it useful to use a new distribution for inference calculations called the $t$-distribution.
...
In practice, it is common to use statistical software, such as R, Python, or SAS for these analyses.
...
Do not forget that with the $t$-distribution, the degrees of freedom must always be specified!
...
Let us get our first taste of applying the $t$-distribution in the context of an example about the mercury content of dolphin muscle.
...
```{r rissosDolphin, fig.cap = "A Risso's dolphin. Photo by Mike Baird, www.bairdphotos.com", out.width="75%", fig.alt = "Photograph of a Risso's dolphin."}
...
Now that we have used the $t$-distribution for making a confidence interval for a mean, let us speed on through to hypothesis tests for the mean.
...
If you are not sure what some of these terms mean, we recommend you go back in the text and review their definitions.
