...
A sampling distribution describes the possible values of a **statistic** (note that here the sample proportion, $\hat{p}$ is described, but other statistics like the sample mean, $\bar{x}$ will be described later) as they change from study to study.\
A sampling distribution is contrasted with a data distribution which shows the variability of the *observed* data values.

A data distribution can be visualized from the observations themselves.
However, because a sampling distribution describes many statistics computed from many studies, it cannot be visualized directly from a single dataset.
Instead, a sampling distribution uses either computational or mathematical structures to describe the variability expected in repeated studies.
:::
