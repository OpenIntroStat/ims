...
gender_discrimination_sim_dist <- gender_discrimination %>%
  specify(response = decision, explanatory = gender, success = "promoted") %>%
  hypothesize(null = "independence") %>%
  generate(reps = 10000, type = "permute") %>%
...
```{r FourCaseStudies, fig.cap = "The null distribution for each of the four case studies presented previously. Note that the center of each distribution is given by the value of the parameter set in the null hypothesis.", fig.asp = 0.5}
...
ggplot(opportunity_cost_sim_dist, aes(x = stat)) +
  ...
  annotate("text", x = 0, y = 300, label = "Opportunity cost", size = 8)

# gender discrimination --------------------------------------------------------

ggplot(gender_discrimination_sim_dist, aes(x = stat)) +
  geom_histogram(binwidth = 0.04, fill = IMSCOL["green", "full"]) +
  scale_x_continuous(breaks = seq(-0.4, 0.4, 0.1), labels = label_number(accuracy = 0.1)) +
  theme(
    axis.text.y = element_blank(),
    panel.grid = element_blank()
  ) +
  ...
  annotate("text", x = 0, y = 500, label = "Gender discrimination", size = 8)
...
ggplot(medical_consultant_sim_dist, aes(x = stat)) +
  ...
  annotate("rect", xmin = 0.025, xmax = 0.175, ymin = 200, ymax = 600, fill = "white", alpha = 0.5) +
  annotate("text", x = 0.1, y = 400, label = "Medical consultant", size = 8)

# tappers and listeners --------------------------------------------------------

ggplot(tappers_listeners_sim_dist, aes(x = stat)) +
  geom_histogram(binwidth = 0.016, fill = IMSCOL["yellow", "full"]) +
  scale_x_continuous(breaks = seq(0.35, 0.70, 0.05), labels = label_number(accuracy = 0.01)) +
  theme(
    axis.text.y = element_blank(),
    panel.grid = element_blank()
  ) +
  labs(x = expression(H[0]: ~ p == 0.5), y = NULL) +
  annotate("rect", xmin = 0.3, xmax = 0.8, ymin = 150, ymax = 450, fill = "white", alpha = 0.5) +
  annotate("text", x = 0.5, y = 300, label = "Tappers + listeners", size = 8)
```
...
```{r simpleNormal, fig.cap = "A normal curve.", fig.asp = 0.5}
normTail(axes = FALSE)
```
...
ucla_textbooks_f18_diff_mean<- ucla_textbooks_f18 %>%
  summarise(diff_mean = round(mean(diff), 2)) %>%
  pull(diff_mean)
```

::: {.workedexample data-latex=""}
Book prices were collected for `r nrow(ucla_textbooks_f18)` courses at UCLA in Fall 2018 Data were collected from both the UCLA Bookstore and Amazon.
The differences in these prices are shown in Figure \@ref(fig:diffInTextbookPricesF18-CLTsection).
The mean difference in the price of the books was \$`r ucla_textbooks_f18_diff_mean`, and we might wonder, does this provide strong evidence that the prices differ between the two book sellers?

------------------------------------------------------------------------

Here again we can apply the normal distribution, this time in the context of numerical data.
We'll explore this example and construct such a hypothesis test in Chapter \@ref(inference-paired-means).
:::

```{r diffInTextbookPricesF18-CLTsection, fig.cap = "Histogram of the difference in price for each book sampled. These data are strongly skewed.", fig.width = 10, message = FALSE}
ggplot(ucla_textbooks_f18, aes(x = diff)) +
  geom_histogram(binwidth = 10) +
  labs(
    x = "UCLA price - Amazon price (USD)",
    y = "Count"
  )
```

::: {.data data-latex=""}
The data from this study can be found in the [**openintro**](http://openintrostat.github.io/openintro) R package: [`ucla_textbooks_f18`](http://openintrostat.github.io/openintro/reference/ucla_textbooks_f18.html).
:::
...
```{r elmhurstScatterWLSROnly-CLTsection, fig.cap = "Gift aid and family income for a random sample of 50 freshman students from Elmhurst College, shown with a regression line."}
...
Who performed better?[^foundations-mathematical-4]
:::

[^foundations-mathematical-4]: We use the standard deviation as a guide.
...
```{r satActNormals, fig.cap = "Nel's and Sian's scores shown with the distributions of SAT and ACT scores.", fig.asp = 0.5}
sat_mean <- 1500
sat_sd   <- 300
act_mean <- 21
act_sd   <- 5
...
ggplot(tests %>% filter(test == "SAT"), aes(x = score)) +
...
ggplot(tests %>% filter(test == "ACT"), aes(x = score)) +
...
::: {.guidedpractice data-latex=""}
Use Sian's ACT score, 24, along with the ACT mean and standard deviation to compute their Z score.[^foundations-mathematical-5]
:::

[^foundations-mathematical-5]: $Z_{Sian} = \frac{x_{Sian} - \mu_{ACT}}{\sigma_{ACT}} = \frac{24 - 21}{5} = 0.6$
...
Compute the Z scores for possums with head lengths of 95.4 mm and 85.8 mm.[^foundations-mathematical-6]
:::

[^foundations-mathematical-6]: For $x_1=95.4$ mm: $Z_1 = \frac{x_1 - \mu}{\sigma} = \frac{95.4 - 92.6}{3.6} = 0.78.$ For $x_2=85.8$ mm: $Z_2 = \frac{85.8 - 92.6}{3.6} = -1.89.$
...
::: {.guidedpractice data-latex=""}
Which of the two brushtail possum observations in the previous guided practice is more *unusual*?[^foundations-mathematical-7]
:::

[^foundations-mathematical-7]: Because the *absolute value* of Z score for the second observation is larger than that of the first, the second observation has a more unusual head length.
...
```{r satBelow1800, fig.cap = "The normal model for SAT scores, shading the area of those individuals who scored below Nel.", fig.asp = 0.5}
...
::: {.guidedpractice data-latex=""}
Determine the proportion of SAT test takers who scored better than Nel on the SAT.[^foundations-mathematical-8]
:::

[^foundations-mathematical-8]: If 84% had lower scores than Nel, the number of people who had better scores must be 16%.
...
```{r satAbove1630, fig.asp = 0.5}
normTail(m = 1500, s = 300, U = 1630, col = IMSCOL["blue", "full"])
```

```{r subtractingArea, fig.asp = 0.3}
...
Draw the normal curve representing this exercise, shading the lower region instead of the upper one.[^foundations-mathematical-9]
:::

[^foundations-mathematical-9]: We found the probability to be 0.6664.
...
(b) What percent of SAT takers did better than Stuart?[^foundations-mathematical-10]
:::

[^foundations-mathematical-10]: Numerical answers: (a) 0.9772.
    (b) 0.0228.

Based on a sample of 100 men,[^foundations-mathematical-11] the heights of male adults between the ages 20 and 62 in the US is nearly normal with mean 70.0'' and standard deviation 3.3''.

[^foundations-mathematical-11]: This sample was taken from the USDA Food Commodity Intake Database.
...
text(67, 0.03, "40%\n(0.40)", cex = 1.5, col = IMSCOL["black", "full"])
```
...
```{r height82Perc, fig.asp = 0.5}
normTail(70, 3.3, L = qnorm(0.82, 70, 3.3), col = IMSCOL["blue", "full"])
text(70, 0.04, "82%\n(0.82)", cex = 1.5, col = IMSCOL["black", "full"])
text(74.3, 0.02, "18%\n(0.18)", cex = 1.25, col = IMSCOL["black", "full"])
```
...
(b) What is the $97.5^{th}$ percentile of the male heights? As always with normal probability problems, first draw a picture.[^foundations-mathematical-12]
:::

[^foundations-mathematical-12]: Remember: draw a picture first, then find the Z score.
...
(b) What is the probability that a male adult is shorter than 5'9'' (69 inches)?[^foundations-mathematical-13]
:::

[^foundations-mathematical-13]: Numerical answers: (a) 0.1131.
    (b) 0.3821.
...
::: {.guidedpractice data-latex=""}
What percent of SAT takers get between 1500 and 2000?[^foundations-mathematical-14]
:::

[^foundations-mathematical-14]: This is an abbreviated solution.
...
::: {.guidedpractice data-latex=""}
What percent of adult males are between 5'5'' and 5'7''?[^foundations-mathematical-15]
:::

[^foundations-mathematical-15]: 5'5'' is 65 inches.
    5'7'' is 67 inches.
...
Similarly there should be an area of about 0.95 between $Z=-2$ and $Z=2.$[^foundations-mathematical-16]
:::

[^foundations-mathematical-16]: First draw the pictures.
...
What percent score between 1500 and 2100 ?[^foundations-mathematical-17]
:::

[^foundations-mathematical-17]: 900 and 2100 represent two standard deviations above and below the mean, which means about 95% of test takers will score between 900 and 2100.
...
```{r include = FALSE}
terms_chp_14 <- c(terms_chp_14, "margin of error")
```
...
```{r OpportunityCostDiffs-w-normal, fig.cap = "Null distribution of differences with an overlaid normal curve for the opportunity cost study. 10,000 simulations were run for this figure.", out.width = "100%", fig.width = 8}
ggplot(opportunity_cost_sim_dist, aes(x = stat)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.013) +
  ...
  labs(
    x = expression('When H'[0]*' is true, simulated difference in proportions of students who do not buy the video'),
...
```{r OpportunityCostDiffs_normal_only, fig.asp = 0.5}
...
```{r MedConsNullSim-normal-only, fig.asp = 0.5}
...
::: {.guidedpractice data-latex=""}
Compute the area between -1.96 and 1.96 for a normal distribution with mean 0 and standard deviation 1.[^foundations-mathematical-18]
:::

[^foundations-mathematical-18]: We will leave it to you to draw a picture.
...
The point estimate from this study can reasonably be modeled with a normal distribution, and a proper standard error for this point estimate is $SE = 0.078.$ Construct a 95% confidence interval.[^foundations-mathematical-19]
...
[^foundations-mathematical-19]: We've used $SE = 0.078$ from the last section.
...
stent30 %>%
  ...
  kbl(booktabs = TRUE, caption = "Descriptive statistics for 30-day results for the stent study.",
...
```{r 95PercentConfidenceInterval, fig.cap = "Twenty-five samples of size $n=300$ were collected from a population with $p = 0.30.$ For each sample, a confidence interval was created to try to capture the true proportion $p.$ However, 1 of these 25 intervals did not capture $p = 0.30.$"}
set.seed(4747)
num.sim <- 25
n <- 300
p <- 0.3
...
::: {.guidedpractice data-latex=""}
In Figure \@ref(fig:95PercentConfidenceInterval), one interval does not contain the true proportion, $p = 0.3.$ Does this imply that there was a problem with the datasets that were selected?[^foundations-mathematical-20]
:::

[^foundations-mathematical-20]: No.
...
inference_method_summary_table %>%
  ...
  kbl(booktabs = TRUE, caption = "Summary of mathematical models as an inferential statistical method.", 
