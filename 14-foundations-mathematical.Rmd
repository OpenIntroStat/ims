...
```{r diffInTextbookPricesF18-CLTsection, fig.cap = "Histogram of the difference in price for each book sampled. These data are strongly skewed."}
...
In Chapter \@ref(inf-model-slr) we will focus on whether that trend is meaningfully different from what would have happened by chance alone if there was no relationship present.
...
Based on a sample of 100 men,[^foundations-mathematical-11] the heights of adults who identify as male, between the ages 20 and 62 in the US is nearly normal with mean 70.0'' and standard deviation 3.3''.
...
This confidence interval can also be used in a way analogous to a hypothesis test: since the interval does not contain 0 (is completely above 0), it means the data provide convincing evidence that the stent used in the study *increases* the risk of stroke within 30 days.
:::
...
::: {.guidedpractice data-latex=""}
In Figure \@ref(fig:95PercentConfidenceInterval), one interval does not contain the true proportion, $p = 0.3.$ Does this imply that there was a problem with the datasets that were selected?[^foundations-mathematical-19]
:::

[^foundations-mathematical-19]: No.
