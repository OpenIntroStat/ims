...
```{r satActNormals, fig.cap = "Ann's and Tom's scores shown with the distributions of SAT and ACT scores.", fig.asp = 0.5}
sat_mean <- 1500
sat_sd   <- 300
act_mean <- 21
act_sd   <- 5
...
ggplot(tests %>% filter(test == "SAT"), aes(x = score)) +
  ...
  annotate("text", x = 1800, y = 0.0011, label = "Ann", color = IMSCOL["blue", "full"]) +
...
ggplot(tests %>% filter(test == "ACT"), aes(x = score)) +
  ...
  annotate("text", x = 24, y = 0.08, label = "Tom", color = IMSCOL["green", "full"]) +
...
```{r satBelow1800, fig.cap = "The normal model for SAT scores, shading the area of those individuals who scored below Ann.", fig.asp = 0.5}
...
text(62, 0.07, "Mike", cex = 1.5)

normTail(70, 3.3, L = 76, col = IMSCOL["blue", "full"])
text(62, 0.07, "Jim", cex = 1.5)
```
:::
