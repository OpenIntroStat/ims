...
#|   analysis of a binary response variable.  $p$ is the population parameter for one sample, $\widehat{p}$ is the sample proportion for one sample, $p_0$ is the value of the parameter which is specified in the null hypothesis, $p_1$ is the population parameter in group 1, $p_2$ is the population parameter for group 2, $\widehat{p}_1$ is the sample proportion for group 1, $\widehat{p}_2$ is the sample proportion for group 2, and $\widehat{p}_{pool}$ is the pooled estimate described in @sec-test-prop-pool.
...
#|   two independent samples analysis of a numeric response variable.
...
In the top image in @fig-blue-triangle-shapes the circled item is the only triangle, while in the bottom image the circled item is one of two triangles.
...
One of the displays from this study is shown in @fig-redundant-adjectives-blue-triangle.
...
Table @tbl-redundant-data shows the top six rows of the data.
...
@fig-compare-lang-viz) shows the null distributions for each of these hypothesis tests.
