...
```{r CPR_study_right_tail, fig.cap="Null distribution of the point estimate for the difference in proportions. The shaded right tail shows observations that are at least as large as the observed difference, 0.13.", warning=FALSE, fig.width=10}
...
```{r CPR_study_p_value, fig.cap="Null distribution of the point estimate for the difference in proportions. All values that are at least as extreme as +0.13 but in either direction away from 0 are shaded.", warning=FALSE, fig.width=10}
...
```{r diffInTextbookPricesS10_CLTsection, fig.cap="Histogram of the difference in price for each book sampled. These data are strongly skewed.", warning=FALSE, fig.width=10}
...
```{r elmhurstScatterWLSROnly_CLTsection, fig.cap="Gift aid and family income for a random sample of 50 first-year students from Elmhurst College, shown with a regression line.", warning=FALSE, fig.width=10}
...
```{r 6895997, fig.cap="Probabilities for falling within 1, 2, and 3 standard deviations of the mean in a normal distribution.", warning=FALSE, fig.width=10}
...
```{r nbaNormal, fig.cap="Histogram and normal probability plot for the NBA heights from the 2008-9 season.", warning=FALSE, fig.width=10}
...
```{block2, type = "todo", echo=TRUE}
Four normal probability plots for Guided Practice ref{normalQuantileExercise}.
```
