...
```{r boot1, fig.caption = "first figure with the ? pop, then sample, then estimate of the pop.  below i said 3 white (success) and 4 red (failure).  easy to change the text below.", warning = FALSE fig.width = 10}
...
```{r boot2, fig.caption = "next fig, has the bootstrap samples", warning = FALSE fig.width = 10}
...
The distribution of $\hat{p}_{bs}$ for the example scenario is shown in Figure \@ref(fig:boot3), and the bootstrap distribution for the medical consulatant data is shown in Figure \@ref(fig:MedConsBSSim).

```{r boot3, fig.caption = "WITH ADDED HISTOGRAM... boot samples, arrow, histogram of all of them", warning = FALSE fig.width = 10}
...
```{r boot4, fig.caption = "sampling with replacement figure", warning = FALSE fig.width = 10}
...
Figure \@ref(fig:boot5) visualizes one simulation for the medical consulatant.
Out of the simulated cases, there were 5 with a complication and 57 without a complication: $\hat{p}_{bs} = 5/62 = 0.081$.

```{r boot5, fig.caption = "how impossible would it be to create one simulation with 62 marbles? 3 red, 59 white?", warning = FALSE fig.width = 10}
hist(rnorm(10))
```
...
```{r bootboth, fig.caption = "(not sure if this should include a histogram or not) the infinite pop compared to with replacement", warning = FALSE, fig.width=10}
