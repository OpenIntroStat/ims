...
For example, in Chapter \@ref(inference-foundations) the standard deviation is used in calculations that help us understand how much a sample mean varies from one sample to the next.

### Box plots, quartiles, and the median

A **box plot** summarizes a data set using five statistics while also plotting unusual observations.
Figure \@ref(fig:loan_int_rate_box_plot_layout) provides a vertical dot plot alongside a box plot of the `interest_rate` variable from the `loan50` data set.
