...
## Sex discrimination case study {#caseStudyGenderDiscrimination}

We consider a study investigating sex discrimination in the 1970s, which is set in the context of personnel decisions within a bank.[^foundations-randomization-2]
The research question we hope to answer is, "Are individuals who identify as female discriminated against in promotion decisions made by their managers who identify as male?"
...
::: {.data data-latex=""}
The data from this study can be found in the [**openintro**](http://openintrostat.github.io/openintro) R package: [`gender_discrimination`](http://openintrostat.github.io/openintro/reference/gender_discrimination.html).
:::

### Observed data

The participants in this study were 48 bank supervisors who identified as male, attending a management institute at the University of North Carolina in 1972.[^foundations-randomization-3]
...
Using the results of the study summarized in Table \@ref(tab:gender-discrimination-obs), we would like to evaluate if individuals who identify as female are unfairly discriminated against in promotion decisions.
...
```{r gender-rand-obs, out.width = "40%", fig.cap = "The sex descriminiation study can be thought of as 48 red and white cards."}
...
We can actually perform this **randomization**, which simulates what would have happened if the bankers' decisions had been independent of `sex` but we had distributed the file sexes differently.[^foundations-randomization-5]

[^foundations-randomization-5]: The test procedure we employ in this section is sometimes referred to as a **permutation test**. The difference between the two is how the explanatory variable was assigned. Permutation tests are used for observational studies, where the explanatory variable was not randomly assigned.\index{permutation test}.
...
Figure \@ref(fig:gender-rand-shuffle-1) highlights both the shuffle and the reallocation to the sham sex groups.

```{r gender-rand-shuffle-1, out.width = "80%", fig.cap = "The sex descriminiation data is shuffled and reallocated to new groups of male and female files."}
...
[^foundations-randomization-6]: $18/24 - 17/24=0.042$ or about 4.2% in favor of the male personnel.
...
```{r gender-rand-shuffle-1-sort, out.width = "100%", fig.cap = "We summarize the randomized data to produce one estimate of the difference in proportions given no gender discrimination."}
...
```{r gender-rand-dot-plot, fig.cap="(ref:gender-rand-dot-plot-cap)", out.width = "100%"}
set.seed(35)
gender_discrimination %>%
...
It appears that a difference of at least `r perc_diff` under the null hypothesis would only happen about 2% of the time according to Figure \@ref(fig:gender-rand-dot-plot).
...
We will define a **success**\index{success} in this study as a student who chooses not to buy the video.[^foundations-randomization-8]
...
[^foundations-randomization-8]: Success is often defined in a study as the outcome of interest, and a "success" may or may not actually be a positive outcome.
...
Because this is an experiment, we'll use the same procedure we applied in Section \@ref(caseStudyGenderDiscrimination): randomization.
...
In Section \@ref(caseStudyGenderDiscrimination) we encountered a study from the 1970's that explored whether there was strong evidence that female candidates were less likely to be promoted than male candidates.
...
::: {.workedexample data-latex=""}
In the opportunity cost study in Section \@ref(caseStudyOpportunityCost), we analyzed an experiment where study participants had a 20% drop in the chance of continuing with a video purchase if they were reminded that the money, if not spent on the video, could be used for other purchases in the future.
