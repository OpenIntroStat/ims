...
ggplot(sandwich2, aes(x = ad, y = rev)) +
  geom_point(size = 3, fill = COL["green", "full"], color = "#FFFFFF", shape = 22) +
  geom_smooth(method = "lm", se = FALSE, fullrange = TRUE, color = COL["green", "full"]) +
  scale_x_continuous(labels = label_dollar(suffix = "K")) +
  scale_y_continuous(labels = label_dollar(suffix = "K")) +
  labs(
    x = "Amount spent on advertising",
...
ggplot(sandwich3, aes(x = ad, y = rev)) +
  geom_point(size = 3, fill = COL["gray", "full"], color = "#FFFFFF", shape = 23) +
  geom_smooth(method = "lm", se = FALSE, fullrange = TRUE, color = COL["gray", "full"]) +
  scale_x_continuous(labels = label_dollar(suffix = "K")) +
  scale_y_continuous(labels = label_dollar(suffix = "K")) +
  labs(
    x = "Amount spent on advertising",
...
ggplot() +
  geom_smooth(
    ...
    method = "lm", se = FALSE, fullrange = TRUE, 
    color = COL["gray", "f2"], size = 0.5
  ) +
  geom_smooth(
    data = sandwich, aes(x = ad, y = rev), method = "lm", se = FALSE,
    fullrange = TRUE, color = COL["red", "full"]
  ) +
  scale_x_continuous(labels = label_dollar(suffix = "K")) +
  scale_y_continuous(labels = label_dollar(suffix = "K")) +
  labs(
...
ggplot(crops_USA_long) +
  ...
  facet_wrap(~crop) +
  ylim(c(1.4, 3.6)) +
  scale_color_gradient(low = COL["blue", "f3"], high = COL["blue", "full"])
```
...
ggplot(data = perm_slope, aes(x = stat)) +
  geom_histogram() +
  geom_vline(xintercept = obs_slope, color = COL["red", "full"])
```
...
ggplot(crops_USA) +
  ...
  geom_point(x = 8.38, y = 2.89, color = COL["green", "full"], pch = 1, size = 8) +
  geom_point(x = 4.15, y = 1.7, color = COL["red", "full"], pch = 1, size = 8) +
...
histPlot(crops_many_lm_BS,
  breaks = breaks, axes = FALSE, col = rgb(1, 1, 1),
  xlab = "", ylab = ""
)
...
$$
\text{% change in House seats for President's party} = -7.36 - 0.89 \times \text{unemployment rate}
$$
...
midterms_house %>%
  ...
  ggplot(aes(x = unemp, y = house_change, 
             color = party, shape = party, fill = party)) +
  geom_point(size = 3, color = "#FFFFFF", alpha = 0.7) +
