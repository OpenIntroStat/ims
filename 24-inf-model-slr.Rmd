...
The concept of the sampling variability is something you have seen before, but in this lesson, you will focus on the variability of the line often measured through the variability of a single statistic: **the slope of the line**.
...
Because interest is now in creating a CI, there is no null hypothesis, so there will not be any reason to permute either of the variables.
...
In this section, we will provide details for when the mathematical model is appropriate and a discussion of technical conditions needed for the randomization and bootstrapping procedures.
.
...
-   **Nearly normal residuals.** Generally, the residuals must be nearly normal.
    When this condition is found to be unreasonable, it is usually because of outliers or concerns about influential points, which we will talk about more in Section \@ref(outliers-in-regression).
