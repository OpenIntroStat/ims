...
```{r include=FALSE}
terms_chp_22 <- c(terms_chp_22, "bootstrap CI paired difference")
```
...
A portion of the data set from these courses is shown in Figure \@ref(tab:textbooksDF), where prices are in US dollars.[^inference-paired-means-1]

[^inference-paired-means-1]: The data used in this section can be found in the **openintro** R package: [`ucla_textbooks_f18`](http://openintrostat.github.io/openintro/reference/ucla_textbooks_f18.html).
...
```{r pairboot, fig.cap = "Bootstrap distribution for the average difference in new book price at the UCLA bookstore versus Amazon. 99% confidence intervals are superimposed using blue (bootstrap percentile interval) and red (bootstrap SE interval) lines."}
...
## Mathematical model for $\mu_d$ {#mathparied}
...
::: {.workedexample data-latex=""}} Set up a hypothesis test to determine whether, on average, there is a difference between Amazon's price for a book and the UCLA bookstore's price.
...
```{r include=FALSE}
terms_chp_22 <- c(terms_chp_22, "T score paired difference")
```

::: {.guidedpractice data-latex=""}
Complete the hypothesis test started in the previous Example.[^inference-paired-means-2]
:::

[^inference-paired-means-2]: To compute the test compute the standard error associated with $\bar{x}_{diff}$ using the standard deviation of the differences $(s_{diff} = 13.42)$ and the number of differences $(n_{diff} = 68):$
...
```{r textbooksF18HTTails, fig.cap = "Distribution of $\\bar{x}_{diff}$ under the null hypothesis of no difference.  The observed average difference of 2.98 is marked with the shaded areas more extreme than the observed difference given as the p-value."}
...
::: {.guidedpractice data-latex=""}
Create a 95% confidence interval for the average price difference between books at the UCLA bookstore and books on Amazon.[^inference-paired-means-3]
:::

[^inference-paired-means-3]: Conditions have already verified and the standard error computed in a previous Example.\
...
Should UCLA students always buy their books on Amazon?[^inference-paired-means-4]
:::

[^inference-paired-means-4]: The average price difference is only mildly useful for this question.
...
```{r include=FALSE}
terms_chp_22 <- c(terms_chp_22, "paired difference t-test", "paired difference CI")
```
...
::: {.exercises}
```{r exercises-22, child = "exercises/22-ex-inference-paired-means.Rmd"}
```
:::
