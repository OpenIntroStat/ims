...
origbox <- ggplot(tires, aes(x = brand, y = tread, 
...
```{r tiredataPermSort, fig.cap = "Boxplots of the tire tread data (in cm) and (left) the brand of tire from which the original measurements came, (center) shuffled brand assignment, (right) sorted by randomized brand.", out.width = "100%"}
...
origbox + shuffbrand + shuffbrandfull
```
...
```{r tiredatarand1, fig.cap = "One randomization where the brand is randomly swapped (or not) across the two tread wear measurements (in cm) from the same car."}
set.seed(47)
tires1 <- tires %>%
  group_by(car) %>%
  mutate(random_brand = sample(brand))
...
```{r pairRandomiz, fig.cap = "Histogram of 1000 differences with tire brand randomly assigned across the two observations (in cm) per pair."}
...
ggplot(tires_shuffled, aes(x = stat)) +
  ...
  labs(
    x = "Difference in tire wear, across randomly assigned groups",
    title = "Difference in wear of tire tread (in inches) after 1,000 miles,\nbrand randomly assigned",
    y = NULL
  ) +
...
boot_diff %>%
  ...
  scale_color_manual(
    ...
    labels = c("Percentile", "SE", "Observed statistic"),
    guide = "legend"
  ) +
  labs(
...
```{r include=FALSE}
terms_chp_22 <- c(terms_chp_22, "paired difference t-test")
```

```{r include=FALSE}
terms_chp_22 <- c(terms_chp_22, "paired difference CI")
```
