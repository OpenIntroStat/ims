...
The dataset has `r nrow(redundant)` rows, which amounts to
...
In this experiment, `r phat_eng_4`% of subjects who responded in English used a redundant adjective (i.e. said "the blue triangle") when presented with an image with 4 items in it.
Using this subset of the data, we can construct a confidence interval for the proportion of English speakers who use redundant color adjectives when describing items in an image that is not very dense.
