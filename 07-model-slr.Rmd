...
When considering linear regression, it is helpful to think deeply about the line fitting process.
...
Instead, it is more common for data to appear as a *cloud of points*, such as those examples shown in Figure \@ref(fig:imperfLinearModel).
...
It is harder to tell if age changes the relationship between total length and head length for these possums.
...
In Chapter \@ref(model-mlr), we will learn about how we can include more than one predictor in our model.
...
We have seen plots with strong linear relationships and others with very weak linear relationships.
...
[^model-slr-4]: We will leave it to you to draw the lines.
...
One important aspect of the correlation is that it is *unitless*.
...
For now, we will not worry about the `price_paid` variable.
...
While manual calculations are not commonly used by practicing statisticians and data scientists, it is useful to work through the first time you are learning about the least squares line and modeling in general.
...
We will consider Ebay auctions for a video game, *Mario Kart* for the Nintendo Wii, where both the total price of the auction and the condition of the game were recorded.
...
We will elaborate further on modeling categorical predictors in Chapter \@ref(model-mlr), where we examine the influence of many predictor variables simultaneously using multiple regression.
...
-   B: There is one outlier on the right, though it is quite close to the least squares line, which suggests it was not very influential.
...
In Plot E, the bulk of the data show no clear trend, but if we fit a line to these data, we impose a trend where there is not really one.
