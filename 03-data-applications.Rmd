...
Stop for a second and think about how many passwords you have used so far today.
You have probably used one to unlock your phone, one to check email, and probably at least one to log on to a social media account.
Made a debit purchase?
You have probably entered a password there too.

If you are reading this book, and particularly if you are reading it online, chances are you have had to create a password once or twice in your life.
And if you are diligent about your safety and privacy, you have probably chosen passwords that would be hard for others to guess, or *crack*.

In this case study we introduce a dataset on passwords.
The goal of the case study is to walk you through what a data scientist does when they first get a hold of a dataset as well as to provide some "foreshadowing" of concepts and techniques we will introduce in the next few chapters on exploratory data analysis.
...
At this stage it is also useful to think about how these data were collected, as that will inform the scope of any inference you can make based on your analysis of the data.
...
Once you have identified the rows and columns, it is useful to review the data dictionary to learn about what each column in the dataset represents.
...
The strength variable is trickier to classify -- we can think of it as discrete numerical or as an ordinal variable as it takes on numerical values, however it is used to categorize the passwords on an ordinal scale.
...
Sometimes the data dictionary (presented in Table \@ref(tab:passwords-var-def) is not sufficient, and we need to go back to the data source and try to understand the data better before we can proceed with the analysis meaningfully.
:::

Next, let us try to get to know each variable a little bit better.
...
Even though passwords that take a large number of years to crack can seem like good options (see Table \@ref(tab:passwords-long-crack) for a list of them), now that you have seen them here (and the fact that they are in a dataset of 500 most common passwords), you should not use them as secure passwords!
...
The stronger the password, the larger text it is represented with on the plot.
...
And if you already do, it is time to go change it!
...
In the next few chapters, we will introduce exploratory data analysis and you will learn more about the various types of data visualizations and summary statistics you can make to get to know your data better.
...
It is okay if your answer is "I am not sure", we simply want to get your exploratory juices flowing to prime you for what is to come!
...
Navigate the concepts you have learned in this chapter in R using the following self-paced tutorials.
...
Further apply the concepts you have learned in this part in R with computational labs that walk you through a data analysis case study.
