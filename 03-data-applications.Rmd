...
passwords %>%
  slice_head(n = 10) %>%
  kable(caption = "Top ten rows of the `passwords` dataset.",
        row.names = FALSE) %>%
...
passwords %>%
  slice_tail(n = 10) %>%
  kable(caption = "Bottom ten rows of the `passwords` dataset.",
        row.names = FALSE) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped")  
```
...
passwords_var_def <- tribble(
  ...
  "strength",  "Strength of password, relative to only to passwords in this dataset. Lower values indicate weaker passwords."
)

passwords_var_def %>%
  mutate(variable = cell_spec(variable, monospace = TRUE)) %>%
  kable(escape = FALSE, caption = "Variables and their descriptions for the `passwords` data set.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped")
```
...
```{r passwords-cat, fig.cap = "Distributions of the categorical variables in the `passwords` dataset. Plot A shows the distribution of password strengths, Plot B password categories, and Plot C length of time it takes to crack the passwords by online guessing.", out.width = "100%", fig.asp = 1}
...
p_time_unit <- passwords %>%
  ...
  geom_col(show.legend = FALSE, fill = COL["blue", "full"]) +
  labs(
    x = "Frequency", 
    y = NULL, 
...
p_strength <- passwords %>%
  ...
  geom_histogram(binwidth = 1, show.legend = FALSE, fill = COL["blue", "full"]) +
...
passwords %>%
  ...
  kable(caption = "Minimum and maximum amount of time it takes to crack a password by online guessing as well as the number of observations that fall into each time unit category.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped")
```
...
passwords %>% 
  filter(value == 92.27) %>%
  kable(caption = "Passwords that take the longest amount of time to crack by online guessing.",
        row.names = FALSE) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped")  
```
