...
{
\begin{enumerate}[(a)]
\item The relationship is linear therefore the residuals plot will show randomly distributed residuals around 0.
...
\eoce{For the residual plots given below, describe any apparent trends and determine if a linear model would be appropriate for these data. Explain your reasoning.
\begin{center}
...
{
(2) and (5) show a strong correlation. Even though (1) and (4) show a strong association, the relationship is not linear therefore correlation would not be strong. (3) and (6) show very weak or no relationship.
}
...
{
(3) and (6) show a strong correlation. Even though (1) and (2) show a strong association, the relationship is not linear therefore correlation would not be strong. (4) and (5) show very weak relationship.
}
...
\item Exam 2 and the final are relatively close to each other chronologically, or Exam 2 is probably cumulative so has greater similarities in material to the final exam.
\end{enumerate}
}
...
\item Data on heights were originally collected in centimeters, and then converter to inches. Does this conversion affect the correlation between husbands' and wives' heights?
\end{enumerate}
}
{
\begin{enumerate}[(a)]
...
{
\begin{enumerate}[(a)]
\item The relationship appears to be strong, positive and linear. There appears to be one outlier, a student who is about 63 inches tall whose fastest speed is 0 mph. This is probably a student who doesn't drive.
\item It is unlikely that being tall makes people drive faster. One possible outside factor may be gender. Males tend to be taller than females on average, and they also tend to drive faster.
\item It appears that males are taller on average than females and they also drive faster. The positive association between speed and height appears to be driven by gender.
\end{enumerate}
}
...
\item Correlation between travel time (in miles) and distance (in minutes), R = 0.636. What is the correlation between travel time (in minutes) and distance (in hours).
\end{enumerate}
%\begin{center}
...
\item How would the relationship change weight was measured in pounds while the units for hip girth remained in centimeters?
\item[]
\end{enumerate}
\begin{center}
...
\eoce{Association of Turkish Travel Agencies reports the number of foreign tourists visiting Turkey and tourist spending by year \citep{data:turkeyTourism}. The below scatterplot shows the relationship between these two variables along with the least squares fit. \vspace{-5mm}
\begin{multicols}{2}
\begin{enumerate}[(a)]
\item[]
...
{
\begin{enumerate}[(a)]
\item There is a positive, moderately strong, linear association between number of tourists and spending. 
...
% Conditions for the least squares line (linearity, nearly normal residuals, constant variability)
...
\edit{\eoce{Exercise~\eoceref{coastStarlight} introduces data on the Coast Starlight Amtrak train that runs from Seattle to Los Angeles. The mean travel time from one stop to the next on the Coast Starlight is 129 mins, with a standard deviation of 113 minutes. The mean distance travelled from one stop to the next is 107 miles with a standard deviation of 99 miles. The correlation between travel time and distance is 0.636.
\begin{enumerate}[(a)]
\setlength{\itemsep}{0mm}
...
\item The distance between Santa Barbara and Los Angeles is 103 miles. Calculate the following based on the linear model.
\item It actually takes the the Coast Starlight 168 mins to travel from Santa Barbara to Los Angeles. Calculate the residual and explain what this residual means.
...
\[ \hat{b}_1 = \frac{s_y}{s_x} R = \frac{113}{99} * 0.636 = 0.726 \]
Then we make use of the fact that the regression line passes through the point $(\bar{x},\bar{y})$.
\begin{align*}
y - \bar{y} &= \hat{b}_1 (x - \bar{x}) \\
y - 129 &= 0.726 * (x - 107) \\
\widehat{travel~time} &=  51 + 0.726 * distance
\end{align*}
\item $\hat{b}_1$ = For one mile increase in distance, we would expect the travel time to increase on average by 0.726 minutes. \\
$\hat{b}_0$ = When the distance travelled is 0 miles, the travel time is expected to be 51 minutes. It does not make sense to have a travel distance of 0 miles. Here, the $y$-intercept serves only to adjust the height of the line and is meaningless by itself.
...
\begin{enumerate}[(a)]
\item First we calculate the slope.
\[ \hat{b}_1 = \frac{s_y}{s_x} R = \frac{9.41}{10.37} * 0.666 = 0.604\]
Then we make use of the fact that the regression line passes through the point $(\bar{x},\bar{y})$.
\begin{align*}
y - \bar{y} &= \hat{b}_1 (x - \bar{x}) \\
...
\end{align*}
\item $\hat{b}_1$ = For each centimeter increase in shoulder girth we would expect height to increase on average by 0.604 centimeters. \\
$\hat{b}_0$ = People who have a shoulder girth of 0 cm are expected on average to be 105.79 cm tall. A person with a 0 cm shoulder girth does not make any sense. Here, the $y$-intercept serves only to adjust the height of the line and is meaningless by itself.
...
{
$R^2 = 0.636^2 = 0.40$. \\
40\% of the variation in travel time is accounted for by the model, i.e. explained by distance travelled.
}
...
{
$R^2 = 0.666^2 = 0.44$. \\
44\% of the variation in heights is accounted for by the model, i.e. explained by shoulder girth.
}}
...
\eoce{
Data was collected on the number of hours per week students watch TV and the grade they got in a statistics class (out of 100). Based on the scatterplot and the residuals plot below, describe the relationship between the two variables and determine if a simple linear model appropriate to predict grade from the number of hours per week the student watches TV.
\vspace{2.5cm}
}
{
...
\noindent \begin{minipage}[c]{0.55\textwidth}
\eoce{
Exercise~\eoceref{starbucksCarbCals} introduces a data set on nutrition information on Starbucks food menu items. Based on the scatterplot and the residuals plot below, describe the relationship between the protein amount (in grams) and the calories of these menu items and determine if a simple linear model is appropriate to predict protein amount from the number of calories.
\vspace{2.5cm}
}
{
...
\eoce{Identify the outliers in the scatter plots shown below and determine what type of an outlier it is. Explain your reasoning.
\begin{center}
...
{
\eoce{Exercise~\eoceref{babiesCrawl} introduces a data on the average monthly temperature during the month babies first try to crawl (about 6 months after birth) and the average first crawling age for babies born in a given month. A scatterplot of these two variables reveals an outlying month when the average temperature is about 53 degrees Fahrenheit and average crawling age is about 28.5 weeks. What type of an outlier is this month? Explain your reasoning.
}
...
{\begin{enumerate}[(a)]
\item There is a negative, moderately strong, somewhat linear relationship between percent of families who own their home and the percent of the population living in urban areas in 2000. There appears to be one outlier, a state where 100\% of the population is urban.
\item The outlier is located in the bottom right corner of the plot away from the regression line. It is an influential point since if we were to exclude this point from the analysis the slope of the regression line would be affected greatly.
\end{enumerate}
}\label{urbanOwner}
}
...
\item Do the data provide convincing evidence that drinking more beers is associated with an increase in blood alcohol? State the null and alternative hypotheses, report the p-value, and state your conclusion.
...
Slope: For each additional can of beer consumed, blood alcohol content is expected to increase on average by 0.0180. \\
...
\eoce{The scatterplot and least squares summary below shows the relationship between weight measured in kilograms and height measured in centimeters based on data discussed in Exercise~\eoceref{bodyHgtShoGi}. \\
\begin{minipage}[c]{0.4\textwidth}
\begin{center}
...
\item Write the equation of the regression line. Interpret the slope and intercept in context.
\item Do the data provide convincing evidence that an increase in height is associated with an increase in weight? State the null and alternative hypotheses, report the p-value, and state your conclusion.
...
Slope: For each centimeter increase in height, weight is expected to increase on average by 1.0176 kilograms. \\
...
\item $H_0$: The true coefficient for \texttt{height} is zero ($b_1 = 0$). \\
$H_0$: The true coefficient for \texttt{height} is greater than zero ($b_1 > 0$). \\
...
52\% of the variability in weights can be explained by height.
\end{enumerate}
}
...
{
\eoce{Exercise~\eoceref{husbandsWives} presents a scatterplots displaying the relationship between husbands' and wives' ages in a random sample of 199 married couples in Britain. Given below is a summary output of the least squares fit for predicting wife's age from husband's age.
\begin{center}
\begin{tabular}{rrrrr}
  \hline
 & Estimate & Std. Error & t value & Pr($>$$|$t$|$) \\ 
...
\begin{enumerate}[(a)]
\setlength{\itemsep}{0mm}
\item Is there a statistically significant relationship between husbands' and wives' heights?  State the hypotheses and include any information used to conduct the test.
...
{
\begin{enumerate}[(a)]
\item $H_0$: The true coefficient for \texttt{ageHusband} is zero ($b_1 = 0$). \\
$H_0$: The true coefficient for \texttt{ageHusband} is not zero ($b_1 \ne 0$). \\
...
\item Slope: For each additional year in husband's age, wife's age is expected to increase on average by 0.9112 years. \\
...
{
\eoce{Exercise~\eoceref{husbandsWives} presents a scatterplots displaying the relationship between husbands' and wives' heights in a random sample of 199 married couples in Britain. Given below is a summary output of the least squares fit for predicting wife's height from husband's height.
\begin{center}
\begin{tabular}{rrrrr}
  \hline
 & Estimate & Std. Error & t value & Pr($>$$|$t$|$) \\ 
...
\begin{enumerate}[(a)]
\setlength{\itemsep}{0mm}
\item Is there convincing evidence to suggest that taller men marry taller women? State the hypotheses and include any information used to conduct the test.
\item Write the equation of the regression line for predicting wife's age from husband's age.
\item Interpret the slope and intercept in context.
\end{enumerate}
}
{
\begin{enumerate}[(a)]
\item $H_0$: The true coefficient for \texttt{htHusband\_in} is zero ($b_1 = 0$). \\
$H_0$: The true coefficient for \texttt{htHusband\_in} is greater than zero ($b_1 > 0$). \\
...
\item Slope: For each additional inch in husband's height, wife's height is expected to increase on average by 0.3310 inches. \\
...
{
\eoce{Exercise~\eoceref{husbandsWivesAgeModel} gives a summary output of the least squares fit for predicting wife's age from husband's age.
\begin{enumerate}[(a)]
\setlength{\itemsep}{0mm}
...
{
\eoce{Exercise~\eoceref{husbandsWivesHeightModel} gives a summary output of the least squares fit for predicting wife's height from husband's height.
\begin{enumerate}[(a)]
\setlength{\itemsep}{0mm}
...
{
\begin{enumerate}[(a)]
\item  The regression of wives' ages on husbands' ages has a positive slope, so the correlation coefficient will be positive.  Since the $R^2 = 0.13$, $R = \sqrt{0.13} = 0.36$.
...
{
\eoce{Exercise~\eoceref{urbanOwner} gives a scatterplot displaying the relationship between the percent of families that own their home. Below is a summary of a least squares line for this data, excluding District of Columbia. There were 51 cases. \\
\begin{center}
\begin{tabular}{rrrrr}
  \hline
 & Estimate & Std. Error & t value & Pr($>$$|$t$|$) \\ 
...
\begin{enumerate}[(a)]
\setlength{\itemsep}{0mm}
\item For this data, $R^2=0.28$. What is the correlation? How can you tell if it is positive or negative?
\item Test the hypothesis that there is no association between percent home ownership and percent of the population living in an urban setting. State the hypotheses and include any information used to conduct the test.
\item Calculate the predicted values percent home ownership for populations where 40\% and 80\% live in an urban setting. Use these two values to sketch the regression line on the scatterplot. State whether you believe a least squares line adequately fits these data.
\item The residual plot for this regression is given below. How would you describe the trend visible in this plot? Based on this plot, should a least square line be fit to these data?
\begin{center}
...
\item $H_0$: The true coefficient for \texttt{gestationalAge} is zero ($b_1 = 0$). \\
$H_0$: The true coefficient for \texttt{gestationalAge} is greater than zero ($b_1 \ne 0$). \\
