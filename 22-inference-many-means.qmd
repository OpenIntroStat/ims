...
-   $H_0:$ The mean outcome is the same across all groups. In statistical notation, $\mu_1 = \mu_2 = \cdots = \mu_k$ where $\mu_j$ represents the mean of the outcome for observations in category $j.$
...
```{r}
#| label: fig-toyANOVA
#| fig-cap: 'Side-by-side dot plot for the outcomes for six groups. Two sets of groups:
#|   first set is comprised of Groups I, II, and III, the second set is comprised of
#|   Groups IV, V, and VI.'
#| fig-alt: Two sets of side by side dot plots.  The first set shows three 
...
#| fig-cap: Side-by-side box plots of the on-base percentage for 429 players across three
...
#| fig-alt: Side-by-side box plots describing on-base percentage broken down by 
...
Naturally, we would pick the groups with the large differences for the formal test, and this would lead to an inflation in the Type 1 Error rate.
...
[^22-inference-many-means-3]: Let $\bar{x}$ represent the mean of outcomes across all groups.
    Then the mean square between groups is computed as $MSG = \frac{1}{df_{G}}SSG = \frac{1}{k-1}\sum_{j=1}^{k} n_{j} \left(\bar{x}_{j} - \bar{x}\right)^2$ where $SSG$ is called the **sum of squares between groups** and $n_{j}$ is the sample size of group $j.$
...
[^22-inference-many-means-4]: See [additional details on ANOVA calculations](https://www.openintro.org/download.php?file=stat_extra_anova_calculations) for interested readers.
    ...
    Then we compute the **sum of squared errors** $(SSE)$ in one of two equivalent ways: $SSE = SST - SSG = (n_1-1)s_1^2 + (n_2-1)s_2^2 + \cdots + (n_k-1)s_k^2$ where $s_j^2$ is the sample variance (square of the standard deviation) of the residuals in group $j.$ Then the $MSE$ is the standardized form of $SSE: MSE = \frac{1}{df_{E}}SSE.$
...
#| fig-alt: Side-by-side boxplots of exam score boken down by exam A, exam B, or 
...
```{r}
#| label: fig-randANOVA
#| fig-cap: The version of the test (A or B or C) is randomly allocated to the test scores,
...
#|   5 of the scores are randomly reassigned to B, 4 of the scores are randomly reassigned
#|   to C).  The third panel shows which score is connected with which new reassigned
...
#|   average score for version B is 78, and the average score for version C is 75.25.  The
#|   randomized F statistic is 0.1637.
...
#| fig-cap: Histogram of F statistics calculated from 1,000 different randomizations
#|   of the exam type.
#| fig-alt: Histogram of the F statistics that were calculated from 1000 different
...
#| fig-cap: Histogram of F statistics calculated from 1000 different randomizations of
...
#| fig-alt: A line plot showing the theoretical F distribution with the first degree
...
#| tbl-cap: ANOVA summary for testing whether the average on-base percentage differs across player positions.
...
#| fig-alt: Three separate histograms of on-base percentage, one for each field position
