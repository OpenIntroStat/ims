...
In this chapter we will focus on bootstrapping to estimate a single proportion, and we will revisit bootstrapping in Chapters \@ref(inference-one-mean) through \@ref(inference-paired-means), so you will get plenty of practice as well as exposure to bootstrapping in many different datasettings.
...
One simulation is not enough to get a sense of the variability from one bootstrap proportion to another bootstrap proportion, so we repeat the simulation 10,000 times using a computer.
...
Here is a game you can try with your friends or family: pick a simple, well-known song, tap that tune on your desk, and see if the other person can guess the song.
...
After selecting 120 marbles, we counted 2 red for $\hat{p}_{boot1} = 0.0167.$ As we did with the randomization technique, seeing what would happen with one simulation is not enough.
...
As before, we will run a total of 10,000 simulations using a computer.
...
```{r tappers-bs-sim, fig.cap = "(ref:tappers-bs-sim-cap)"}
bsprop_tap <- tibble(
