...
#| tbl-cap: |
#|   Descriptions of nine variables from the `resume` data set. Many 
...
@tbl-resume-full-fit-aic provides the AIC (and BIC, which is similar to AIC but beyond the scope of this text) and the number of observations used to fit the model.  We also know from @tbl-resume-full-fit that eight variables (with nine coefficients, including the intercept) were fit.
...
resume_full_fit |>
  glance() |>
  select(AIC, BIC, nobs) |>
  dplyr::rename(number_observations = nobs) |>
  kbl(linesep = "", booktabs = TRUE, 
      digits = 2, align = "rrr") |>
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = c("striped", "hold_position")) |>
...
We will look for models with a lower AIC using a backward elimination strategy.  @tbl-resume-fit-aic provides the AIC (and BIC) values for the model with variables as given in @tbl-resume-fit.  Notice that the same number of observations are used, but one fewer variables (`college_degree` is dropped from the model).
