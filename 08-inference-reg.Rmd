...
\begin{eqnarray*}
H_0: \beta_1 = 0 && \mbox{ given `cc` and `dollar` and `urgent_subj` are included in the model}\\
H_0: \beta_2 = 0 &&\mbox{ given `to_multiple` and `dollar`  and `urgent_subj` are included in the model}\\
H_0: \beta_3 = 0 &&\mbox{ given `to_multiple` and `cc` and `urgent_subj` and are included in the model}\\
H_0: \beta_4 = 0 &&\mbox{ given `to_multiple` and `cc` and `dollar` are included in the model}
\end{eqnarray*}
...
The model from the first cross validation sample is given in Table \@ref(tab:emailCV1) and can be compared to the coefficients in Table \@ref(tab:emaillogmodel1).
...
The logistic regression model can be written as (where $\hat{p}$ is the estimated probability of being a SPAM email message):
\begin{align}
\log_e\bigg(\frac{\hat{p}}{1-\hat{p}}\bigg) =  -0.34 - 2.56\times \mbox{to_multiple} + 0.20 \times \mbox{attach} + 1.73 \times \mbox{winner}_{yes} - 1.28 \times \mbox{format} - 2.86 \times \mbox{re_subj} + 0 \times \mbox{exclaim_mess} - 1.07 \times \mbox{number}_{small} - 0.42 \times \mbox{number}_{big}
\end{align}
