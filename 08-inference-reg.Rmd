...
The p-value gives a sense of which variables are important to the model (with the caveat that a longer treatment of variable selection is waranted in a follow-up course or textbook).
We use cross validation prediction error to focus on which variable(s) are important for predicting the response variable of interest.  We continue to encourage you to take up further study on the topic of cross validation, as it is among the most important ideas in modern data analysis, and we are only able to scratch the surface here.
...
The goal is to be able to predict the birth weight (`bwt` in oz) of the baby as a function of the `weight` gained (in lbs) by the mother and the `gestation` time (in days) of the pregnancy.
The multiple linear model shows that both `weight` and `gestation` are statistically significant predictors of `bwt`, but our cross validation analysis will be independent of the calculated p-values.
The question we will seek to answer is wehter the predictions of `bwt` are substantially better when both `weight` and `gestation` are used in the model, as compared with the predictions from only `weight`.

```{r babieslm}
lm(bwt ~ weight + gestation, data = babies) %>% tidy()
```

\begin{eqnarray*}
\mbox{Model } A && E[\texttt{age}] = \beta_0 + \beta_1 \times \texttt{weight}\\
\mbox{Model } B && E[\texttt{age}] = \beta_0 + \beta_1 \times \texttt{weight} + \beta_2 \times \texttt{age}
\end{eqnarray*}
