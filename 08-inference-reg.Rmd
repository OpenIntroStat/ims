...
We start this chapter with a hypothetical example describing the linear relationship between dollars spent advertising for a chain sandwich restaurant and monthly revenue.  The hypothetical example serves the purpose of illustrating how a linear model varies from sample to sample.  Because we have made up the example and the data (and the entire population), we can take many many samples from the population to visualize the variability.  Note that in real life, we always have exactly one sample (that is, one dataset), and through the inference process, we imagine what might have happened had we taken a different sample.  The change from sample to sample leads to an understanding of how the single observed dataset is different from the population of values, which is the fundamental inference goal. 

Consider the following hypothetical population of all of the sandwich stores of a particular chain seen in Figure \@ref(fig:sandpop).  There appears to be linear relationship between advertising dollars and revenue (both in $000).
...
You may remember from Chatper \@ref(intro-linear-models) that the population model is: $$y = \beta_0 + \beta_1 x + \varepsilon.$$


The least squares regression model will use the data to find a sample linear fit:  $$\hat{y} = b_0 + b_1 x.$$
...
A subset of size 20 stores shows a similar positive trend between advertisting and revenue (to what we saw in Figure \@ref(fig:sandpop) which described the population) despite having fewer observations on the plot.
...
ggplot(sandwich2) +
  ...
  geom_smooth(aes(x = ad, y = rev), method = "lm", se = FALSE, color = COL[1,1]) +
  xlab("amount spent on advertising in thousands of dollars") +
  ylab("revenue in thousands of dollars")
```
...
ggplot(sandwich3) +
  geom_point(aes(x = ad, y = rev)) +
  geom_smooth(aes(x = ad, y = rev), method = "lm", se = FALSE, color = COL[4,1]) +
  xlab("amount spent on advertising in thousands of dollars") +
  ylab("revenue in thousands of dollars")
```
...
```{r echo = FALSE, fig.cap = "If repeated samples of size 20 are taken from the entire population, each linear model will be slightly different."}
ggplot(sandwich_many, aes(x=ad, y=rev, group=replicate)) + 
  geom_point() + 
  geom_smooth(method="lm", se=FALSE, color=COL[1,1], fullrange = TRUE)
```  
...
Remember that the tools in this textbook are designed to evaluate only one single sample of data.  
...
Instead, we use the same techniques covered in previous chapters to understand the variability of a slope.
We will describe randomization test on the slope (see Section \@ref(randslope)), bootstrapping the slope (see Section \@ref(bootbeta1)), and the mathematical model (see Section \@ref(mathslope)) applied to the linear model.
...
```{r crop2BS, echo = FALSE, fig.cap = "Two bootstrap samples of the crop data.  Note that it is difficult to differentiate the two plots, as (within a single bootstrap sample) the observations which have been resampled twice are plotted as points on top of one another.",  fig.show = 'hold', out.width = '47%', fig.ncol = 2}
...
Figure \@ref(fig:crop2BS) shows two different bootstrapped resamples and resulting (slightly) different linear models.  By repeatedly resampling, we can see dozens of bootstrapped slopes on the same plot in Figure \@ref(fig:cropBS).
...
```{r peasBSslopes, fig.cap="The original Awesome Auto data is bootstrapped 1,000 times. The histogram provides a sense for the variability of the standard deviation of the car prices from sample to sample.", warning=FALSE, fig.width=15}
