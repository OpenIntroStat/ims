...
The unknown population model can be written as:
\begin{align}
\log_e\bigg(\frac{p}{1-p}\bigg) =  \beta_0 + \beta_1\times \mbox{to_multiple} + \beta_2 \times \mbox{cc} + \beta_3 \times \mbox{dollar} + \beta_4 \times \mbox{urgent_subj}
\end{align}
...
\begin{align}
\log_e\bigg(\frac{\hat{p}}{1-\hat{p}}\bigg) =  -2.05 + -1.91\times \mbox{to_multiple} + 0.02 \times \mbox{cc} - 0.07 \times \mbox{dollar} + 2.66\times \mbox{urgent_subj}
\end{align}
...
\begin{eqnarray*}
H_0: \beta_1 = 0 && \mbox{ given \mbox{cc} and \mbox{dollar} and \mbox{urgent_subj} are included in the model}\\
H_0: \beta_2 = 0 &&\mbox{ given \mbox{to_multiple} and \mbox{dollar}  and \mbox{urgent_subj} are included in the model}\\
H_0: \beta_3 = 0 &&\mbox{ given \mbox{to_multiple} and \mbox{cc} and \mbox{urgent_subj} and are included in the model}
H_0: \beta_4 = 0 &&\mbox{ given \mbox{to_multiple} and \mbox{cc} and \mbox{dollar} are included in the model}
\end{eqnarray*}
...
That p-value provides information about the degree of the relationship (e.g., above we measure the relationship between `spam` and `to_multipl` using a p-value), but the p-value does not measure how well the model will predict the individual emails (e.g., the accuracy of the model).
Depending on the goal of the research project, you might be inclined to focus on variable importance (through p-values) or you might be inclined to focuse on prediction accuracy (through cross validation).

Here we present a method for using cross validation accuracy to detrmine which variables (if any) should be used in a model which predicts whether an email is SPAM.
A full treatment of cross validation and multiple regression models is beyond the scope of this text.
The models below compare the predictions using first only one variable (see Table \@ref(tab:email-spam)) and then using seven variables (see Table \@ref(tab:email-spam2)) .



```{r email-spam, fig.cap = "One quarter at a time, the data were removed from the model building, and whether the email was spam (TRUE) or not (FALSE) was predicted.  The logistic regression model was fit independently of the removed emails. Only `to_multiple` is used to predict whether the email is spam.", warning=FALSE, fig.width=20}

set.seed(47)
data(email)
emailfolds <- caret::createFolds(email$spam, k = 4)

logCV <- data.frame(predicted = glm(spam ~ to_multiple, data = email[-emailfolds$Fold1,], family = "binomial") %>% 
