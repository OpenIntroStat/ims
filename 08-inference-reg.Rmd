...
&\qquad\qquad= -7.36 - 0.89 \times \texttt{(unemployment rate)}\end{aligned}$$
...
$$E[\texttt{interest_rate}] = \beta_0 + \beta_1\times \texttt{debt_to_income} + \beta_2 \times \texttt{term} + \beta_3 \times \texttt{credit_checks}$$
...
\begin{eqnarray*}
H_0: \beta_1 = 0 && \mbox{ given \texttt{term} and \texttt{credit_checks} are included in the model}\\
H_0: \beta_2 = 0 &&\mbox{ given \texttt{debt_to_income} and \texttt{credit_checks} are included in the model}\\
H_0: \beta_3 = 0 &&\mbox{ given \texttt{debt_to_income} and \texttt{term} and are included in the model}
\end{eqnarray*}
...
The presentation below (see the comparison of Figures \@ref(fig:peng_mass1) and \@ref(fig:peng_mass2)) shows that the model with more variables predicts `body_mass_g` with much smaller errors (predicted minus actual body mass) than the model which uses only `bill_length_g`.
We have deliberate used a model that intuitively makes sense (the more body measurements, the more predictable mass is).
...
The population model $A$ is given below, and the estimates of the parameters (on the entire dataset) are provided in Table \@ref(tab:peng_lm_bill).
The population model $B$ is given below, and the estimates of the parameters (on the entire dataset) are provided in Table \@ref(tab:peng_lm_all).
...
\begin{eqnarray*}
\mbox{Model } A && E[\texttt{body_mass_g}] = \beta_0 + \beta_1 \times \texttt{bill_length_mm}\\
\mbox{Model } B && E[\texttt{body_mass_g}] = \beta_0 + \beta_1 \times \texttt{bill_length_mm} + \beta_2 \times \texttt{bill_depth_mm} + \beta_3 \times \texttt{flipper_length_mm} + \beta_4 \times \texttt{sex} + \beta_5 \times \texttt{species}\\
\end{eqnarray*}

```{r peng_lm_bill, echo = FALSE}
...
\begin{eqnarray*}
\mbox{Model } B_1 && \widehat{\texttt{bwt}} = 99.69 + 0.15 \times \texttt{weight} + 0.03 \times \texttt{age}\\
\mbox{Model } B_2 && \widehat{\texttt{bwt}} = 100.79 + 0.14 \times \texttt{weight} + 0.03 \times \texttt{age}\\
\mbox{Model } B_3 && \widehat{\texttt{bwt}} = 102.57 + 0.11 \times \texttt{weight} + 0.08 \times \texttt{age}\\
\mbox{Model } B_4 && \widehat{\texttt{bwt}} = 103.41 + 0.12 \times \texttt{weight} + 0.06\times \texttt{age}\\
\end{eqnarray*}


```{r peng_mass2, fig.cap = "One quarter at a time, the data were removed from the model building, and the body mass of the removed penguins was predicted.  The least squares regression model was fit independently of the removed penguins.  The predictions of body mass are based on bill length only.  The x-axis represents the predicted value, the y-axis represents the error (difference between predicted value and actual value).", warning=FALSE, fig.width=15}
data.frame(predicted = lm(body_mass_g ~ bill_length_mm + bill_depth_mm + flipper_length_mm + sex + species, data = penguins[-penguinfolds$Fold1,]) %>% 
           ...
           obs = penguins[penguinfolds$Fold1,]$body_mass_g,
           fold = rep("1st auarter", length(penguinfolds$Fold1))) %>%
...
Figure \@ref(fig:peng_mass1) shows that the independent predictions are centered around the true values (i.e., errors are centered around zero), but that the predictions can be as much as 1000g off when using only `bill_length_mm` to predict `body_mass_g`.
On the other hand, when using `bill_length_mm`, `bill_depth_mm`, `flipper_length_mm`, `sex`, and `species`  to predict `body_mass_g`, the prediction errors seem to be about half as big, as seen in Figure \@ref(fig:peng_mass2).
