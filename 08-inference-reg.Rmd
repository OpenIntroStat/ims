...
In Section \@ref(inf-mult-reg), p-values were calculated on each of the model coefficients.
...
data.frame(predicted = lm(bwt ~age, data = babies[-babyfolds$Fold1,]) %>% 
             predict(newdata = babies[babyfolds$Fold1,"age"]) ,
           babies[babyfolds$Fold1,"bwt"],
           fold = rep("Fold 1", length(babyfolds$Fold1))) %>%
  rbind(data.frame(predicted = lm(bwt ~age, data = babies[-babyfolds$Fold2,]) %>% 
             predict(newdata = babies[babyfolds$Fold2,"age"]) ,
           babies[babyfolds$Fold2,"bwt"],
           fold = rep("Fold 2", length(babyfolds$Fold2))) ) %>%
  rbind(data.frame(predicted = lm(bwt ~age, data = babies[-babyfolds$Fold3,]) %>% 
             predict(newdata = babies[babyfolds$Fold3,"age"]) ,
           babies[babyfolds$Fold3,"bwt"],
           fold = rep("Fold 3", length(babyfolds$Fold3))) ) %>%
  rbind(data.frame(predicted = lm(bwt ~age, data = babies[-babyfolds$Fold4,]) %>% 
             predict(newdata = babies[babyfolds$Fold4,"age"]) ,
           babies[babyfolds$Fold4,"bwt"],
           fold = rep("Fold 4", length(babyfolds$Fold4))) ) %>%
  mutate(error = bwt - predicted) %>%
  ggplot(aes(x = predicted, y = error )) +
  geom_point() +
  geom_hline(yintercept = 0) +
  facet_wrap(~fold) +
  ylim(c(-80,80)) +
  xlab("Predicted Birthweight (oz)") +
  ylab("Prediction Error (oz)") 
```
...
data.frame(predicted = lm(bwt ~gestation + weight, data = babies[-babyfolds$Fold1,]) %>% 
             predict(newdata = babies[babyfolds$Fold1,c("gestation", "weight")]) ,
           babies[babyfolds$Fold1,"bwt"],
           fold = rep("Fold 1", length(babyfolds$Fold1))) %>%
  rbind(data.frame(predicted = lm(bwt ~gestation + weight, data = babies[-babyfolds$Fold2,]) %>% 
             predict(newdata = babies[babyfolds$Fold2,c("gestation", "weight")]) ,
           babies[babyfolds$Fold2,"bwt"],
           fold = rep("Fold 2", length(babyfolds$Fold2))) ) %>%
  rbind(data.frame(predicted = lm(bwt ~gestation + weight, data = babies[-babyfolds$Fold3,]) %>% 
             predict(newdata = babies[babyfolds$Fold3,c("gestation", "weight")]) ,
           babies[babyfolds$Fold3,"bwt"],
           fold = rep("Fold 3", length(babyfolds$Fold3))) ) %>%
  rbind(data.frame(predicted = lm(bwt ~gestation + weight, data = babies[-babyfolds$Fold4,]) %>% 
             predict(newdata = babies[babyfolds$Fold4,c("gestation", "weight")]) ,
           babies[babyfolds$Fold4,"bwt"],
           fold = rep("Fold 4", length(babyfolds$Fold4))) ) %>%
  mutate(error = bwt - predicted) %>%
  ggplot(aes(x = predicted, y = error )) +
