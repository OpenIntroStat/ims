...
$$\begin{aligned}
&\mbox{% change in House seats for President's party}  \\
&\qquad\qquad= -7.36 - 0.89 \times \mbox{(unemployment rate)}\end{aligned}$$
...
* **Nearly normal residuals.**  Generally, the residuals must be nearly normal. When this condition
    ...
    residual that would be a potentially concern is shown in the second panel of
...
##### The importance of **I**ndependence {-}
...
##### **E**qual variability for prediction in particular {-}

As with normality, the equal variability condition (that points are spread out in similar ways around the line for all values of $x$) will not cause problems for the estimate of the linear model. That said, the **inference** on the model (e.g., computing p-values) will be incorrect if the variability around the line is heterogeneous.
...
$$E[\mbox{interest_rate}] = \beta_0 + \beta_1\times \mbox{debt_to_income} + \beta_2 \times \mbox{term} + \beta_3 \times \mbox{credit_checks}$$
...
\begin{eqnarray*}
H_0: \beta_1 = 0 && \mbox{ given \mbox{term} and \mbox{credit_checks} are included in the model}\\
H_0: \beta_2 = 0 &&\mbox{ given \mbox{debt_to_income} and \mbox{credit_checks} are included in the model}\\
H_0: \beta_3 = 0 &&\mbox{ given \mbox{debt_to_income} and \mbox{term} and are included in the model}
\end{eqnarray*}
...
```{r cv, fig.cap = "This isn't the right picture, but I'd love to keep CV in the book and be able to explain it in a straightforward way.", fig.width = 10}
...
The observations of three different penguin species include measurements on body size and sex.  The data were collected by [Dr. Kristen Gorman](https://www.uaf.edu/cfos/people/faculty/detail/kristen-gorman.php) and the [Palmer Station, Antarctica LTER](https://pal.lternet.edu/) as part of the [Long Term Ecological Research Network](https://lternet.edu/).
:::
...
The population model $A$ is given below, and the estimates of the parameters (on the entire dataset) are provided in Table \@ref(tab:peng-lm-bill).
The population model $B$ is given below, and the estimates of the parameters (on the entire dataset) are provided in Table \@ref(tab:peng-lm-all).
...
\begin{eqnarray*}
\mbox{Model } A: && E[\mbox{body_mass_g}] = \beta_0 + \beta_1 \times \mbox{bill_length_mm}\\
\mbox{Model } B: && E[\mbox{body_mass_g}] = \beta_0 + \beta_1 \times \mbox{bill_length_mm} + \beta_2 \times \mbox{bill_depth_mm} + \\
&& \beta_3 \times \mbox{flipper_length_mm} + \beta_4 \times \mbox{sex}_{male} + \\
&& \beta_5 \times \mbox{species}\\
\end{eqnarray*}

```{r peng-lm-bill, echo = FALSE}
lm(body_mass_g ~ bill_length_mm, data = penguins) %>% 
  tidy() %>%
   kable(caption = "The least squares estimates of the regression model predicting `body_mass_g` from `bill_length_mm`.") %>%
 kable_styling() 
```
...
lm(body_mass_g ~ bill_length_mm + bill_depth_mm  + flipper_length_mm + sex + species, data = penguins) %>% 
  tidy()%>%
   kable(caption = "The least squares estimates of the regression model predicting `body_mass_g` from `bill_length_mm`, `bill_depth_mm`, `flipper_length_mm`, `sex`, and `species`.") %>%
 kable_styling() 
```
...
So, using cross validation, we remove one quarter of the data before running the least squares calculations.
...
\begin{eqnarray*}
\mbox{Model } A_1 && \widehat{\mbox{bst}} = 99.42 + 0.16 \times \mbox{weight}\\
\mbox{Model } A_2 && \widehat{\mbox{bst}} = 101.36 + 0.14 \times \mbox{weight}\\
\mbox{Model } A_3 && \widehat{\mbox{bst}} = 104.52 + 0.12 \times \mbox{weight}\\
\mbox{Model } A_4 && \widehat{\mbox{bst}} = 103.54 + 0.12 \times \mbox{weight}\\
\end{eqnarray*}
...
\begin{eqnarray*}
\mbox{Model } B_1 && \widehat{\mbox{bwt}} = 99.69 + 0.15 \times \mbox{weight} + 0.03 \times \mbox{age}\\
\mbox{Model } B_2 && \widehat{\mbox{bwt}} = 100.79 + 0.14 \times \mbox{weight} + 0.03 \times \mbox{age}\\
\mbox{Model } B_3 && \widehat{\mbox{bwt}} = 102.57 + 0.11 \times \mbox{weight} + 0.08 \times \mbox{age}\\
\mbox{Model } B_4 && \widehat{\mbox{bwt}} = 103.41 + 0.12 \times \mbox{weight} + 0.06\times \mbox{age}\\
\end{eqnarray*}


```{r peng-mass2, fig.cap = "One quarter at a time, the data were removed from the model building, and the body mass of the removed penguins was predicted.  The least squares regression model was fit independently of the removed penguins.  The predictions of body mass are based on bill length only.  The x-axis represents the predicted value, the y-axis represents the error (difference between predicted value and actual value).", warning=FALSE, fig.width=20}
data.frame(predicted = lm(body_mass_g ~ bill_length_mm + bill_depth_mm + flipper_length_mm + sex + species, data = penguins[-penguinfolds$Fold1,]) %>% 
             predict(newdata = penguins[penguinfolds$Fold1,c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "sex", "species")]) ,
           obs = penguins[penguinfolds$Fold1,]$body_mass_g,
           fold = rep("1st quarter", length(penguinfolds$Fold1))) %>%
  rbind(data.frame(predicted = lm(body_mass_g ~ bill_length_mm + bill_depth_mm + flipper_length_mm + sex + species, data = penguins[-penguinfolds$Fold2,]) %>% 
             predict(newdata = penguins[penguinfolds$Fold2,c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "sex", "species")]) ,
           obs = penguins[penguinfolds$Fold2,]$body_mass_g,
           fold = rep("2nd quarter", length(penguinfolds$Fold2))) ) %>%
  rbind(data.frame(predicted = lm(body_mass_g ~ bill_length_mm + bill_depth_mm + flipper_length_mm + sex + species, data = penguins[-penguinfolds$Fold3,]) %>% 
             predict(newdata = penguins[penguinfolds$Fold3,c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "sex", "species")]) ,
           obs = penguins[penguinfolds$Fold3,]$body_mass_g,
           fold = rep("3rd quarter", length(penguinfolds$Fold3))) ) %>%
  rbind(data.frame(predicted = lm(body_mass_g ~ bill_length_mm + bill_depth_mm + flipper_length_mm + sex + species, data = penguins[-penguinfolds$Fold4,]) %>% 
...
We continue to work with the Palmer Penguins collected by [Dr. Kristen Gorman](https://www.uaf.edu/cfos/people/faculty/detail/kristen-gorman.php) and the [Palmer Station, Antarctica LTER](https://pal.lternet.edu/).
Now, however, the research goal changes to focus on a model which would predict from which island a particular penguin came.
The logistic model (see Section \@ref(logistic-regression)) can be used both as a way to understand which variables are most important for modeling island origin (inference on the logistic coefficients) as well as to predict island origin for individual penguins (cross validation).  

### Multiple logistic regression output from software {#inf-log-reg-soft}

Recall the `penguins` data from Chapter \@ref(inf-mult-reg-cv).  
...
The unknown population model can be written as:
$$\log_e\bigg(\frac{p}{1-p}\bigg) =  \beta_0 + \beta_1\times \mbox{bill_length_mm} + \beta_2 \times \mbox{bill_depth_mm} + \beta_3 \times \mbox{flipper_length_mm} + \beta_4 \times \mbox{sex}_{male}$$
...
glm(island ~ bill_length_mm + bill_depth_mm + flipper_length_mm + sex, data = penguins2, family = "binomial") %>% tidy() %>%
  mutate(p.value = "<0.0001") %>%
  kable(caption = "Summary of a logistic model for predicting island of origina based on the variables bill_length_mm, bill_depth_mm, flipper_length_mm, and sex. Each of the variables has its own coefficient estimate as well as p-value significance.", digits = 2) %>%
  kable_styling(full_width = FALSE)
```
...
Not only does Table \@ref(tab:penglogmodel) provide the estimates for the coefficients, it also provides information on the inference analysis (i.e., hypothesis testing) which are the focus of this chapter.
...
\begin{eqnarray*}
H_0: \beta_1 = 0 && \mbox{ given \mbox{bill_depth_mm} and \mbox{flipper_length_mm} and \mbox{sex} are included in the model}\\
H_0: \beta_2 = 0 &&\mbox{ given \mbox{bill_length_mm} and \mbox{flipper_length_mm}  and \mbox{sex} are included in the model}\\
H_0: \beta_3 = 0 &&\mbox{ given \mbox{bill_length_mm} and \mbox{bill_depth_mm} and \mbox{sex} and are included in the model}
H_0: \beta_4 = 0 &&\mbox{ given \mbox{bill_length_mm} and \mbox{bill_depth_mm} and \mbox{flipper_length_mm} are included in the model}
\end{eqnarray*}

The very low p-values from the software output tell us that each of the variables acts as an important predictor in the model, despite the inclusion of any of the other three.
Consider the p-value on $H_0: \beta_1$.
The low p-value says that it would be extremely unlikely to see data that produce a coefficient on `bill_length_mm` as large as 0.22 if the true relationship between `bill_length_mm` and `island` was non-existent (i.e., if $\beta_1 = 0$) and the model also included ` `bill_depth_mm` and `flipper_length_mm` and `sex`.
You might have thought that the value 0.22 is a small number (i.e., close to zero), but in the units of the problem, 0.22 turns out to be far away from zero, it's all about context!
...
Here, we revisit cross validation within the context of predicting island of origin for each of the individual penguins.  
...
```{r peng-island, fig.cap = "One quarter at a time, the data were removed from the model building, and the location (island) of the removed penguins was predicted.  The logistic regression model was fit independently of the removed penguins.  Only bill_length_mm is used to predict the island of origin.", warning=FALSE, fig.width=20}

penguins2 <- penguins %>%
  filter(island %in% c("Biscoe", "Dream"))

penguinfolds2 <- caret::createFolds(penguins2$island, k = 4)

logCV <- data.frame(predicted = glm(island ~ bill_length_mm, data = penguins2[-penguinfolds2$Fold1,], family = "binomial") %>% 
             predict(newdata = penguins2[penguinfolds2$Fold1,c("bill_length_mm")], type="response") ,
           obs = penguins2[penguinfolds2$Fold1,]$island,
           fold = rep("1st quarter", length(penguinfolds2$Fold1))) %>%
  rbind(data.frame(predicted = glm(island ~ bill_length_mm , data = penguins2[-penguinfolds2$Fold2,], family = "binomial") %>% 
             predict(newdata = penguins2[penguinfolds2$Fold2,c("bill_length_mm")], type="response") ,
           obs = penguins2[penguinfolds2$Fold2,]$island,
           fold = rep("2nd quarter", length(penguinfolds2$Fold2))) ) %>%
  rbind(data.frame(predicted = glm(island ~ bill_length_mm , data = penguins2[-penguinfolds2$Fold3,], family = "binomial") %>% 
             predict(newdata = penguins2[penguinfolds2$Fold3,c("bill_length_mm")], type="response") ,
           obs = penguins2[penguinfolds2$Fold3,]$island,
           fold = rep("3rd quarter", length(penguinfolds2$Fold3))) ) %>%
  rbind(data.frame(predicted = glm(island ~ bill_length_mm , data = penguins2[-penguinfolds2$Fold4,], family = "binomial") %>% 
             predict(newdata = penguins2[penguinfolds2$Fold4,c("bill_length_mm")], type="response") ,
           obs = penguins2[penguinfolds2$Fold4,]$island,
           fold = rep("4th quarter", length(penguinfolds2$Fold4))) ) %>%
  mutate(predIsland = ifelse(predicted <= 0.5, "Biscoe", "Dream")) 

logCV %>%
  select(obs, predIsland, fold) %>%
  table()

logCV %>%
  filter(!is.na(predIsland)) %>%
  group_by(fold) %>%
  summarize(count = n(), accuracy = sum(obs==predIsland) / n())

```


```{r peng-island2, fig.cap = "One quarter at a time, the data were removed from the model building, and the location (island) of the removed penguins was predicted.  The logistic regression model was fit independently of the removed penguins.", warning=FALSE, fig.width=20}
...
logCV <- data.frame(predicted = glm(island ~ bill_length_mm + bill_depth_mm + flipper_length_mm + sex, data = penguins2[-penguinfolds2$Fold1,], family = "binomial") %>% 
             predict(newdata = penguins2[penguinfolds2$Fold1,c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "sex")], type="response") ,
           obs = penguins2[penguinfolds2$Fold1,]$island,
           fold = rep("1st quarter", length(penguinfolds2$Fold1))) %>%
  rbind(data.frame(predicted = glm(island ~ bill_length_mm + bill_depth_mm + flipper_length_mm + sex, data = penguins2[-penguinfolds2$Fold2,], family = "binomial") %>% 
             predict(newdata = penguins2[penguinfolds2$Fold2,c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "sex")], type="response") ,
           obs = penguins2[penguinfolds2$Fold2,]$island,
           fold = rep("2nd quarter", length(penguinfolds2$Fold2))) ) %>%
  rbind(data.frame(predicted = glm(island ~ bill_length_mm + bill_depth_mm + flipper_length_mm + sex, data = penguins2[-penguinfolds2$Fold3,], family = "binomial") %>% 
             predict(newdata = penguins2[penguinfolds2$Fold3,c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "sex")], type="response") ,
           obs = penguins2[penguinfolds2$Fold3,]$island,
           fold = rep("3rd quarter", length(penguinfolds2$Fold3))) ) %>%
  rbind(data.frame(predicted = glm(island ~ bill_length_mm + bill_depth_mm + flipper_length_mm + sex, data = penguins2[-penguinfolds2$Fold4,], family = "binomial") %>% 
             predict(newdata = penguins2[penguinfolds2$Fold4,c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "sex")], type="response") ,
           obs = penguins2[penguinfolds2$Fold4,]$island,
           fold = rep("4th quarter", length(penguinfolds2$Fold4))) ) %>%
  mutate(predIsland = ifelse(predicted <= 0.5, "Biscoe", "Dream")) 
