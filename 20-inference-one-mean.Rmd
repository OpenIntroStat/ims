...
::: {.workedexample data-latex=""}} Using Figure \@ref(fig:carsbsmean), find the 90% and 95% bootstrap percentile confidence intervals for the true average price of a car from Awesome Auto.
...
::: {.workedexample data-latex=""}} Explain how the standard error (SE) of the bootstrapped means is calculated and what it is measuring.
...
```{r include=FALSE}
terms_chp_20 <- c(terms_chp_20, "SE single mean", "SD single mean")
```
...
::: {.workedexample data-latex=""}} Compare and contrast the two different 95% confidence intervals for $\mu$ created by finding the percentiles of the bootstrapped means and created by finding the SE of the bootstrapped means.
...
```{r include=FALSE}
terms_chp_20 <- c(terms_chp_20, "point estimate")
```

::: {.workedexample data-latex=""}} Describe the bootstrap distribution for the standard deviation shown in Figure \@ref(fig:carsbssd).
...
## Mathematical model for $\mu$ {#one-mean-math}
...
```{r include=FALSE}
terms_chp_20 <- c(terms_chp_20, "t-distribution")
```
...
```{r include=FALSE}
terms_chp_20 <- c(terms_chp_20, "Central Limit Theorem")
```
...
::: {.workedexample data-latex=""}} Consider the four plots provided in Figure \@ref(fig:outliersandsscondition) that come from simple random samples from different populations.
...
```{r outliersandsscondition, fig.cap = "Histograms of two samples from different populations.", warning = FALSE}
set.seed(3456)
...
```{r tDistCompareToNormalDist, fig.cap="Comparison of a $t$-distribution and a normal distribution."}
df <- tibble(
...
```{r include=FALSE}
terms_chp_20 <- c(terms_chp_20, "degrees of freedom")
```
...
::: {.workedexample data-latex=""}} What proportion of the $t$-distribution with 18 degrees of freedom falls below -2.10?
...
```{r tDistDF18LeftTail2Point10, fig.cap="The $t$-distribution with 18 degrees of freedom. The area below -2.10 has been shaded."}
...
::: {.workedexample data-latex=""}} A $t$-distribution with 20 degrees of freedom is shown in the top panel of Figure \@ref(fig:tDistDF20RightTail1Point65).
...
```{r tDistDF20RightTail1Point65, fig.cap="Top: The $t$-distribution with 20 degrees of freedom, with the area above 1.65 shaded. Bottom: The $t$-distribution with 2 degrees of freedom, with the area further than 3 units from 0 shaded."}
...
::: {.workedexample data-latex=""}} A $t$-distribution with 2 degrees of freedom is shown in the bottom panel of Figure \@ref(fig:tDistDF20RightTail1Point65).
...
::: {.workedexample data-latex=""}} Are the independence and normality conditions satisfied for this data set?
...
::: {.workedexample data-latex=""}} Using the summary statistics in Table \@ref(tab:summaryStatsOfHgInMuscleOfRissosDolphins), compute the standard error for the average mercury content in the $n = 19$ dolphins.
...
::: {.workedexample data-latex=""}} When $n = 19,$ what is the appropriate degrees of freedom?
...
::: {.workedexample data-latex=""}} Compute and interpret the 95% confidence interval for the average mercury content in Risso's dolphins.
...
::: {.workedexample data-latex=""}} Estimate the standard error of $\bar{x} = 0.287$ ppm using the data summaries in the previous Guided Practice.
...
```{r include=FALSE}
terms_chp_20 <- c(terms_chp_20, "T score single mean")
```

::: {.workedexample data-latex=""}} With both the independence and normality conditions satisfied, we can proceed with a hypothesis test using the $t$-distribution.
...
```{r echo = TRUE}
# using pt() to find the tail area
1 - pt(3.32, df = 99)
```
...
::: {.exercises}
```{r exercises-01, child = "exercises/20-ex-inference-one-mean.Rmd"}
```
:::
