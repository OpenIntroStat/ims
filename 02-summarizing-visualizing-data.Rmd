...
This is different from previous scatterplots we have seen, which show relationships that do not show much, if any, curvature in the trend.
:::
...
And $x_2$ Can you infer a general meaning to what $n_i$ might represent?[^summarizing-visualizing-data-3]
:::
...
The sample mean, `r round(loan50_interest_rate_mean, 2)`, provides a rough estimate of $\mu_x$.
While it is not perfect, this is our single best guess **point estimate**\index{point estimate} of the average interest rate of all the loans in the population under study.
...
To do so, we might first think to take the mean of the per capita incomes across the 3,142 counties in the \data{county} data set.
...
::: {.guidedpractice}
Besides the mean (since it was labeled), what can you see in the dot plot in Figure \@ref(fig:loan-int-rate-dotplot) that you cannot see in the histogram in Figure \@ref(fig:loan50IntRateHist)?[^summarizing-visualizing-data-6]
:::
...
(b) Is the standard deviation or IQR more affected by extreme observations?[^summarizing-visualizing-data-12]
:::

[^summarizing-visualizing-data-12]: (a) Mean is affected more.
...
::: {.workedexample}
Consider the histogram of county populations shown in Figure \@ref(fig:county-pop-transform), which shows extreme skew\index{skew!extreme}.
What is not so useful about this plot?
...
However, if we apply a log$_{10}$ transformation to the population variable, as shown in Figure \@ref(fig:county-pop-change-transform), a positive association between the variables is revealed.
...
Table \@ref(tab:countyIncomeSplitByPopGainTable) shows a sample 5 observations from each group.
...
[^summarizing-visualizing-data-17]: Answers may vary a little.
    ...
    The box plots indicate there are many observations far above the median in each group, though we should anticipate that many observations will fall beyond the whiskers when examining any data set that contain more than a couple hundred data points.
...
[^summarizing-visualizing-data-18]: Answers will vary.
    The side-by-side box plots are especially useful for comparing centers and spreads, while the hollow histograms are more useful for seeing distribution shape, skew, and potential anomalies.

Another useful visualisation for comparing numerical data across groups is **ridge plots**, which are density plots for various groups drawn on the same scale in a single plotting window.
Figure \@ref(fig:countyIncomeSplitByPopGainRidge) display ridge plots for the distribution of median household income in counties, split by whether there was a population gain or not.
...
::: {.guidedpractice}
What components of the ridge plots in Figure \@ref(fig:countyIncomeSplitByPopGain) do you find most useful compared to those in Figure \@ref(fig:countyIncomeSplitByPopGain)?[^summarizing-visualizing-data-19]
:::

[^summarizing-visualizing-data-19]: The ridge plots give us a better sense of the shape, and especially modality, of the data.
...
[^summarizing-visualizing-data-20]: The ridge plots give us a better sense of the shape, and especially modality, of the data.
...
-   $H_A$: **Alternative model.** The variables are *not* independent.
    The difference in infection rates of 64.3% was not due to chance, and vaccine affected the rate of infection.
...
While in this first simulation, we physically dealt out notecards to represent the patients, it is more efficient to perform this simulation using a computer.
...
We should be cautious not to misinterpret such anecdotal evidence.] So in this case, we reject the independence model in favor of the alternative.
