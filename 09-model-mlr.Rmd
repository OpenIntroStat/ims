...
tidy(m_bankruptcy) %>%
  ...
  kbl(booktabs = TRUE, 
      caption = caption_helper("Summary of a linear model for predicting `interest_rate` based on whether the borrower has a bankruptcy in their record. Degrees of freedom for this model is 9998."), digits = 4) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
...
tidy(m_verified_income) %>%
  mutate(p.value = "<0.0001") %>%
  kbl(booktabs = TRUE, 
      caption = caption_helper("Summary of a linear model for predicting `interest_rate` based on whether the borrower’s income source and amount has been verified. This predictor has three levels, which results in 2 rows in the regression output.")) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = c("striped", "hold_position"), full_width = FALSE)
```
