...
loans %>%
  slice_head(n = 6) %>%
  kbl(caption = "First six rows from the `loans_full_schema` data set.") %>%
...
loans_var_def %>%
  ...
  kbl(escape = FALSE, caption = "Variables and their descriptions for the `loans` data set.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE)
```
...
tidy(m_full_minus_issue_month) %>%
  ...
  kbl(caption = "The fit for the regression model after dropping the `issue_month` variable.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE) %>%
  pack_rows("", 9, 10) %>%
  add_indent(9:10) %>%
...
::: {.exercises}
```{r exercises-09, child = "exercises/09-ex-model-mlr.Rmd"}
```
:::
