...
m_bankruptcy %>%
  ...
  kbl(linesep = "", booktabs = TRUE, 
      caption = caption_helper("Summary of a linear model for predicting `interest_rate` based on whether the borrower has a bankruptcy in their record. Degrees of freedom for this model is 9998."), 
...
m_full %>%
  ...
  kbl(linesep = "", booktabs = TRUE, 
      caption = "Output for the regression model, where interest rate is the outcome and the variables listed are the predictors. Degrees of freedom for this model is 9990.", 
      digits = 2, align = "lrrrr") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = c("striped", "hold_position")) %>%
  column_spec(1, width = "17em", monospace = TRUE) %>%
...
We say the two predictor variables are colinear (pronounced as *co-linear*) when they are correlated, and this **collinearity** complicates model estimation.
...
```{r include=FALSE}
terms_chp_9 <- c(terms_chp_9, "degrees of freedom")
```
...
m_full_minus_issue_month_w_rsq <- m_full_minus_issue_month %>%
  tidy() %>%
  mutate(p.value = ifelse(p.value < 0.001, "<0.0001", round(p.value, 4))) %>%
  add_row(term = glue("Adjusted R-squared = {m_full_minus_issue_month_r_sq_adj}")) %>%
...
The model including `verified_income` has the larges increase in adjusted $R^2$ (0.24183) from the baseline (0.20046), so we add `verified_income` to the model as a predictor as well.
...
Additionally, you should know that backward elimination and forward selection sometimes arrive at different final models.
This is because the decision for whether to include a variable or not depends on what other variables are already in the model.
With forward selection, you start with a model that includes no variables and add variables from there.
In backward selection, you start with a model that includes all of the variables and remove variables from there.
A variable's "effect" depends on what other variables are in the model. 
This is especially important if the variables are correlated with each other.
Backward selection models will naturally discard the variable(s) that explain similar aspects in the response.
Whereas forward selection models will only allow for a new variable to be added, if it adds new information.
For this reason, forward selection is also sensitive to the order that variables are added to the model.
One variable could be included in the model if it was added first, but not included if it was added fifth. 
