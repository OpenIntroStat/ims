...
We will consider data about loans from the peer-to-peer lender, Lending Club, which is a dataset we first encountered in Chapters \@ref(data-hello).
...
::: {.data data-latex=""}
The data can be found in the [**openintro**](http://openintrostat.github.io/openintro) R package: [`loans_full_schema`](http://openintrostat.github.io/openintro/reference/loans_full_schema.html).
...
loans %>%
  slice_head(n = 6) %>%
  kbl(booktabs = TRUE, caption = caption_helper("First six rows of the `loans` dataset.")) %>%
...
loans_var_def %>%
  ...
  kbl(booktabs = TRUE, caption = caption_helper("Variables and their descriptions for the `loans` dataset."), 
...
$$\widehat{\texttt{interest\_rate}} = 12.33 + 0.74 \times \texttt{bankruptcy}$$
...
tidy(m_bankruptcy) %>%
  ...
  kbl(booktabs = TRUE, caption = "Summary of a linear model for predicting interest rate based on whether the borrower has a bankruptcy in their record. Degrees of freedom for this model is 9998.", digits = 4) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
...
tidy(m_verified_income) %>%
  mutate(p.value = "<0.0001") %>%
  kbl(booktabs = TRUE, caption = "Summary of a linear model for predicting interest rate based on whether the borrower’s income source and amount has been verified. This predictor has three levels, which results in 2 rows in the regression output.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE)
```
...
$$
\begin{aligned}
\widehat{\texttt{interest\_rate}} = 11.10 &+ 1.42 \times \texttt{verified\_income}_{\texttt{Source Verified}} \\
...
For example, $\texttt{verified\_income}_{\texttt{Source Verified}}$ would take a value of 1 if was for a loan, and it would take a value of 0 otherwise.
Likewise, $\texttt{verified\_income}_{\texttt{Verified}}$ would take a value of 1 if took a value of `verified` and 0 if it took any other value.
:::
...
::: {.guidedpractice data-latex=""}
Interpret the coefficients in the model.[^model-mlr-2]
:::
...
```{r include=FALSE}
terms_chp_9 <- c(terms_chp_9, "multiple regression")
```
...
$$\begin{aligned}
\widehat{\texttt{interest\_rate}} = b_0 &+ b_1 \times \texttt{verified\_income}_{\texttt{Source Verified}} \\
...
tidy(m_full) %>%
  ...
  kbl(booktabs = TRUE, caption = "Output for the regression model, where interest rate is the outcome and the variables listed are the predictors. Degrees of freedom for this model is 9990.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE)
```
...
$$
\begin{aligned}
\widehat{\texttt{interest\_rate}} = 1.925 &+ 0.975 \times \texttt{verified\_income}_{\texttt{Source Verified}} \\
&+ 2.537 \times \texttt{verified\_income}_{\texttt{Verified}} \\
&+ 0.021 \times \texttt{debt\_to\_income} \\
&+ 4.896 \times \texttt{credit\_util} \\
&+ 0.386 \times \texttt{bankruptcy} \\
&+ 0.154 \times \texttt{term} \\
&+ 0.028 \times \texttt{issue\_month}_{\texttt{Jan-2018}} \\
&- 0.040 \times \texttt{issue\_month}_{\texttt{Mar-2018}} \\
&+ 0.228 \times \texttt{credit\_checks}
\end{aligned}
$$
...
::: {.guidedpractice data-latex=""}
Compute the residual of the first observation in Table \@ref(tab:loans-data-matrix) on page using the full model.[^model-mlr-4]
:::

[^model-mlr-4]: To compute the residual, we first need the predicted value, which we compute by plugging values into the equation from earlier.
    For example, $\texttt{verified\_income}_{\texttt{Source Verified}}$ takes a value of 0, $\texttt{verified\_income}_{\texttt{Verified}}$ takes a value of 1 (since the borrower's income source and amount were verified), was 18.01, and so on.
    This leads to a prediction of $\widehat{\texttt{interest\_rate}}_1 = 18.09$.
...
For instance, when we modeled the relationship of the outcome `interest_rate` and predictor `bankruptcy` using linear regression, we were unable to control for other variables like whether the borrower had her income verified, the borrower's debt-to-income ratio, and other variables.
...
```{r include=FALSE}
terms_chp_9 <- c(terms_chp_9, "collinearity")
```
...
Is this reasonable?
Is there any value gained by making this interpretation?[^model-mlr-5]
:::

[^model-mlr-5]: Many of the variables do take a value 0 for at least one data point, and for those variables, it is reasonable.
...
Calculate $R^2$ for this model.[^model-mlr-6]
:::

[^model-mlr-6]: $R^2 = 1 - \frac{18.53}{25.01} = 0.2591$.
...
```{r include=FALSE}
terms_chp_9 <- c(terms_chp_9, "adjusted R-squared")
```
...
::: {.guidedpractice data-latex=""}
There were $n=10000$ auctions in the dataset and $k=9$ predictor variables in the model.
Use $n$, $k$, and the variances from the earlier Guided Practice to calculate $R_{adj}^2$ for the interest rate model.[^model-mlr-7]
:::

[^model-mlr-7]: $R_{adj}^2 = 1 - \frac{18.53}{25.01}\times \frac{10000-1}{1000-9-1} = 0.2584$.
...
What would happen to the adjusted $R^2$?[^model-mlr-8]
:::

[^model-mlr-8]: The unadjusted $R^2$ would stay the same and the adjusted $R^2$ would go down.
...
```{r include=FALSE}
terms_chp_9 <- c(terms_chp_9, "parsimonious")
```
...
```{r include=FALSE}
terms_chp_9 <- c(terms_chp_9, "full model")
```
...
```{r include=FALSE}
terms_chp_9 <- c(terms_chp_9, "stepwise selection")
```
...
```{r include=FALSE}
terms_chp_9 <- c(terms_chp_9, "backward elimination", "forward selection")
```
...
tidy(m_full) %>%
  ...
  kbl(booktabs = TRUE, caption = "The fit for the full regression model, including the adjusted $R^2$.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE) %>%
  pack_rows("", 11, 12) %>%
  add_indent(11:12) %>%
...
tidy(m_full_minus_issue_month) %>%
  ...
  kbl(booktabs = TRUE, caption = "The fit for the regression model after dropping issue month.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE) %>%
  pack_rows("", 9, 10) %>%
  add_indent(9:10) %>%
...
$$
\begin{aligned}
\widehat{\texttt{interest\_rate}} = 1.921 &+ 0.974 \times \texttt{verified\_income}_\texttt{Source only} \\
&+ 2.535 \times \texttt{verified\_income}_\texttt{Verified} \\
&+ 0.021 \times \texttt{debt\_to\_income} \\
&+ 4.896 \times \texttt{credit\_util} \\
&+ 0.387 \times \texttt{bankruptcy} \\
&+ 0.154 \times \texttt{term} \\
&+ 0.228 \times \texttt{credit\_check}
\end{aligned}
$$
:::
...
## Exercises {#chp9-exercises}

::: {.exercises}
```{r exercises-09, child = "exercises/09-ex-model-mlr.Rmd", eval = FALSE}
```
:::
