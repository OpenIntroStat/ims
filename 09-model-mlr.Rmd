...
loans %>%
  slice_head(n = 6) %>%
  kbl(caption = caption_helper("First six rows of the `loans` data set.")) %>%
...
loans_var_def %>%
  ...
  kbl(caption = caption_helper("Variables and their descriptions for the `loans` data set."), 
...
tidy(m_bankruptcy) %>%
  ...
  kbl(caption = "Summary of a linear model for predicting interest rate based on whether the borrower has a bankruptcy in their record. Degrees of freedom for this model is 9998.", digits = 4) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
...
tidy(m_verified_income) %>%
  mutate(p.value = "<0.0001") %>%
  kbl(caption = "Summary of a linear model for predicting interest rate based on whether the borrower’s income source and amount has been verified. This predictor has three levels, which results in 2 rows in the regression output.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE)
```
...
tidy(m_full) %>%
  ...
  kbl(caption = "Output for the regression model, where interest rate is the outcome and the variables listed are the predictors. Degrees of freedom for this model is 9990.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE)
```
...
tidy(m_full) %>%
  ...
  kbl(caption = "The fit for the full regression model, including the adjusted $R^2$.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE) %>%
  pack_rows("", 11, 12) %>%
  add_indent(11:12) %>%
...
tidy(m_full_minus_issue_month) %>%
  ...
  kbl(caption = "The fit for the regression model after dropping issue month.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE) %>%
  pack_rows("", 9, 10) %>%
  add_indent(9:10) %>%
