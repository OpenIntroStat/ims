...
tidy(m_bankruptcy) %>%
  ...
  kbl(booktabs = TRUE, 
      caption = caption_helper("Summary of a linear model for predicting `interest_rate` based on whether the borrower has a bankruptcy in their record. Degrees of freedom for this model is 9998."), digits = 2, align = "lrrrr") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
...
tidy(m_full) %>%
  ...
  kbl(booktabs = TRUE, caption = "Output for the regression model, where interest rate is the outcome and the variables listed are the predictors. Degrees of freedom for this model is 9990.", digits = 2, align = "lrrrr") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = c("striped", "hold_position"), full_width = FALSE) %>%
  column_spec(1, width = "15em") %>%
  column_spec(2:5, width = "6em")
...
[^model-mlr-6]: Many of the variables do take a value 0 for at least one data point, and for those variables, it is reasonable.
    ...
    If `term` is set to zero, then the loan must be paid back immediately; the borrower must give the money back as soon as she receives it, which means it is not a real loan.
...
The reasoning behind the adjusted $R^2$ lies in the **degrees of freedom** associated with each variance, which is equal to $n - k - 1$ for the multiple regression context.
...
tidy(m_full) %>%
  ...
  kbl(booktabs = TRUE, caption = "The fit for the full regression model, including the adjusted $R^2$.", digits = 2, align = "lrrrr") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = c("striped", "hold_position"), full_width = FALSE) %>%
  pack_rows("", 11, 12) %>%
  add_indent(11:12) %>%
