...
loans %>%
  slice_head(n = 6) %>%
  kbl(caption = "First six rows from the `loans` data set.",
      booktabs = TRUE) %>%
...
loans_var_def %>%
  ...
  kbl(caption = "Variables and their descriptions for the `loans` data set.", 
...
tidy(m_full_minus_issue_month) %>%
  ...
  kbl(caption = "The fit for the regression model after dropping issue month.", booktabs = TRUE) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE) %>%
  pack_rows("", 9, 10) %>%
  add_indent(9:10) %>%
