...
lm(interest_rate ~ debt_to_income + term + credit_checks, data = loans) %>%
  ...
  kbl(
    ...
    caption = caption_helper("Summary of a linear model for predicting interest rate based on `debt_to_income`, `term`, and `credit_checks`. Each of the variables has its own coefficient estimate as well as a resulting p-value."),
...
```{r}
#| label: peng-mass1
#| fig.cap: One quarter at a time, the data were removed from the model building, and
