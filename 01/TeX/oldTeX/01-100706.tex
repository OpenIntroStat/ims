...
When a scientist asks a question, she seeks an answer based on empirical evidence: observations. When observations are collected and recorded, they are called \term{data}. Statistics is the study of how best to collect, analyze, and draw conclusions from data.
...
\end{enumerate}
Statistics as a subject focuses on making steps (2)-(4) objective, rigorous, and efficient. That is, statistics has three primary components: How best can we collect the data? How should it be analyzed? And what can we infer from the analysis?

The topics scientists investigate are very diverse, as are the questions they ask. However, many of these investigations can be addressed with a small number of data collection techniques, analytic tools, and a foundation understanding of the principles of statistical inference. This book emphasizes the basic principles and most commonly used techniques in each of the three components of statistics.

In this chapter, we introduce the basic principles of each branch and learn some tools along the way. Along the way we encounter applications from other fields, some of which are not typically associated with scientific study but nonetheless can benefit from statistical study.

\section{An example: treating heart attack patients with a new drug}

%Science is the study of the natural world through observation and experiment. Statistics is a part of this process of understanding.

% The science of statistics is really a process or tool used to rigorously and objectively questions in other fields.

%Statistics is the science of collecting and analyzing data to make conclusions. Generally statistics is not the subject of interest, only the tool used to look for an the answer.

In this first section, we will consider an experiment used to evaluate whether a drug, sulphinpyrazone, reduces the number of deaths in heart attack patients. In this investigation and in others, it might be helpful to keep track of each step of the process of investigation in each application. For instance, in this study we might start with the principle question we hope to answer:
\begin{quote}
Does administering sulphinpyrazone to a heart attack patient reduce the risk of death?
\end{quote}
To help them answer this question, the researchers who asked this question collected data on 1475 volunteer heart attack patients. Now we are in the second step: data collection. Each patient was assigned to one of two groups:
\begin{itemize}
...
\end{itemize}
In the end, there were 742 patients in the treatment group and 733 patients in the control group. The patients were not told which group they were in, and the reason for this secrecy is that %patients who know they are not being treated may seek out alternative treatment. Additionally, 
...
After three weeks in the study \Comment{[check time]}, each patient was either still alive or had died. In this study, we call this the patient's \var{outcome}. So far, there are two characteristics about each patient of relevance to the study: the group the patient was in and the outcome of the patient. If we liked, this information could be organized into a table. One common such organization method is shown in Table~\ref{sulphinpyrazoneDF}. Here each patient is represented by a row, and each column pertains to a particular type of characteristic: \var{group} and \var{outcome}.
\begin{table}
\begin{center}
\begin{tabular}{l cc}
\hline
...
It is difficult to analyze data by looking at each patient individually to answer the original research question. For this reason, an analysis of the data can be performed, and here we move into the third step of the investigative process. A first step in this analysis might be to summarize the data into a form that that summarizes all the information in Table~\ref{sulphinpyrazoneDF}. For instance, we could summarize the data in a new table like the one shown in Table~\ref{sulphinpyrazoneResultsInIntro}.
\begin{table}
\begin{center}
...
We can also look at summary statistics. \term{Summary statistics} are used to condense a large amount of data into a single number\footnote{Formally, a summary statistic is a number computed from the data. Some summary statistics are more useful than others.}. For instance, the primary resulta of the study could be placed in two numbers: the proportion of people who died in each group.
\begin{itemize}
\item[] Proportion who died in the treatment group: $41/733 = 0.056$.
\item[] Proportion who died in the control group: $60/742 = 0.081$.
\end{itemize}
These proportions are summary statistics and are useful in evaluating whether the drug worked. There is an observed difference in the outcomes: 2.5\% fewer patients died in the treatment group.

Here we now (albeit prematurely) move into the fourth stage of the investigative process: drawing a conclusion. We might ask, Does this 2.5\% difference imply that the drug worked? Even if the drug didn't work, typically we would not get the exact same death rate in each group. Maybe the difference of 2.5\% was just due to chance, or maybe the 2.5\% is actually the result of the drug working. Regrettably, our analysis does not help us answer whether what we are seeing is real or is natural fluctuation and so we will have to wait until a later section before we can make a more formal conclusion. % In a formal analysis, our goal is to analyze the data so that in the conclusion step, we can assess whether chance had anything to do with it.

We perform a more formal analysis in Section~\ref{caseStudyOfSulphinpyrazone} so that we can come to a conclusion. However, before this analysis will not make much sense before we discuss additional principles, ideas, and tools in Sections~\ref{dataBasics}-\ref{}.
