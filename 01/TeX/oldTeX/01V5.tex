...
\begin{tabular}{cc ccc c}
 ...
 & \var{spam} & \var{num\_char} & \var{line\_breaks} & \var{format} & \var{number} \\ 
  \hline
1 & no & 25,173 & 635 & html & small \\ 
  2 & no & 6,425 & 143 & html & big \\ 
  3 & no & 10,271 & 154 & html & small \\ 
$\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ \\
  50 & yes & 13,140 & 188 & text & none \\ 
   \hline
\end{tabular}
...
\begin{eqnarray}
\bar{x} = \frac{25.2 + 6.4 + \cdots + 13.1}{50} = 13.6
%\bar{x} = \frac{15.9 + 33.9 + \cdots + 26.7}{54} = 19.99259
\label{sampleMeanEquation}
\end{eqnarray}
The sample mean is often labeled $\bar{x}$\marginpar[\raggedright$\bar{x}$\\\footnotesize sample\\ mean]{\raggedright$\bar{x}$\\\footnotesize sample\\ mean}. The letter $x$ is being used as a generic placeholder for the variable of interest, \var{num\_char}, and the bar says it is the average number of characters in the 50 emails was 13,600. It is useful to think of the mean as the balancing point of the distribution. The sample mean is shown as a blue triangle in Figure~\ref{emailCharactersDotPlot}.
...
\begin{exercise}
Examine equations~(\ref{sampleMeanEquation}) and~(\ref{meanEquation}) above. What does $x_1$ correspond to? And $x_2$? Can you infer a general meaning to what $x_i$ might represent?\footnote{$x_1$ corresponds to the number of characters in the first email in the sample (25.2, in thousands), $x_2$ to the number of characters in the second email (6.4, in thousands), and $x_i$ corresponds to the number of characters in the $i^{th}$ email in the data set.}
\end{exercise}
...
Characters & \\
(in thousands) & \raisebox{1.5ex}[0pt]{0-5} & \raisebox{1.5ex}[0pt]{5-10} & \raisebox{1.5ex}[0pt]{10-15} & \raisebox{1.5ex}[0pt]{15-20} & \raisebox{1.5ex}[0pt]{20-25} & \raisebox{1.5ex}[0pt]{25-30} & \raisebox{1.5ex}[0pt]{$\cdots$} & \raisebox{1.5ex}[0pt]{50-55} & \raisebox{1.5ex}[0pt]{65-70} \\
  \grayline
%Count & 11 &  11 &  10 &   7 &   6 &   3 & $\cdots$ & 0 & 1 \\
Count & 19 & 8 & 5 & 6 & 2 & 4 & $\cdots$ & 0 & 1 \\
  \hline
\end{tabular}
...
\begin{align*}
x_1^{}-\bar{x} &= 25.2 - 13.6 = 11.6 \hspace{5mm}\text{ } \\
x_2^{}-\bar{x} &= 6.4 - 13.6 = -7.2 \\
x_2^{}-\bar{x} &= 10.3 - 13.6 = -3.3 \\
			&\ \vdots \\
x_{50}^{}-\bar{x} &= 13.1 - 13.6 = -0.5
\end{align*}
% library(openintro); d <- email50$num_char; round(mean(d)); d[c(1,2,3,50)]; d[c(1,2,3,50)] - round(mean(d)); (d[c(1,2,3,50)] - round(mean(d)))^2; sum((d - round(mean(d)))^2)/49; sqrt(sum((d - round(mean(d)))^2)/49); var(d); sd(d)
...
\begin{align*}
s_{}^2 &= \frac{11.6_{}^2 + (-7.2)_{}^2 + (-3.3)_{}^2 + \cdots + (-0.5)_{}^2}{50-1} \\
	&= \frac{134.56 + 51.84 + 10.89 + \cdots + 0.25}{49} \\
	&= 211.81
\end{align*}
We divide by $n-1$, rather than dividing by $n$, when computing the variance; you need not worry about this mathematical nuance for the material in this textbook. Notice that squaring the deviations does two things: (i) it makes large values much larger, seen by comparing $11.6^2$, $(-7.2)^2$, $(-3.3)^2$, and $(-0.5)^2$, and (ii) it gets rid of any negative signs.
...
}\index{s@$s$}The standard deviation of the number of characters in an email is about 14,600. A subscript of $_x$ may be added to the variance and standard deviation -- that is, $s_x^2$ and $s_x^{}$ -- as a reminder that these are the variance and standard deviation of the observations represented by $x_1^{}$, $x_2^{}$, ..., $x_n^{}$. These may be omitted when it is clear which data the standard deviation is referencing.
...
\caption{The 70\% within 1 standard deviation and 95\% within 2 are not strict guidelines. In fact, they usually break down for strongly skewed data. In the \var{num\_char} data, 44 of 50 emails (88\%) are within 1 standard deviation of the mean, 13,600, and 47 of the 50 emails (94\%) are within 2 standard deviations.}
...
The distribution of email character counts is unimodal and extremely skewed \index{skew!extreme skew} to the high end. Many of the counts fall near the mean at 13,600, and most fall within one standard deviation (14,600) of the mean. There is one exceptionally long email with more than 60,000 characters.
\end{example}
...
\begin{figure}[h]
   ...
   \caption{A vertical dot plot next to a labeled box plot for the number of characters in 50 emails. The median (9,011), splits the data into the bottom 50\% and the top 50\%, marked in the dot plot by horizontal dashes and open circles, respectively.}
...
\item Identifying data collection or entry errors. For instance, we re-examined the email purported to have 66,924 characters to ensure this value was accurate.
...
\begin{exercise}
The observation 66,924, a suspected outlier, was found to be an accurate observation. What would such an observation suggest about the nature of character counts in emails?\footnote{That occasionally there may be extremely long emails.} % (In Section~\ref{}, we will see that those emails with the most characters actually contain much styling in the form of HTML, which contributes to the large number of characters.)}.
\end{exercise}
...
estimate the following values for \var{num\_char} in the \data{email50} data set: (a) $Q_1$, (b) $Q_3$, and (c) IQR.\footnote{These visual estimates will vary a little from one person to another: $Q_1=$ 3,000, $Q_3=$ 19,000, $\text{IQR}=Q_3 - Q_1 = $ 16,000. (The true values: $Q_1=$ 2,433, $Q_3=$ 19,181, $\text{IQR} = $ 16,748.)}
\end{exercise}

\subsection{Robust statistics}

How are the \indexthis{sample statistics}{sample statistic} of the \data{num\_char} data set affected by the outlier observation, 66,924? What would have happened if this email wasn't observed? What would happen to these \indexthis{summary statistics}{summary statistic} if the outlier at 66,924 had been even larger, say 200,000? These scenarios are plotted alongside the original data in Figure~\ref{email50NumCharDotPlotRobustEx}, and sample statistics are computed under each scenario in Table~\ref{robustOrNotTable}.
\begin{figure}[ht]
\centering
...
\raisebox{1.5ex}[0pt]{\var{spam}} 
	& not spam &  457 & 2672 & 482 & 3611 \\ 
  \cline{2-6}
& total & 552 & 2774 & 516 & 3842 \\
  \cline{2-6}
\end{tabular}
...
A bar plot is a common way to display a single categorical variable. The left panel of Figure~\ref{emailNumberBarPlot} shows a \term{bar plot} for the \var{number} variable. In the right panel, the counts are converted into proportions (e.g. $552/3842=0.144$ for \resp{none}), showing the proportion of observations that are in each level (i.e. category).
\begin{figure}[bht]
   \centering
...
\begin{tabular}{l rrr r}
 ...
 & none & small & big & total \\ 
  \hline
spam &  $95/231 = 0.411$ & $102/231 = 0.442$ & $34/231 = 0.147$ & 1.000 \\ 
not spam &  $457/3611 = 0.127$ & $2672/3611 = 0.740$ & $482/3611 = 0.133$ & 1.000 \\ 
   \hline
total & $552/3842 = 0.144$ & $2774/3842 = 0.722$ & $516/3842 = 0.134$ & 1.000 \\
  \hline
\end{tabular}
...
\begin{tabular}{l rrr r}
 ...
 & none & small & big & total \\ 
  \hline
spam &  $95/552 = 0.172$ & $102/2774 = 0.037$ & $34/516 = 0.066$ & $231/3842 = 0.060$ \\ 
not spam &  $457/552 = 0.828$ & $2672/2774 = 0.963$ & $482/516 = 0.934$ & $3611/3842 = 0.940$ \\ 
   \hline
...
\begin{exercise}
What does 0.442 represent in Table~\ref{rowPropSpamNumber}? What does 0.037 represent in Table~\ref{colPropSpamNumber}?\footnote{0.442 represents the proportion of spam emails that had a small number. 0.037 represents the fraction of emails with small numbers that are spam.}
\end{exercise}

\begin{exercise}
What does 0.133 represent in Table~\ref{rowPropSpamNumber}? What does 0.934 represent in the Table~\ref{colPropSpamNumber}?\footnote{0.133 represents the fraction of non-spam email that had a big number. 0.934 represents the fraction of emails with big numbers that are non-spam emails.}
\end{exercise}

\begin{example}{Software engineers use statistics to filter spam from incoming email messages. By noting specific characteristics of an email, an engineer may be able to classify some emails as spam or not spam with high accuracy. One of those characteristics is whether the email contains no numbers, small numbers, or big numbers. Another characteristic is whether or not an email has any HTML content. A contingency table for the \var{spam} and \var{format} variables from the \data{email} data set are shown in Table~\ref{emailSpamHTMLTableTotals}. Recall that an email that is HTML simply has the capacity for special formatting, e.g. bold text. (That's all you need to know about HTML.) In this instance, which would be more helpful to a statistician hoping to classify email as spam or regular email: row or column proportions?} \label{weighingRowColumnProportions}
...
If we generate the column proportions, we can see that a higher fraction of plain text emails are spam ($120/1110 = 10.8\%$) than compared to HTML emails ($111/2621 = 4.2\%$). This information on its own is insufficient to classify an email as spam or not spam, as nearly 90\% of plain text emails are not spam. Yet, when we carefully combine this information with many other characteristics, such as \var{number} and other variables, we stand a reasonable chance of being able to classify some email as spam or not spam. This is a topic we will return to in Chapter~\ref{multipleRegressionAndANOVA}.
\end{example}
...
\begin{tabular}{l cc r}
 ...
 & text & HTML & total \\ 
  \hline
spam & 120 & 111  & 231 \\ 
not spam & 990 & 2621  & 3611 \\ 
   \hline
total & 1110 & 2732 & 3842 \\
   \hline
\end{tabular}
