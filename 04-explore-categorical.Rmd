...
Color can be used to split histograms (see Section \@ref(histograms) for an introduction to histograms) for numerical variables by levels of a categorical variable.
...
An example is shown in Plot B of Figure \@ref(fig:countyIncomeSplitByPopGain), where there are two box plots (see Section \@ref(boxplots) for an introduction to box plots), one for each group, placed into one plotting window and drawn on the same scale.
...
Another useful visualization for comparing numerical data across groups is a **ridge plot**, which combines density plots (see Section \@ref(histograms) for an introduction to density plots) for various groups drawn on the same scale in a single plotting window.
