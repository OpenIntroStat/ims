...
```{r loan-homeownership-type-mosaic-plot, fig.cap = "The mosaic plots: one for homeownership alone and the other displaying the relationship between homeownership and application type.", fig.asp = 0.5, out.width = "100%"}
...
```{r loan-homeownership-pie-chart, fig.cap = "A pie chart and bar plot of homeownership.", out.width = "100%"}
p_pie <- loans_full_schema %>% 
...
```{r loan-grade-pie-chart, fig.cap = "A pie chart and bar plot of loan grades.", out.width = "100%"}
p_pie <- loans_full_schema %>% 
  count(grade) %>% 
  mutate(text_y = cumsum(n) - n/2) %>% 
...
Another useful technique or visualizing categorical data is a **waffle chart**.
...
p_waffle_homeownership <- loans_full_schema %>%
  count(homeownership) %>%
...
p_waffle_loan_status <- loans_full_schema %>%
  count(loan_status) %>%
...
```{r countyIncomeSplitByPopGain, fig.cap = "Histograms (Plot A) and side by-side box plots (Plot B) for median household income, where counties are split by whether there was a population gain or not."}
p_hist <- county %>%
  filter(!is.na(pop_change)) %>%
...
In Figure \@ref(fig:countyIncomeRidgeMulti), we represent median education level using color, where red (solid line) represents counties where the median education level is Bachelor's, orange (dotted line) is some college degree, and yellow (dashed line) is high school diploma.
