...
```{r loan-homeownership-pie-chart, fig.cap = "A pie chart and bar plot of homeownership."}
...
```{r loan-grade-pie-chart, fig.cap = "A pie chart and bar plot of loan grades."}
p_pie <- loans_full_schema %>% 
  count(grade) %>% 
  mutate(text_y = cumsum(n) - n/2) %>% 
...
```{r loan-waffle, fig.alt = "Plot A: Waffle chart of homeownership, with levels rent, morgage, and own. Plot B: Waffle chart of loan status, with levels current, fully paid, in grade period, and late.", out.width = "100%"}
...
p_waffle_homeownership + p_waffle_loan_status +
  plot_annotation(tag_levels = "A") & 
