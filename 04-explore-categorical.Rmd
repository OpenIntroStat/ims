...
The `loan50` data set from Chapter \@ref(data-hello) represents a sample from a larger loan data set called `loans`.
...
::: {.data data-latex=""}
The data can be found in the [openintro](http://openintrostat.github.io/openintro) package: [loans](http://openintrostat.github.io/openintro/reference/loans_full_schema.html).
:::
...
loans_full_schema %>%
  ...
  kbl(caption = "A contingency table for application type and homeownership.") %>%
...
loans_full_schema %>%
  ...
  kbl(caption = "A table summarizing the frequencies for each value of the homeownership variable: mortgage, own, and rent.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE)
```
...
loans_full_schema %>%
  ...
  kbl(caption = "A contingency table with row proportions for the application type and homeownership variables.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE) %>%
  add_header_above(c(" " = 1, "homeownership" = 3, " " = 1))
```
...
loans_full_schema %>%
  ...
  pivot_wider(names_from = homeownership, values_from = proportion) %>% 
  adorn_totals(where = "row") %>% 
  kbl(caption = "A contingency table with column proportions for the application type and homeownership variables.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = "striped", full_width = FALSE) %>%
...
(b) What does 0.802 represent in Table \@ref(tab:loan-home-app-type-column-proportions)?[^explore-categorical-1]
:::

[^explore-categorical-1]: (a) 0.451 represents the proportion of individual applicants who have a mortgage.
...
(b) What does 0.135 represent in Table \@ref(tab:loan-home-app-type-column-proportions)?[^explore-categorical-2]
:::

[^explore-categorical-2]: (a) 0.122 represents the fraction of joint borrowers who own their home.
...
::: {.data data-latex=""}
The data can be found in the [openintro](http://openintrostat.github.io/openintro) package: [email](http://openintrostat.github.io/openintro/reference/email.html).
:::
...
email %>%
  ...
  adorn_totals(where = c("row", "col")) %>%
  kbl(caption = "A contingency table for spam and format") 
```
...
county %>%
  ...
  kbl(caption = "The median household income from a random sample of 5 counties with population gain between 2010 to 2017 and another random sample of 5 counties with no population gain.")
```
