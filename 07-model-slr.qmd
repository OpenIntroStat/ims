...
Figure @fig-perfLinearModel shows two variables whose relationship can be modeled perfectly with a straight line.
...
#| fig-cap: Requests from twelve separate buyers were simultaneously placed with a trading
...
Instead, it's more common for data to appear as a *cloud of points*, such as those examples shown in Figure @fig-imperfLinearModel.
...
#| fig-cap: Three datasets where a linear model may be useful even though the data do
...
#| fig-cap: The best fitting line for these data is flat, which is not a useful way to
...
```{r}
#| label: brushtail-possum
#| fig-cap: (ref:brushtail-possum-cap)
#| out.width: 50%
...
#| fig-cap: A scatterplot showing head length against total length for 104 brushtail
#|   possums. A point representing a possum with head length 86.7 mm and total length
#|   84 cm is highlighted.
...
#| fig-cap: A reasonable linear model was fit to represent the relationship between head
...
```{r}
#| label: scattHeadLTotalL-sex-age
#| fig-cap: Relationship between total length and head length of brushtail possums, taking
#|   into consideration their sex (Plot A) or age (Plot B).
#| fig.asp: 1.236
...
```{r}
#| label: scattHeadLTotalLLine-highlighted
#| fig-cap: A reasonable linear model was fit to represent the relationship between head
#|   length and total length, with three points highlighted.
...
#| fig-cap: Residual plot for the model predicting head length from total length for
#|   brushtail possums.
...
#| fig-cap: Sample data with their best fitting lines (top row) and their corresponding
...
[^07-model-slr-3]: Formally, we can compute the correlation for observations $(x_1, y_1),$ $(x_2, y_2),$ ..., $(x_n, y_n)$ using the formula $$r = \frac{1}{n-1} \sum_{i=1}^{n} \frac{x_i-\bar{x}}{s_x}\frac{y_i-\bar{y}}{s_y}$$ where $\bar{x},$ $\bar{y},$ $s_x,$ and $s_y$ are the sample means and standard deviations for each variable.
...
#| fig-cap: Sample scatterplots and their correlations. The first row shows variables
#|   with a positive relationship, represented by the trend up and to the right. The
#|   second row shows variables with a negative trend, where a large value in one variable
#|   is associated with a lower value in the other.
#| fig.asp: 0.5
#| out.width: 100%
...
#| fig-cap: ' Sample scatterplots and their correlations. In each case, there is a strong
#|   relationship between the variables. However, because the relationship is not linear,
...
crops_country <- crops_country %>%
  ...
  filter(n_missing <= 5)  # filter out countries that do not have data for most of the crops
...
```{r}
#| label: bdims-units
#| fig-cap: Two scatterplots, both displaying the relationship between weights and heights
#|   of 507 physically healthy adults. In Plot A, the units are kilograms and centimeters.
#|   In Plot B, the units are pounds and inches. Also noted on both plots is the correlation
#|   coefficient, $r = 0.72.$
#| fig.asp: 0.5
...
```{r}
#| label: elmhurstScatterWLine
#| fig-cap: Gift aid and family income for a random sample of 50 freshman students from
#|   Elmhurst College.
...
::: {.guidedpractice data-latex=""}
Is the correlation positive or negative in Figure \@ref(fig:elmhurstScatterWLine)?[^07-model-slr-5]
:::
...
Table @tbl-summaryStatsElmhurstRegr shows the sample means for the family income and gift aid as \$101,780 and \$19,940, respectively.
...
#| fig-cap: Total auction prices for the video game Mario Kart, divided into used ($x
#|   = 0$) and new ($x = 1$) condition games. The least squares regression line is also
#|   shown.
mariokart_lt100 %>%
  filter(total_pr < 100) %>%
...
```{r}
#| label: outlier-plots-1
#| fig-cap: Three plots, each with a least squares line and corresponding residual plot.
...
```{r}
#| label: outlier-plots-2
#| fig-cap: Three plots, each with a least squares line and residual plot. All datasets
