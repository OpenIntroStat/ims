...
#| fig-cap: |
#|   Three data sets where a linear model may be useful even though the data do
...
::: {.guidedpractice data-latex=""}
No straight line is a good fit for any of the data sets represented in Figure @fig-corForNonLinearPlots.
...
#| fig-alt: From a real data set, we display scatterplots of the relationship 
...
This section considers a data set on family income and gift aid data from a random sample of fifty students in the freshman class of Elmhurst College in Illinois.
...
The data set where these data are stored is called `elmhurst`.
The first 5 rows of this data set are given in @tbl-elmhurst-data.

```{r}
#| label: tbl-elmhurst-data
#| tbl-cap: First five rows of the `elmhurst` data set.
...
::: {.workedexample data-latex=""}
Among 50 students in the `elmhurst` data set, the total variability in gift aid is $SST = 1461$.[^07-model-slr-10]
...
Note that the original data set contains some Mario Kart games being sold at prices above \$100 but for this analysis we have limited our focus to the `r nrow(mariokart_lt100)` Mario Kart games that were sold below \$100.
...
However, when the predictor variable is binary, the coefficient estimates ($b_0$ and $b_1$) are directly interpretable with respect to the data set at hand.
...
Outliers in a bivariate regression model are observations that fall far from the cloud of points.
...
#|   Each data set has at least one outlier.
