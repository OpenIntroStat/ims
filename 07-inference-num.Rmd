...
As seen in Section \@ref(two-prop-boot-ci), another method for creating bootstrap confidnece intervals directly uses a calculation of the variability of the bootstrap statistics (here, the bootstrap means).
...
Figure XXX1 shows how to shuffle the scores into two groups assuming no difference in the difficulty.  Figure \@ref(fig:randexams) shows the values of the simulated statistics $\bar{x}_{1, sim} - \bar{x}_{2, sim}$ over 1000 random simulations.
