...
Figure \@(fig:5flats) presents the details of the random sample of observations where the monthly rent of five flats has been recorded.
...
Figure \@ref(fig:flats-bs-mean) summarizes one thousand bootstrap samples in a histogram of the bootstrap sample means.
The bootstrapped average rent prices vary from $\textsterling$ 1250 to $\textsterling$ 1995 (with a small observed sample of size 5, a bootstrap resample can sometimes, although rarely, include only repeated measurements of the same observation).
...
Using Figure \@ref(fig:flats-bs-mean), find the 90% and 95% confidence intervals for the true mean monthly rental price of a three bedroom flat in Edinburgh.

---
A 90% confidence interval is given by $\textsterling$ 1429 to $\textsterling$ 1876.  The conclusion is that we are 90% confident that the true average rental price for three bedroom flats in Edinburgh lies somewhere between $\textsterling$ 1429 and $\textsterling$ 1876.


A 95% confidence interval is given by $\textsterling$ 1389.75 to $\textsterling$ 1916.  The conclusion is that we are 90% confident that the true average rental price for three bedroom flats in Edinburgh lies somewhere between $\textsterling$ 1389.75 and $\textsterling$ 1916.
```



```{r flats-bs-mean, fig.cap="The original Edinburgh data is bootstrapped 1,000 times. The histogram provides a sense for the variability of the average rent values from sample to sample.", warning=FALSE, fig.width=10}
...
It turns out that the standard deviation of the bootstrapped means from Figure \@ref(fig:flats-bs-mean) is $\textsterling 136.9$.  [Note: in R the calculation was done using the function `sd()`.]  The average of the observed prices, the best guess point estimate for $\mu$, is  $\textsterling 1648$.

Find and interpret the confidence interval for $\mu$ (the true average rental price of flats in Edinbugh) using the Bootstrap SE inverval formula.^[Using the formula for the boostrap SE interval, we find the 95% confidence interval for $\mu$ is: $$1648 \pm 2 \cdot 136.9 \rightarrow (\textsterling 1374.2, \textsterling 1921.8)$$
                                                                                                                                                      
We are 95% confident that the true average rent price for a three bedroom flat in Edinburgh is somewhere between $\textsterling 1374.2 and \textsterling 1921.8$.]
```
...
* Percentile interval: $(\textsterling 1389.75, \textsterling 1916)$
* SE interval: $(\textsterling 1374.2, \textsterling 1921.8)$
...
The point estimate of the five observations is calculated to be $s = \textsterling  340.23$.
While $s = \textsterling  340.23$ might be a good guess for $\sigma$, we prefer to have an interval 
...
```{block2, type = "example", echo = TRUE}
Describe the bootstrap distribution for the standard deviation shown in Figure \@ref(fig:flats-bs-sd).

---
  
The distribution is skewed left and centered near $\textsterling 340.23$, which is the point estimate from the original data. Most observations in this distribution lie between $\textsterling 0$ and $\textsterling 408.1$.
```

```{block2, type = "guidedpractice", echo = TRUE}
Using Figure \@ref(fig:flats-bs-sd), find *and interpret* a 90% confidence interval for the population standard deviation for three bedroom flat prices in Edinburgh.^[By looking at the percentile values in Figure \@ref(fig:flats-bs-sd), the middle 90% of the bootstrap standard deviations are given by the 5 percentile ($\\textsterling$ 153.9) and 95 percentile ($\\textsterling$ 385.6).  That is, we are 90% confident that the true standard deviation of rent prices is between $\\textsterling$ 153.9 and $\\textsterling$ 385.6.]
```

```{r flats-bs-sd, fig.cap="The original Edinburgh data is bootstrapped 1,000 times. The histogram provides a sense for the variability of the standard deviation of the rent values from sample to sample.", warning=FALSE, fig.width=10}
...
```{r outliers_and_ss_condition, fig.cap="", warning=FALSE, fig.width=10}
