...
```{r tiredataPermSort, fig.cap = "Boxplots of the tire tread data and (left) the brand of tire from which the original measurements came, (center) shuffled brand assignment, (right) sorted by randomized brand.", warning = FALSE, fig.asp = 0.33, out.width = "100%"}
...
ggplot(tires2, aes( x = `Randomized Tire Brand`, y = tread, color = `Randomized Tire Brand`)) +
  ...
  geom_text(data = permed_means, 
            aes(label = mean_label, 
                y = 0.318), parse = TRUE, show.legend = FALSE) + 
  ggtitle("Wear of tire tread (in inches) after 1,000 miles,\n broken down by different tire brands, randomly assigned (3).")
