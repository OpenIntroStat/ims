...
To compute the test  compute the standard error associated with
  ...
  $H_0$ is true,
  and the p-value is represented by the two shaded tails in Figure \@(fig:textbooksF18HTTails).
...
```{r textbooksF18HTTails, fig.cap = "Distribution of $\\bar{x}_{diff}$ under the null hypothesis of no difference.  The observed average difference of 2.98 is marked with the shaded areas more extreme than the observed difference given as the p-value.", warning = FALSE, echo = TRUE}
...
and books on Amazon.^[Conditions
