...
mariokart %>%
  ...
  labs(
    title = "1,000 slopes",
    subtitle = "Models fit to 1,000 random permutations of the data",
...
mariokart %>%
  ...
  geom_histogram(binwidth = 0.5, fill = IMSCOL["green", "full"]) +
  labs(
    title = "Slopes of models fit to 1,000 random bootstrap samples of the data",
...
```{r mariokart-cv-residuals, fig.cap = "Cross-validation predictions and errors from linear models built on two different sets of variables. Left regressed `price` on `cond_new`; right regressed `price` on `cond_new`, `stock_photo`, `duration`, and `wheels`.", out.width = "100%", fig.asp = 0.5, fig.width = 8}
set.seed(4)
