...
mariokart %>%
  ...
  kbl(
    linesep = "", booktabs = TRUE,
    caption = caption_helper("Summary of a linear model for predicting `price` based on `cond_new` which has a coefficient estimate and p-value."),
...
mariokart %>%
  ...
  kbl(
    linesep = "", booktabs = TRUE,
    caption = caption_helper("Summary of a linear model for predicting `price` based on `cond_new`, `stock_photo`, `duration`, and `wheels`. Each of the variables has its own coefficient estimate and p-value."),
    digits = 2, align = "lrrrr"
  ) %>%
  kable_styling(
    bootstrap_options = c("striped", "condensed"),
