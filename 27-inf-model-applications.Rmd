...
If we examined the data carefully, we would see that there is collinearity\index{collinear} among some predictors.
...
In Chapter \@ref(model-mlr), models were compared using $R^2_adj.$ In Chapter \@ref(inf-model-mlr), however, a computational approach was introduced to compare models by removing chunks of data one at a time and assessing how well the variables predicted the observations that had been held out.

Figure \@ref(fig:cvresidmario) was created by cross validating models with the same variables as in Table \@ref(tab:MarioKartModelOutput) and Table \@ref(tab:MarioKartModelFullOutput).
