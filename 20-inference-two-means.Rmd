...
[^inference-two-means-3]: The bootstrapped differences in sample means vary roughly from 4 to 12, a range of 8.
    Using the empirical rule (that with bell-shaped distributions, most observations are within two standard errors of the center), the standard error of the difference in sample means is approximated to be 2 units of pumping capacity.
    You might note that the standard error calculation given in Section \@ref(math2samp) is $SE(\bar{x}_{ESC} - \bar{x}_{Control}) = \sqrt{s^2_{ESC}/n_{ESC} + s^2_{Control}/n_{Control}} = \sqrt{5.17^2/9 + 2.76^2/9} = 1.95$, very close to the bootstrap approximation.
