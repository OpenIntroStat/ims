...
#| include: false
source("_common.R")
paralympic_1500_raw <- read_csv("~/Desktop/openintro/data-raw/paralympic_1500/paralympic_1500.csv")
...
## Case study: Olympic 1500m {#case-study-passwords}
...
The Paralympic Games began as a way to support soldiers who had been wounded in World War II as a way to help soldiers use sport to rehabilitate.
The first Paralympic Games were held in Rome, Italy in 1960.
Since 1988 (Seoul, South Korea), the Paralymic Games have been held a few weeks later than the Olympic Games in the same city, in both the summer and winter.
...
T12 and T13 athletes have some visual impairment, and Olympic runners do not have visual impairment.
...
When you encounter a new dataset, taking a peek at the last few rows as we did in @tbl-paralympic-df-tail is almost instinctual.
It can often be helpful to look at the first few rows of the data as well to get a sense for other aspects of the data which may not be apparent int he last few rows.
...
The column information is provided in @tbl-paralympic-var-def.
...
The lubridate R package is fantastic for working with date and time variables.
Sometimes the data dictionary (presented in @tbl-paralympic-var-def) isn't sufficient, and we need to go back to the data source and try to understand the data better before we can proceed with the analysis meaningfully.
:::
...
There are a large number of coutries who have had a single gold medal winner of the 1500m.
Similarly, there are a large number of countries who have hosted the Games only once.
Over the last century, the name describing the country for athletes from one particular region has changed and includes Russian Federaion, Unified Team, and Russian Paralympic Committee.
...
#| fig-cap: Distributions of categorical variables in the `paralympic_1500` dataset. 
#|   Plot A shows the distribution of the country of origin of the athlete; Plot B
#|   shows the distribution of the country in which the Games gook place.
#| fig-asp: 1.0
#| out.width: 100%
...
We already know that the 1500m times are mostly between 3.5min and 4.5min, based on @tbl-paralympic-df-tail and @tbl-paralympic-df-head. We can break down the 1500m time by division and type of race.
...
#| tbl-cap: Mean, minimum, and maximum of the gold medal times for the 1500m race
...
@fig-paralympic-ungrouped shows a scatterplot describing 1500m race times and year for Men's Olympic and Paralympic (T11) athletes with a line of best fit (to the entire dataset) superimposed.
...
The line of best fit will be further explored in @sec-model-slr.

```{r}
#| label: fig-paralympic-ungrouped
#| fig-cap: 1500m race time for Men's Olympic and Paralympic (T11) athletes.  The line represents a line of best fit to the entire dataset.
#| fig-asp: 1.2
#| out.width: 100%
...
#| fig-cap: 1500m race time for Men's Olympic and Paralympic (T11) athletes.  The best fit line is now fit separately to the Olympic and Paralympic athletes.
#| fig-asp: 1.2
#| out.width: 100%
...
paralympic_1500 |>
  filter(division == "Men", type == "Olympic" | type == "T11") |>
  filter(year > 1950) |>
  ggplot(aes(x = year, y = time_min, group = type, color = type)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  labs(
    x = "Year",
...
An association or relationship between two variables in one direction (e.g., positive) that reverses (e.g., becomes negative) when broken down by a third variable is known as Simpson's paradox.
:::
...
Simpson's paradox can be seen with categorical or numeric data.
Oftentimes the paradox happens because the third variable (here, race type) is imbalanced.
