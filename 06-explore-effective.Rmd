...
```{r brexit-bars, fig.cap = "Bar plot three different ways. The first graph is the default (with bars alphabetically ordered); the middle graph sorts the bars in descending order; the last graph orders the bars in the same order the original question was asked.", fig.asp = 0.35, fig.width = 10}
...
```{r brexit-region, fig.cap = "Segmented bar plots vertically and horizontally. The horizontal orientation makes the region labels easier to read.", fig.asp = 0.35}
brexit <- brexit %>%
  mutate(
    region = fct_relevel(
      region,
...
```{r seg-three-ways, fig.cap = "Three different representations of the two variables including survey opinion and region. Use the graphic that best conveys the data narrative at hand.", fig.asp = 0.5, fig.width = 10}
...
```{r brexit-viridis, fig.cap = "Identical bar plots with two different coloring options. The left uses default rainbow colors; the right uses viridis colors.", fig.width = 10}
