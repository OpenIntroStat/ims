...
```{r pie-to-bar, out.width="30%", fig.show = 'hold', fig.ncol = 2, fig.cap = "A pie chart (with added irrelevant features) as compared to a simple bar plot."}
...
```{r include=FALSE}
terms_chp_6 <- c("bar plot", "pie chart")
```
...
```{r red-bar, fig.width = 5, fig.asp=0.8, fig.cap = "The default coloring in the first bar plot does nothing for the udnerstanding of the data.  In the second plot, the color draws attention directly to the bar on Buildings and Administration."}
p_bad <- ggplot(expenses, aes(y = fct_infreq(category), fill = category)) +
  geom_bar(show.legend = FALSE) +
  scale_color_openintro("five") +
  labs(x = NULL, y = NULL)

p_good <- ggplot(expenses, aes(y = fct_infreq(category), fill = category)) +
  geom_bar(show.legend = FALSE) +
  scale_fill_manual(values = c(IMSCOL["red", "full"], rep(IMSCOL["lgray", "full"], 4))) +
  labs(x = NULL, y = NULL)
...
```{r include=FALSE}
terms_chp_6 <- c(terms_chp_6, "color")
```
...
```{r include=FALSE}
terms_chp_6 <- c(terms_chp_6, "order")
```
...
```{r brexit-bars, out.width="30%", fig.show = 'hold', fig.ncol = 3, fig.cap = "Bar plot three different ways.  The first graph is the default (with bars alphabetically ordered); the middle graph sorts the bars in descending order; the last graph orders the bars in the same order the original question was asked."}
p_bad <- ggplot(data = brexit, aes(x = opinion)) +
  geom_bar() + 
  labs(x = "Opinion", y = "Count")

p_desc <- ggplot(data = brexit, aes(x = fct_infreq(opinion))) +
  geom_bar() + 
  labs(x = "Opinion", y = "Count")

p_ordered <- ggplot(data = brexit, 
                    aes(x = fct_relevel(opinion, c("Very well", "Fairly well", "Fairly badly", "Very badly", "Don't know")))) +
  geom_bar() + 
    labs(x = "Opinion", y = "Count")
...
```{r Brex-region,  out.width="45%", fig.show = 'hold', fig.ncol = 2, fig.cap = "Segmented bar plots vertically and horizontally.  The horizontal orientation makes the region labels easier to read."}
brexit <- brexit %>%
  mutate(
    region = fct_relevel(
      region,
...
p_bad <- ggplot(data = brexit, aes(x = region, fill = Opinion)) +
  geom_bar() +
  scale_fill_manual(values = c(IMSCOL["green", "full"], IMSCOL["blue", "full"], IMSCOL["pink", "full"], IMSCOL["red", "full"], IMSCOL["gray", "full"]))
...
```{r include=FALSE}
terms_chp_6 <- c(terms_chp_6, "labels")
```
...
If a separate bar plot for each region makes the point you'd like, use the bottom image which has been **faceted** by region.

```{r include=FALSE}
terms_chp_6 <- c(terms_chp_6, "facet")
```
...
```{r seg-three-ways,fig.cap = "Three different representations of the two variables including survey opinion and region.  Use the graphic that best conveys the data narrative at hand."}
p_bad <- ggplot(data = brexit, aes(y = region,  fill = Opinion)) +
...
The Viridis scale[^explore-effective-2] works well with ordinal data.
...
[^explore-effective-2]: originally developed as colormap in the Matplotlib library of Python <https://bids.github.io/colormap>

```{r Brexit-viridis, out.width="45%", fig.show = 'hold', fig.ncol = 2, fig.cap = "Identical bar plots with two different coloring options.  The left uses default rainbow colors; the right uses viridis colors."}
p_bad <- ggplot(data = brexit, aes(x = region, fill = Opinion)) +
  geom_bar(position = "fill") +
  coord_flip() +
  labs(
