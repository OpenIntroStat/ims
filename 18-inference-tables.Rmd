...
\end{align*} Adding the computed value for each cell gives the chi-squared test statistic $X^2:$ \begin{align*}
...
```{r ipodRandDotPlot, fig.cap="A stacked dot plot of chi-squared statisics from 1000 simulations produced under the null hypothesis, $H_0,$ where the question is independent of the disclosure. None of the 1000 simulations had a chi-squared value of at least 40.13, the chi-squared value observed in the study.  Indeed, none of the simulated chi-squared statistics came anywhere close to the observed statistic!"}
...
plot(X, Y, xlim = c(0, 40), xlab = "Chi-squared statistics assuming a true null hypothesis.", axes = FALSE, ylim = c(0, max(Y)), col = IMSCOL["blue", "full"], cex = 0.8, pch = 1, lwd = 1.5)
axis(1, at = seq(0, 40, 2.5), labels = c(0, "", 5, "", 10, "", 15, "", 20, "", 25, "", 30, "", 35, "", 40))
theta <- seq(0, 2 * pi, length.out = 500)
points(X[X >= 40.1], Y[X >= 40.1], lwd = 3, col = IMSCOL["red", "full"], cex = 0.4)
```

## Mathematical model {#mathchisq}

### The chi-squared test of $H_0:$ independence
