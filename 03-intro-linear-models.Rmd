...
```{r r-squared-explanation, fig.cap = "For these 50 students, the range of gift aid is $32.7K - $7K = $25.7K. Among students with roughly the same family income (e.g., 100K), the range in gift aid is reduced to about $15.6K, or about a 40% reduction, which matches $R^2 = 0.48$, or 48%."}
ggplot(elmhurst, aes(x = family_income, y = gift_aid)) +
  geom_point(col = COL["blue", "full"], size = 2, alpha = 0.8) +
  geom_smooth(method = "lm", color = "black", size = 0.5, se = FALSE) +
  scale_y_continuous(labels = label_dollar(scale = 1, suffix = "K", accuracy = 1)) +
  ...
  annotate("rect", 
           xmin = 85, xmax = 115, 
           ymin = min(elmhurst$gift_aid[elmhurst$family_income > 85 & elmhurst$family_income < 115]), 
           ymax = max(elmhurst$gift_aid[elmhurst$family_income > 85 & elmhurst$family_income < 115]),
           fill = COL["gray","full"], alpha = 0.2) +
  annotate("text", 
           x = 80, y = 16, 
           hjust = 1, vjust = 1, 
           label = "Variability in gift aid\nfor students with\nfamily income\naround 100K",
           color = COL["gray", "full"], size = 5) +
  annotate("rect", 
           xmin = max(elmhurst$family_income)-30, 
           xmax = max(elmhurst$family_income), 
           ymin = min(elmhurst$gift_aid), 
           ymax = max(elmhurst$gift_aid),
           fill = COL["green","full"], alpha = 0.2) +
  annotate("text", x = 240, y = 30, 
           hjust = 1, vjust = 1, 
