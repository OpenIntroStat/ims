...
Plot A in Figure \@ref(fig:scattHeadLTotalL-sex-age) shows the relationship between total length and head lentgh of brushtail possums, taking into consideration their sex. 
...
::: {.example}
The linear fit shown in Figure \@ref(fig:scattHeadLTotalLLine-highlighted) is given as $\hat{y} = 42.7 + 0.57x$.
...
$$\hat{y} = 42.7+0.57x = 42.7+0.57\times 76.0 = 86.0$$


Next we compute the difference of the actual head length and the predicted head length: 

$$e = y - \hat{y} = 85.1 -  86.0 = -0.9$$

The model's error is $e = -0.9$mm, which is very close to the
