...
\begin{align}  
\widehat{rate} &= 1.921 \\
&+ 0.974 \times \texttt{verified_income}_\texttt{Source only} \\
...
Instead of eliminating variables one-at-a-time, we add variables one-at-a-time until we cannot find any variables that improve the model (as measured by adjusted $R^2$).

```{r include=FALSE}
terms_chp_4 <- c(terms_chp_4, "forward selection")
```

::: {.workedexample}
Construct a model for predicting `rate` from the `loans` data using forward selection.
...
If trying both techniques and this happens, it's common to choose the model with the larger $R_{adj}^2$.
:::
