...
(See Section \@ref(categorical-predictor-two-levels) for a review of the interpretation for two-level categorical predictor variables.) Examining the regression output in Table \@ref(tab:intRateVsPastBankrModel), we can see that the p-value for is very close to zero, indicating there is strong evidence the coefficient is different from zero when using this simple one-predictor model.
:::
...
$$\widehat{\texttt{interest_rate}} = 11.10 + 1.42 \times \texttt{verified_income}_{\texttt{Source Verified}} + 3.25 \times \texttt{verified_income}_{\texttt{Verified}}$$
...
When `verified_income` takes a value of `Source Verified`, then the corresponding variable takes a value of 1 while the other ($\texttt{verified_income}_{\texttt{Verified}}$) is 0:
...
[^multi-logistic-models-10]: We did omit discussion of some structure in the data for the analysis presented: the experiment design included blocking, where typically four resumes were sent to each job: one for each inferred race/sex combination (as inferred based on the first name).
...
Race and sex are in the United States, meaning they are not legally permitted factors for hiring or employment decisions.
...
kable(resume_variables,
      caption = "Descriptions for the `received_callback` variable along with 8 other variables in the `resume` data set. Many of the variables are indicator variables, meaning they take the value 1 if the specified characteristic is present and 0 otherwise.")
```
...
::: {.important}
**Notation for a logistic regression model.** The outcome variable for a GLM is denoted by $Y_i$, where the index $i$ is used to represent observation $i$.
...
As with most applied data problems, we substitute the point estimates for the parameters (the $\beta_i$) so that we can make use of this formula.
...
$$
\begin{aligned}  &log_e \left(\frac{p}{1 - p}\right) \\    &\quad= - 2.7162        - 0.4364 \times \texttt{job_city}_{\texttt{Chicago}}        + 0.0206 \times \texttt{years_experience}        + 0.7634 \times \texttt{honors} \\      &\quad\qquad          - 0.3443 \times \texttt{military}          + 0.2221 \times \texttt{email}          + 0.4429 \times \texttt{race}_{\texttt{white}}          - 0.1959 \times \texttt{sex}_{\texttt{man}} \\    &\quad= - 2.7162        - 0.4364 \times 1        + 0.0206 \times 14        + 0.7634 \times 0 \\      &\quad\qquad          - 0.3443 \times 0          + 0.2221 \times 1          + 0.4429 \times 1          - 0.1959 \times 1 \\    &\quad= - 2.3955  \end{aligned}
$$
...
::: {.workedexample}
Let's consider a sex-imbalanced company that consists of 20% women and 80%[^multi-logistic-models-11] men, and we'll suppose that the company is very large, consisting of perhaps 20,000 employees.
...
[^multi-logistic-models-11]: A more thoughtful example would include non-binary individuals.
...
Additionally, if we would complete a handful of examples like the one above with different numbers, we'd learn that the greater the imbalance in the population groups, the more the smaller group is disproportionately impacted.[^multi-logistic-models-12]

[^multi-logistic-models-12]: If a proportion $p$ of a company are women and the rest of the company consists of men, then under the hypothetical situation the ratio of rates of discrimination against women vs men would be given by $\frac{1 - p}{p}$; this ratio is always greater than 1 when $p < 0.5$.
