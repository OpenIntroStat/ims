...
In our investigations, we will identify a reasonable point estimate of $p_1 - p_2$ based on the sample, and you may have already guessed its form: $\hat{p}_1 - \hat{p}_2.$ \index{point estimate!difference of proportions} Then we will look at the inferential analysis in three different ways: using a randomization test, applying bootstrapping for interval estimates, and, if we verify that the point estimate can be modeled using a normal distribution, we compute the estimate's standard error, and we apply the mathematical framework.
:::
...
Let us take another look at the cardiopulmonary resuscitation (CPR) study we introduced in Chapter \@ref(two-sided-hypotheses).
...
Whether mammograms should be used is part of a controversial discussion, and it is the topic of our next example where we learn about 2-proportion hypothesis tests when $H_0$ is $p_1 - p_2 = 0$ (or equivalently, $p_1 = p_2).$
...
No intervention was made during the following 25 years of the study, and we will consider death resulting from breast cancer over the full 30-year period.
...
This proportion is an estimate of the breast cancer death rate across the entire study, and it is our best estimate of the proportions $p_{MGM}$ and $p_{C}$ *if the null hypothesis is true that* $p_{MGM} = p_{C}.$ We will also use this pooled proportion when computing the standard error.
...
-   If mammograms are helpful or harmful, the data suggest the effect is not very large.
...
Over your lifetime, about 95% of the confidence intervals you create will capture the parameter of interest, and about 5% will not.
