...
lm(weight ~ weeks, data = births14_100) %>%
  tidy() %>%
  kbl(booktabs = TRUE, caption = "The least squares estimates of the intercept and slope are given in the estimate column. The observed slope is 0.335.") %>%
...
p1 <- ggplot(births14_100, aes(x = weeks, y = sample(weight))) +
  ...
  geom_smooth(method = "lm", se = FALSE, fullrange = TRUE) +
  labs(
    x = "Permuted weight of baby (pounds)",
    y = "Length of gestation (weeks)",
...
lm(house_change ~ unemp, d[th, ]) %>%
  tidy() %>%
  kbl(booktabs = TRUE, caption = "Output from statistical software for the regression line modeling the midterm election losses for the President's party as a response to unemployment.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = c("striped", "hold_position"))
```
...
lm(gift_aid ~ family_income, d) %>%
  tidy() %>%
  kbl(booktabs = TRUE, caption = "Summary of least squares fit for the Elmhurst College data, where we are predicting the gift aid by the university based on the family income of students.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = c("striped", "hold_position"))
```
