...
```{r include=FALSE}
terms_chp_25 <- c("inference with single precictor regression")
```
...
```{r sandpop, fig.cap = "Revenue as a linear model of advertising dollars for a population of sandwich stores, in thousands of dollars.", echo = FALSE, message = FALSE, warning = FALSE}
set.seed(4747)
popsize <- 1000
...
```{r include=FALSE}
terms_chp_25 <- c(terms_chp_25, "variability of the slope")
```
...
Consider data on 100 randomly selected births gathered originally from the US Department of Health and Human Services.[^inf-model-slr-1]
Some of the variables are plotted in Figure \@ref(fig:babyweight).

[^inf-model-slr-1]: The data used in this section can be found in the **openintro** R package: [`births14`](http://openintrostat.github.io/openintro/reference/births14.html).
...
```{r babyweight, echo = FALSE, fig.cap = "Weight of baby at birth (in lbs) as plotted by four other birth variables (mother's weight gain, mother's age, number of hospital visits, and weeks gestation)."}
...
lm(weight ~ weeks, data = births14_100) %>%
  tidy() %>%
  kbl(caption = "The least squares estimates of the intercept and slope are given in the `estimate` column.  The observed slope is 0.335.") %>%
  kable_styling()
```
...
```{r permweekslm, fig.cap = "Two different permutations of the `weight` variable with slightly different least squares regression lines.", fig.show = 'hold', fig.ncol = 2, out.width='47%'}
set.seed(470)
ggplot(births14_100, aes(x = weeks, y = sample(weight))) +
  ...
  ylab("permuted weight of baby") +
  xlab("weeks gestation") +
  ggtitle("First permutation of weight") 
...
ggplot(births14_100, aes(x = weeks, y = sample(weight))) +
  ...
  ylab("permuted weight of baby") +
  xlab("weeks gestation") +
  ggtitle("Second permutation of weight") 
```
...
```{r nulldistBirths, echo=FALSE, fig.cap = "Histogram of slopes given different permutations of the `weight` variable.  The vertical red line is at the observed value of the slope, 0.335."}
...
```{r magePlot, echo = FALSE, fig.cap = "Original data: `weight` of baby as a linear model of mother's age.  Notice that the relationship between `mage` and `weight` is not as strong as the relationship we saw previously between `weeks` and `weight`."}
ggplot(births14_100) +
...
ggplot(births5) +
  ...
  xlab("mother's age") +
  ylab("weight of baby") +
  ggtitle("Bootstrap sample from the births data.") +
  geom_point(x = 29, y = 9, color = IMSCOL["green", "full"], pch = 1, size = 8) +
  geom_point(x = 20, y = 6, color = IMSCOL["red", "full"], pch = 1, size = 8) +
  geom_point(x = 39, y = 9, pch = 1, size = 8) #+
  #ylim(c(1.4, 3.6))
...
```{r birthBS, echo = FALSE, fig.cap = "Repeated bootstrap resamples of size 100 are taken from the original data.  Each of the bootstrapped linear model is slightly different."}
...
```{r mageBSslopes, fig.cap = "The original births data on `weight` and `mage` is bootstrapped 1,000 times. The histogram provides a sense for the variability of the slope of the linear model slope from sample to sample.", warning=FALSE, fig.width=15}
...
ggplot(births_many_lm_BS, aes(x = estimate)) +
  geom_histogram() +
  annotate("segment", x = lower, xend = lower, y = 0, yend = 20, linetype = "dashed") +
  annotate("segment", x = upper, xend = upper, y = 0, yend = 20, linetype = "dashed") +
  annotate("text", x = lower, y = 25, label = "2.5 percentile\n-0.01", size = 6) +
  annotate("text", x = upper, y = 25, label = "97.5 percentile\n0.081", size = 6) +
  labs(
    x = "Bootstrapped values of the slope statistic describing weight and mother's age",
    y = NULL
  ) +
  theme(axis.text.y = element_blank())
```

## Mathematical model for testing $\beta_1$ {#mathslope}
...
Why or why not?[^inf-model-slr-2]
:::

[^inf-model-slr-2]: The answer to this question relies on the idea that statistical data analysis is somewhat of an art.
...
```{r include=FALSE}
terms_chp_25 <- c(terms_chp_25, "t-distribution for slope")
```
...
::: {.workedexample data-latex=""}} What do the first and second columns of Table \@ref(tab:midtermUnempRegTable) represent?
...
::: {.workedexample data-latex=""}} Use Table \@ref(tab:midtermUnempRegTable) to determine the p-value for th hypothesis test
...
::: {.workedexample data-latex=""}} Examine Figure \@ref(fig:elmhurstScatterWLSROnly-CLTsection), which relates the Elmhurst College aid and student family income.
...
Use the output to formally evaluate the following hypotheses.[^inf-model-slr-3]

-   $H_0$: The true coefficient for family income is zero.
-   $H_A$: The true coefficient for family income is not zero.
:::

[^inf-model-slr-3]: We look in the second row corresponding to the family income variable.
...
## Mathematical model, interval for $\beta_1$
...
::: {.workedexample data-latex=""}} Compute the 95% confidence interval for the coefficient using the regression output from Table \@ref(tab:rOutputForIncomeAidLSRLineInInferenceSection).
...
```{r include=FALSE}
terms_chp_25 <- c(terms_chp_25, "techinical conditions linear regression")
```
...
::: {.guidedpractice data-latex=""}
Should we have concerns about applying least squares regression to the Elmhurst data in Figure \@ref(fig:elmhurstScatterW2Lines)?[^inf-model-slr-4]
:::

[^inf-model-slr-4]: The trend appears to be linear, the data fall around the line with no obvious outliers, the variance is roughly constant.
