...
ggplot(births_many_lm_BS, aes(x = estimate)) +
  geom_histogram() +
  annotate("segment", x = lower, xend = lower, y = 0, yend = 18, linetype = "dashed") +
  annotate("segment", x = upper, xend = upper, y = 0, yend = 18, linetype = "dashed") +
  annotate("text", x = lower, y = 20, label = "2.5 percentile\n-0.01", size = 6) +
  annotate("text", x = upper, y = 20, label = "97.5 percentile\n0.081", size = 6) +
  labs(
    x = "Bootstrapped values of the slope statistic describing `weight` and `mage`",
    y = NULL
  ) +
