...
-   **Part 3: Regression modeling.** Modeling numerical and categorical outcomes with linear and logistic regression and using model results to describe relationships and made prediction.
...
Each part contains multiple chapters and ends with a case study that uses the tools and techniques introduced in the part and builds on the content of the part for a high level overview.
...
These are self-paced tutorials developed using the [learnr](https://rstudio.github.io/learnr/index.html) R package and you only need a browser to complete them.
