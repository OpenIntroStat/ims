...
Four observations from this dataset are shown in Table \@ref(tab:mariokart-data-frame), and descriptions for each variable are shown in Table \@ref(tab:mariokart-var-def).
Notice that the condition and stock photo variables are indicator variables\index{indicator variable}, similar to `bankruptcy` in the `loans` dataset from Chapter \@ref(inf-model-mlr).
