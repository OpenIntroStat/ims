...
But even when describing how a statistic varies, there is a further consideration with respect to the experimental design (e.g., does the statistic vary from random sample to random sample? or does it vary from random allocation to random allocation?).
The methods presented in this text (and used in science generally) are typically used interchangeably across ideas of random samples or random allocation of the treatment.
Oftentimes, two different analysis methods will give equivalent conclusions.
